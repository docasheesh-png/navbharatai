  ...
  return (
    <div className="bg-card border border-line rounded-2xl p-5 space-y-4">
      <div className="flex items-center gap-2">
        <Box className="w-4 h-4 text-accent-text" />
        <h3 className="text-xs font-black text-ink uppercase tracking-widest">Team Library</h3>
        <span className="text-[10px] text-faint ml-auto">Shared prompts · templates · components</span>
      </div>

      {/* Add form */}
      <div className="bg-surface rounded-xl p-3 border border-line space-y-2">
        <div className="flex gap-2">
          {KINDS.map((k) => (
            <button key={k} onClick={() => setKind(k)}
              className={`text-[11px] font-bold px-2.5 py-1 rounded-full border capitalize ${kind === k ? 'bg-indigo-500/20 border-indigo-500/50 text-accent-text' : 'bg-well border-line text-muted'}`}>{k}</button>
          ))}
        </div>
        ...
        <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Content to reuse…" rows={3}
          className="w-full bg-card border border-line rounded-lg px-3 py-2 text-xs text-ink placeholder:text-faint focus:outline-none focus:border-indigo-500 resize-y" />
        {error && <p className="text-[11px] text-danger">{error}</p>}
        <button onClick={add} disabled={busy} className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 text-on-accent rounded-lg text-xs font-bold disabled:opacity-50">
      ...
      {/* Items */}
      {items.length === 0 ? (
        <p className="text-[11px] text-faint">No saved items yet. Save a prompt, template, or component your team can reuse.</p>
      ) : (
        <div className="space-y-2">
          {pagedItems.visible.map((item) => (
            <div key={item.id} className="bg-surface rounded-xl p-3 border border-line">
              <div className="flex items-center gap-2">
                <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded-full bg-raised border border-line text-muted capitalize">{item.kind}</span>
                <span className="text-xs font-bold text-ink truncate">{item.title}</span>
                <button onClick={() => copy(item)} className="ml-auto p-1 text-muted hover:text-accent-text shrink-0" aria-label="Copy">
                  {copiedId === item.id ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                </button>
                <button onClick={() => remove(item.id)} className="p-1 text-muted hover:text-danger shrink-0" aria-label="Delete">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
              <pre className="text-[10px] text-muted mt-1.5 whitespace-pre-wrap break-words max-h-24 overflow-auto font-mono">{item.content.slice(0, 400)}</pre>
            </div>
          ))}
