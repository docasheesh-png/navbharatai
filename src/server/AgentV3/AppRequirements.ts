...
const STRINGS: Record<string, NoticeStrings> = {
  ...
  ml: {
    ...
    add: 'ചേർക്കുക', tail: 'അതുവരെ ആ ഭാഗങ്ങൾ demo mode-ൽ ആയിരിക്കും — ബാക്കി ആപ്പ് പൂർണ്ണമായി പ്രവർത്തിക്കും.',
  },
  ar: {
    head: (n) => `🔑 ایپ بن گئی ہے۔ اِسے پوری طرح چلانے کے لیے ${n === 1 ? '1 چیز' : `${n} چیزیں`} آپ کے اپنے account سے چاہیے:`,
    add: 'ڈالیں', tail: 'تب تک وہ حصے demo mode میں رہیں گے — باقی ایپ پوری چلے گی۔',
  },
};
...
const ENGLISH: NoticeStrings = {
  ...
  add: 'add',
  tail: 'Until then those parts stay in demo mode — the rest of the app works fully.',
};
