...
import { findInCatalog } from '../lib/heroObjectSpec';
import { PLACEMENT } from '../lib/objectCatalogTypes';
import { resolveObjectSpec, renderSpec, memoryCache } from '../lib/objectSpecProvider';
  ...
  private async run(call: ToolUse, agent: AgentRole): Promise<string> {
    ...
    switch (call.name) {
      ...
      case 'bash': {
        ...
        // WHAT THE COMPILER ERROR MEANS (autopsy baa0b3c7). The model runs `tsc --noEmit` through bash far
        // more often than through the `typecheck` tool — the VPN build's own root cause line is literally
        // `$ ./node_modules/.bin/tsc --noEmit 2>&1 → exit 2` — and that path handed the output back raw.
        // A missing-declaration error names the file where the symbol is USED, so its remedy is nowhere in
        // the message: that build read `Property 'setState' does not exist on type 'ErrorBoundary'` six
        // times and rewrote a CORRECT file four times because React's types were simply not installed.
        // Keyed on output that really parses as tsc errors, so an ordinary command is never annotated.
        try {
          const bashTscErrors = parseTscErrors(`${stdout}\n${stderr}`);
          if (bashTscErrors.length > 0) out = `${out}${tscCauseNote(tscErrorCauses(bashTscErrors))}`;
        } catch { /* the analysis is advisory — the raw output is still reported */ }
        if (risk.level !== 'none') {
          getWorkspaceMemory(this.workspaceId).recordAudit(
      ...
      case 'object_spec': {
        // WHEN THE LIST DOES NOT HAVE IT (admin 2026-09-14: "provider se banwao").
        //
        // 🔴 The reported bike was not badly BUILT — `createCar` proves this engine models beautifully
        // when it knows what it is modelling. It was built without anyone having decided what a bike
        // IS, so it went straight to primitives. The spec is the missing input, not the skill.
        //
        // Cheapest source first: the 100-object catalogue (instant, free, human-checked) → the process
        // cache → one small engine call, STRICTLY validated. And if every one of those is unavailable,
        // the spec-first PROTOCOL, which costs nothing and can never fail a build. That last rung is
        // why this tool is safe to put in front of every hand-modelled object.
        const osRec = (input as Record<string, unknown>) || {};
        const wanted = typeof osRec.object === 'string' ? osRec.object.trim() : '';
        if (!wanted) return 'Name the object to look up, for example: object_spec({ object: "auto rickshaw" }).';
        const known = findInCatalog(wanted);
        const spec = await resolveObjectSpec(wanted, {
          findInCatalog,
          cacheGet: memoryCache.get,
          cacheSet: memoryCache.set,
          // The FREE router namespace, which by construction never reaches Claude — so this stays
          // inside the weak-tier rule, and its leader is a zero-cost model. It never throws.
          ask: async (prompt, system) => {
            const r = await aiRouter.routeDetailed(prompt, [], 'navbharat', undefined, system);
            return r.ok ? r.content : null;
          },
        });
        const placement = known ? PLACEMENT[known.category] : undefined;
        const body = [
          renderSpec(spec, { builder: known?.builder }),
          placement
            ? '\n  HOW TO PLACE IT — ' + placement.headline + '\n'
              + placement.rules.map((r) => '  - ' + r).join('\n')
            : '',
          // An object the engine had to invent is worth PROMOTING into the permanent catalogue by
          // hand, where someone checks its numbers. The tool RESULT is the admin-visible record, so
          // the note rides it rather than needing plumbing the dispatcher does not have.
          spec.source === 'provider'
            ? '\n[OBJECT_SPEC_LEARNED] No catalogue entry for "' + wanted + '" — this spec was '
              + 'generated by the engine. Worth adding to objectCatalog.ts if it recurs.'
            : '',
        ].filter(Boolean).join('\n');
        return body;
      }
