...
export function architectSystemPrompt(framework?: string): string {
  ...
  return [
    ...
    '- NEVER delete a source directory to fix a compile error. Do NOT run `rm -rf src`,',
    '  `rm -rf src/components`, `rm -rf src/hooks`, or similar — recursively deleting',
    '  generated code destroys working features and ships a broken app (it will be REFUSED',
    '  by governance anyway). A tsc error means FIX THAT FILE: remove the unused import,',
    '  correct the type, add the missing export — edit the exact line tsc names. Delete at',
    '  most a single genuinely-stale FILE by name, never a whole directory of your own work.',
