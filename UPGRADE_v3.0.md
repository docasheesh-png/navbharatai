...
| U-5 | ✅ **DONE (2026-07-06)** — **Cost-alerting thresholds** on real month-to-date spend vs the user's monthly budget. Pure `CostAlertEngine.ts` (`buildCostAlertReport`, unit-tested): 80% → amber "approaching", ≥100% → red "exceeded"; no budget set → no alert (nothing invented). `GET /api/profile/cost-alerts` (own data, USD→INR via canonical `usdToInr`). Banner on `ProfilePage.tsx`; `AppKnowledgeBase` updated. Files: `src/server/lib/CostAlertEngine.ts` (+`.test.ts`, 9), `routes/profile.ts`, `ProfilePage.tsx`, `AppKnowledgeBase.ts`. | Lead F3/8 | closes the billing category to a clean Lead | S |
...
| U-7 | 🟡 **FOUNDATION DONE (2026-07-06)** — **Public API keys + v1 endpoint**. Pure `ApiKeyManager.ts` (generate/hash/verify **timing-safe**, scopes, header extraction; unit-tested) — plaintext shown ONCE, only the SHA-256 hash stored. `ApiKeyStore.ts` (Firestore, VITEST-skip, ownership-enforced revoke, lookup-by-hash). Routes: `POST/GET/DELETE /api/keys` (Firebase-authed key management) + key-gated `GET /api/v1/me` (scope `read:profile`) proving keys work end-to-end. `ApiKeysCard.tsx` UI in My Profile (create/reveal-once/revoke). OpenAPI + AppKnowledgeBase updated. Live smoke: missing/bogus key → 401, scope guard enforced. **STILL TODO:** headless build trigger via API + the `nbai` CLI (the automation half). Files: `ApiKeyManager.ts`+test, `ApiKeyStore.ts`, `routes/apiKeys.ts`, `ApiKeysCard.tsx`, `server.ts`, `apiContract.ts`, `AppKnowledgeBase.ts`. | Lead F6 | automation, programmatic/headless builds | M |
| U-8 | **Teams / RBAC / SSO + append-only platform audit-log** (the SOC2 *substrate*) | Lead F7 | enterprise + the controls an auditor checks | L |
| U-9 | ✅ **DONE (2026-07-06)** — **Docs site auto-generated from AppKnowledgeBase**. Pure `KnowledgeDocs.ts` (`buildDocsModel` groups all 158 features → builder/chat/professionals/platform sections; `renderDocsHtml` → self-contained, CSP-safe, searchable page; HTML-escaped). Served: `GET /guide` (site) + `GET /api/knowledge-base` (JSON), wired in `server.ts` (added `/guide` to the SPA-fallback allowlist). Live prod smoke: `/guide` 200 docs HTML, `/api/knowledge-base` JSON. Template Gallery NOT rebuilt — already ships as the in-app Templates panel (`CURATED_TEMPLATES`, `project-templates`); docs link to it. Files: `src/server/lib/KnowledgeDocs.ts` (+`.test.ts`, 11), `server.ts`, `AppKnowledgeBase.ts`. | Lead F8 | onboarding depth + community substrate | M |
...
| U-15 | ✅ **DONE (2026-07-06)** — **Status page + deep `/api/health`**. Pure `HealthReport.ts` (`buildHealthReport` → overall status from ready + maintenance + per-dependency checks; `formatUptime`; `renderStatusPageHtml` self-contained CSP-safe page; unit-tested). `GET /api/health` now a DEEP report (real uptime/memory/version/node + checks: init, Firestore backup, AI providers, maintenance) — **removed the legacy shallow inline handler** that shadowed it (root-cause). Public `/status` page polls it (15s). Live prod smoke: `/api/health` deep JSON (honest `degraded` when backup unset), `/status` 200. Files: `src/server/lib/HealthReport.ts` (+`.test.ts`, 9), `routes/health.ts`, `server.ts`, `AppKnowledgeBase.ts`. **Honest:** current-instance uptime is real; the long-term SLA/uptime *record* still accrues from monitoring over time (Bucket C). | Lead F10 | substrate; SLA/uptime *record* is earned over time (Bucket C) |
