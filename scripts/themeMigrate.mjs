 ...
 */
export const SOLID_FILL = /(?<![\w-])(?:[a-z-]+:)*bg-(?:indigo|blue|emerald|green|red|rose|purple|violet|fuchsia|amber|orange|teal|cyan|pink|sky)-(?:500|600|700)(?:\/(?:[6-9]\d|100))?(?![\w/-])|(?<![\w-])(?:[a-z-]+:)*bg-gradient-/;
 ...
 */
export function fillContext(line, idx) {
...
export function mapToken(base, { onSolidFill = false } = {}) {
  const m = base.match(/^(text|bg|border(?:-[trblxy])?|divide|placeholder|ring|decoration)-(.+?)(?:\/(\d{1,3}))?$/);
  if (!m) return null;
  ...
  if (prop === 'bg') {
    ...
    if (/^(zinc|neutral|gray|slate|stone)-950$/.test(value)) return { token: opacity >= 80 ? 'bg-surface' : 'bg-raised', kind: 'exact' };
    return null;
  }
  if (prop === 'decoration') {
...
export function migrate(src) {
  ...
  const out = normalised.split('\n').map((line) => line.replace(LITERAL, (m, v1, v2, offset) => {
    const variant = v1 ?? v2 ?? '';
    const base = m.slice(variant.length);
    const ctx = /(?:text|placeholder)-white/.test(base) ? fillContext(line, offset) : 'no';
