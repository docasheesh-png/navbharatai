...
export const APP_KNOWLEDGE_BASE: AppFeature[] = [
  // ─── 🧪 TEST CHAT (isolated AWS Bedrock GLM-5 validation) ────────────────
  {
    ...
    description:
      'An isolated, debug-only chat page for validating AWS Bedrock GLM-5 access independently of the production AI. It sends your message straight to Amazon Bedrock’s Mantle endpoint (the OpenAI-compatible Chat Completions API at https://bedrock-mantle.<region>.api.aws/v1 — the same endpoint the new Bedrock console/Workbench uses for GLM/Qwen and other third-party models) and shows the reply plus a full debug panel (endpoint, HTTP status, AWS request id, exact request/response payloads, and the COMPLETE raw AWS error on failure). It does not use, and cannot affect, the production chat, provider router, or model selector. It is off unless the TEST_CHAT_ENABLED environment variable is set to true, and it needs AWS_REGION, BEDROCK_GLM_MODEL, and a Bedrock API key as AWS_BEARER_TOKEN_BEDROCK configured on the server.',
    howToUse:
