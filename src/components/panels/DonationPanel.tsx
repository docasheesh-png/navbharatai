    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-8 relative min-h-screen transition-colors duration-500 bg-surface">
        ...
        <div className="sticky top-0 right-0 z-50 flex justify-end pb-4">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl active:scale-95 ${
              isDonationEditing
                ? 'bg-emerald-600 shadow-emerald-500/20 text-on-accent'
                : 'bg-indigo-600 shadow-indigo-600/20 text-on-accent'
            }`}
          >
      ...
      <div className="max-w-3xl mx-auto space-y-8 pb-12">
        ...
        <motion.div
          ...
          className="bg-card border border-line rounded-[2.5rem] p-8 sm:p-12 text-center shadow-3xl relative overflow-hidden group"
        >
          <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-orange-500 via-white to-green-500 text-on-accent" />
            ...
            {donationData.logoUrl ? (
              <div className="relative">
                ...
                {isAdmin && isDonationEditing && (
                  <label className="absolute -bottom-2 -right-2 p-2 bg-indigo-600 rounded-xl cursor-pointer shadow-lg hover:bg-indigo-700 transition-colors text-on-accent">
                    <Camera className="w-3.5 h-3.5 text-on-accent" />
              ...
              <div className="relative">
                <div className="w-20 h-20 bg-indigo-600/10 rounded-3xl flex items-center justify-center mx-auto border border-indigo-500/20 shadow-inner group-hover:rotate-12 transition-transform duration-500">
                  <HeartHandshake className="w-10 h-10 text-accent-text" />
                </div>
                {isAdmin && isDonationEditing && (
                  <label className="absolute -bottom-2 -right-2 p-2 bg-indigo-600 rounded-xl cursor-pointer shadow-lg hover:bg-indigo-700 transition-colors text-on-accent">
                    <Upload className="w-3.5 h-3.5 text-on-accent" />
                    <input type="file" className="hidden" accept="image/*" onChange={e => onFileUpload(e, 'logoUrl')} />
                  </label>
                )}
              </div>
          ...
          <div className="relative group/edit">
            {isDonationEditing ? (
              <input
                ...
                className="text-2xl sm:text-3xl font-black text-ink mb-4 tracking-tighter leading-tight bg-raised border border-line rounded-xl px-4 py-2 w-full text-center outline-none focus:border-indigo-500"
              />
            ) : (
          ...
          <div className="relative group/edit">
            {isDonationEditing ? (
              <input
                ...
                className="text-sm font-bold text-accent-text uppercase tracking-[0.2em] mb-8 bg-raised border border-line rounded-xl px-4 py-2 w-full text-center outline-none focus:border-indigo-500"
              />
            ) : (
          ...
          <div className="max-w-2xl mx-auto text-left space-y-6">
            <div className="relative group/edit">
              <div className="text-body text-base leading-relaxed font-medium">
                Namaskar, friends across India,
                <br /><br />
                {isDonationEditing ? (
                  <div className="space-y-4">
                    <div className="flex gap-2 items-center">
                      <span className="text-[10px] text-muted font-bold uppercase">Name:</span>
                      <input value={donationData.name} onChange={e => onDonationDataChange({ ...donationData, name: e.target.value })} className="bg-raised border border-line rounded-lg px-2 py-1 text-ink text-sm outline-none focus:border-indigo-500" />
                    </div>
                    <textarea value={donationData.missionStatement} onChange={e => onDonationDataChange({ ...donationData, missionStatement: e.target.value })} className="w-full bg-raised border border-line rounded-xl px-4 py-3 text-sm text-body outline-none focus:border-indigo-500 min-h-[100px]" />
                  </div>
                ) : (
            ...
            <div className="p-6 bg-raised rounded-3xl border border-line backdrop-blur-sm relative group/edit">
              {isDonationEditing ? (
                <textarea value={donationData.dreamStatement} onChange={e => onDonationDataChange({ ...donationData, dreamStatement: e.target.value })} className="w-full bg-raised border border-line rounded-xl px-4 py-3 text-sm text-muted italic text-center outline-none focus:border-indigo-500 min-h-[100px]" />
              ) : (
            ...
            <div className="space-y-4">
              <div className="text-xs font-bold text-accent-text uppercase tracking-widest flex items-center gap-2">
                <div className="w-1 h-3 bg-indigo-500 rounded-full text-on-accent" />
                मिशन के लिए आवश्यक संसाधन:
              </div>
              <div className="grid grid-cols-2 gap-4">
                {['बेहतर सर्वर', 'AI ट्रेनिंग', 'रिसर्च', 'डेवलपमेंट'].map(item => (
                  <div key={item} className="flex items-center gap-2 text-xs text-ink font-bold bg-raised p-3 rounded-xl border border-line">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse text-on-accent" />
                    {item}
                  </div>
                ))}
              </div>
            ...
            <p className="text-body text-sm leading-relaxed font-medium border-l-2 border-indigo-500 pl-4 py-1">
            ...
            <p className="text-center text-accent-text font-bold text-sm bg-indigo-500/10 py-3 rounded-2xl border border-indigo-500/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <motion.div
            ...
            className="bg-card border border-line rounded-[2.5rem] p-8 text-center flex flex-col items-center justify-center shadow-2xl relative group"
          >
            <div className="absolute top-4 right-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <QrCode className="w-12 h-12 text-ink" />
            </div>
            <h4 className="text-sm font-black text-ink uppercase tracking-widest mb-6">Scan to contribute</h4>
            <div className="relative group/edit">
              <div className="w-48 h-48 bg-white p-3 rounded-2xl shadow-inner relative flex items-center justify-center">
                ...
                />
                {!isDonationEditing && (
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity bg-card rounded-2xl pointer-events-none text-center p-4">
                    <span className="text-[10px] font-bold text-black uppercase tracking-tight">{donationData.upiId}</span>
                  </div>
                )}
                {isDonationEditing && (
                  <label className="absolute inset-0 flex flex-col items-center justify-center bg-well backdrop-blur-[2px] text-ink opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl cursor-pointer">
              ...
              {isDonationEditing && donationData.qrUrl && (
                <button onClick={() => onDonationDataChange({ ...donationData, qrUrl: '' })} className="mt-2 text-[8px] font-bold text-danger uppercase flex items-center gap-1 mx-auto">
                  Reset to Auto-Generated QR
                </button>
              )}
            </div>
            <p className="text-[10px] text-muted mt-4 font-medium italic">Supports all UPI apps (GPay, PhonePe, Paytm)</p>
          </motion.div>

          {/* UPI & CTA Section */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col gap-6">
            <div className="bg-card border border-line rounded-[2.5rem] p-8 shadow-2xl relative group/edit">
              ...
              <div className="relative group">
                <input
                  ...
                  className={`w-full border rounded-2xl px-5 py-4 text-sm font-mono text-ink outline-none transition-all shadow-inner ${
                    isDonationEditing ? 'bg-raised border-indigo-500/50' : 'bg-surface border-line group-hover:border-indigo-500/50'
                  }`}
                />
                {!isDonationEditing && (
                  <button
                    onClick={() => { navigator.clipboard.writeText(donationData.upiId); onCopySuccess(); }}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 bg-indigo-600 hover:bg-indigo-700 text-on-accent rounded-xl transition-all shadow-lg active:scale-95 flex items-center gap-2 group/btn"
                  >
              ...
              <p className="text-[9px] text-faint mt-3 font-medium text-center">Verify the name displayed: <span className="text-ink">{donationData.name}</span></p>
            </div>

            <div className="bg-indigo-600 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group text-on-accent">
              <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent pointer-events-none text-on-accent" />
              <div className="relative z-10">
                <h4 className="text-xl font-black text-on-accent mb-2 flex items-center gap-3">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 text-center border-t border-line">
          <p className="text-xs text-faint italic font-medium max-w-xl mx-auto leading-relaxed">
