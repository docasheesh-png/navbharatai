...
• SELF-EVALUATION: agents can "evaluate" the project for real structural defects (unresolved imports that would break the build, import cycles, front-end→back-end layering violations, server-only Node builtins (fs, child_process, …) imported by front-end code that break the browser build, forEach(async …) loops that silently do not await, new Promise(async …) executors whose errors are swallowed, filter/find/some/every/sort(async …) predicates that are always-wrong because the method uses the return synchronously), security issues (hardcoded secrets/keys, hardcoded JWT signing secrets, the JWT "none" algorithm that accepts unsigned/forgeable tokens (auth bypass), credentials embedded in DB/queue connection strings, eval, new Function() dynamic code, setTimeout/setInterval with a string argument (eval-equivalent), command injection via a shell exec built from dynamic input, path traversal from a file read/response built on request input (../../etc/passwd), SQL injection from queries built by interpolating/concatenating input instead of using parameterised placeholders, NoSQL injection from passing a raw req.body/req.query straight into a database query ($gt/$ne operator abuse → auth bypass), open redirects (res.redirect to request input), hardcoded Authorization Bearer/Basic credentials, distinctive provider tokens hardcoded in source (GitHub classic + fine-grained PATs/Google/Slack/Stripe/Anthropic keys), dangerouslySetInnerHTML, Vue v-html raw-HTML binding, Angular bypassSecurityTrust* (disables sanitisation), raw innerHTML/outerHTML/insertAdjacentHTML (assign or +=) XSS sinks, document.write() (XSS sink + page-overwrite), javascript: URLs in href/src (script-execution XSS sink), postMessage to a wildcard '*' target origin (data leak), target="_blank" links without rel="noopener" (reverse tabnabbing), the insecure legacy crypto.createCipher/createDecipher (no IV) instead of createCipheriv, the non-cryptographic crypto.pseudoRandomBytes for tokens/keys, insecure http, insecure ws:// websockets) AND an authenticity check that detects fake/incomplete/placeholder code (TODO/FIXME/HACK markers, "not implemented" throws, stub/dummy/mock data, lorem ipsum, placeholder-image services (via.placeholder.com etc.) left in markup, placeholder @example.com contact emails, empty console.log-only handlers, leftover debug prints (console.log of a bare number or throwaway sentinel like "here"/"test"), empty catch blocks and empty promise .catch(() => {}) handlers that silently swallow errors, @ts-nocheck that disables type-checking for a whole file, blind @ts-ignore that hides a real type error behind a green build, and a bare eslint-disable that turns off every lint rule) — enforcing the "real features only, no fakes" rule — AND a dependency-consistency check (packages imported in code but missing from package.json, which would break the build at install/runtime; declared-but-unused dependencies; plus floating/unpinned versions like "*"/"latest" that make builds non-reproducible) AND an environment-variable completeness check (variables read in code via process.env / import.meta.env but missing from .env.example, which would break the app at runtime for the user, who is never told to set them) AND an accessibility check (Layer 78 "Sabke-Liye"/Inclusion: images with no alt text (including <input type="image"> graphical buttons), iframes with no accessible title, icon-only links with no accessible name, media that auto-plays sound (autoplay audio / unmuted autoplay video), a viewport meta that disables pinch-zoom (user-scalable=no / maximum-scale=1, locking out low-vision users), a <meta http-equiv="refresh"> timed auto-refresh/redirect that disorients users (WCAG 2.2.1/3.2.5), interactive controls hidden from screen readers with aria-hidden="true" (focusable but unannounced) or stripped of their semantics with role="none"/"presentation", form controls with no accessible name, click handlers on non-interactive elements that keyboard and screen-reader users cannot reach, positive tabindex that breaks focus order, duplicate static ids that break label/aria references, and pages with no document language) — so the apps it builds are usable by everyone — AND a trust/safety/compliance check (Layer 77 "Bharosa", DPDP/GDPR-oriented: personal data written to logs, secrets/tokens carried in a URL query string (leak into server logs, browser history and the Referer header), sensitive values kept in browser storage, cookies set without SameSite, server cookies set without httpOnly (JS-readable → XSS token theft), server cookies set without the Secure flag (sent in the clear over http), personal data sent over plain http, third-party trackers running with no cookie-consent surface, and collecting personal data with no privacy policy) that ends with an honest "launch-safe" certificate (CERTIFIED / CONDITIONAL / NOT CERTIFIED) — so the apps it builds are safe to launch publicly — AND a calibrated "build confidence" score (Layer 74 "Sahyog": 0–100% with a High/Medium/Low band and a plain-language "here's why", synthesized from all the checks above) so the assistant tells you honestly how confident it is rather than over-promising — and fix them before claiming the app is done.
...
• SEO/METADATA check (Section I #19): v3.0 checks the app's HTML entry for the discoverability essentials — a page title (and flags a leftover template-default title like "Vite + React"/"Document"), viewport (mobile — and flags a viewport pinned to a fixed pixel width instead of device-width), meta charset (correct text encoding), meta description, html lang, Open Graph tags (rich link previews on WhatsApp/social), and a favicon link — and flags any that are missing, plus a leftover robots "noindex" that would keep the live site out of search results, so your app is search-friendly and shareable, not invisible.
...
• SECURITY CONFIG check (Section I #4 — Security): v3.0 scans for insecure configuration — disabled TLS certificate verification (man-in-the-middle risk), wildcard "*" CORS (any site can call your API), CORS that reflects any origin while allowing credentials (any site can make authenticated requests with the user's cookies), and Math.random() used to make tokens/secrets (predictable, guessable) — and flags them so your app isn't shipped with an open security hole.
...
• PREVIEW on mitrify.xyz: the live preview of a v3.0-built app is served under the platform's own hosting domain (a mitrify.xyz address) instead of a raw sandbox URL. Until you connect your own website, mitrify.xyz handles both preview and hosting; once you link your own domain (GitHub or DNS), your domain serves the app and mitrify.xyz is used only for preview.
...
• BUILD PERSISTENCE — survives a refresh (D7): your v3.0 build is saved to the server AS IT RUNS, so if you refresh the page or lose your connection, reopening v3.0 automatically brings back that build's chat history (and the generated files come back via the sandbox's git repo). A build that was still mid-way is offered for "Resume". You never lose a build just because the tab reloaded.
...
• HONEST billing (shown in ₹): in normal mode you are charged the Claude Sonnet-equivalent token cost × 3.5; in Power mode you are charged the real Claude Opus token cost × 2.5. The USD cost is converted to Indian Rupees at the real-time exchange rate, and a live cost estimate is shown.`,
    howToUse: 'Open it from the floating "v3.0" button (bottom-right) or from the menu (Pro v3.0) — when opened from the menu it appears as a "NavBharatAI Pro v3.0" tab in the top header, alongside your other open tabs. Type what you want to build, and press Send. To analyze or build from a file, click the paperclip next to the message box and attach images, PDFs, Word/Excel/PowerPoint, ZIP, or text/code files (or paste a screenshot) — then ask your question. Open the build-options menu (the sliders icon on the left of the message box) to toggle Planning, Thinking, or Power (the "Only Opus" max-capability mode, billed at the real Claude Opus cost × 2.5 — also makes Claude read attached images). The live surfaces — Preview / Files / Diff / Terminal / History — are tab pills in the header: tap one to open that workspace beside the chat (it takes over the screen on mobile), and tap it again (or the ✕) to collapse back to full-width chat. Press Stop to cancel. If a build is already running and you reopen v3.0, the top-right corner shows two buttons — "Resume" (jump back into the live running build and watch it continue) and "Stop" (end the running build so you can start a fresh one); when nothing is running it shows "New" to start a fresh project.',
    ...
    keywords: [
      ...
      'persistence', 'survives refresh', 'refresh', 'reload', 'page refresh', 'lost build', 'build saved', 'reconnect', 'resume build', 'build gayab', 'refresh ke baad', 'build wapas',
