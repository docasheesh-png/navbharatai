...
export class GeminiProvider implements AIProvider {
  ...
  async execute(prompt: string, schema?: any, modelOverride?: string): Promise<AIProviderResponse> {
    ...
    try {
        const config = schema ? { responseMimeType: "application/json", responseSchema: schema } : undefined;
        const result = await ai.models.generateContent({ model: model, contents: prompt, config: config as any });
