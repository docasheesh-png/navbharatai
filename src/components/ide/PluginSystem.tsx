    ...
    <div className="h-full flex flex-col bg-surface text-ink overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-line bg-card">
        <div className="w-10 h-10 bg-violet-600/20 rounded-xl flex items-center justify-center">
          <Puzzle className="w-5 h-5 text-accent-text" />
        </div>
        <div>
          <h2 className="font-semibold text-ink text-base">Plugin System</h2>
          <p className="text-xs text-faint">{PLUGINS.length} integrations — add one to drop its real setup code into your app</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => setShowInstalledOnly(!showInstalledOnly)} className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all ${showInstalledOnly ? 'border-violet-500/40 bg-violet-500/10 text-accent-text' : 'border-line bg-raised text-faint'}`}>
      ...
      <div className="px-4 py-3 border-b border-line bg-card space-y-2">
        <div className="relative">
          ...
          <input className="w-full bg-surface border border-line rounded-xl pl-9 pr-4 py-2 text-sm text-ink placeholder-faint focus:outline-none focus:border-violet-500/50" placeholder="Search plugins..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
        <div className="flex gap-1.5 overflow-x-auto">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`shrink-0 text-[10px] px-2.5 py-1 rounded-full border transition-all whitespace-nowrap ${selectedCategory === cat ? 'border-violet-500/50 bg-violet-500/15 text-accent-text' : 'border-line bg-raised text-faint hover:border-line'}`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 gap-2">
              <Puzzle className="w-10 h-10 text-faint" />
              <p className="text-sm text-faint">{showInstalledOnly ? 'No plugins installed' : 'No plugins found'}</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              {filtered.map(plugin => {
                ...
                return (
                  <button key={plugin.id} onClick={() => setSelectedPlugin(plugin)} className={`text-left p-3.5 rounded-xl border transition-all ${selectedPlugin?.id === plugin.id ? 'border-violet-500/50 bg-violet-500/5' : 'border-line bg-card hover:border-line'}`}>
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2">
                        <span className="text-xl">{plugin.icon}</span>
                        <div>
                          <p className="text-xs font-semibold text-ink">{plugin.name}</p>
                          <p className="text-[9px] text-faint">by {plugin.author} • v{plugin.version}</p>
                        </div>
                      </div>
                      <div className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${isInstalled ? 'bg-emerald-500/20 border border-emerald-500/40' : 'bg-raised border border-line'}`}>
                        {isInstalled ? <Check className="w-3 h-3 text-success" /> : <Plus className="w-3 h-3 text-faint" />}
                      </div>
                    </div>
                    <p className="text-[10px] text-faint mb-2 line-clamp-2">{plugin.description}</p>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-md border ${plugin.free ? 'text-success bg-emerald-500/10 border-emerald-500/20' : 'text-warn bg-amber-500/10 border-amber-500/20'}`}>{plugin.free ? 'Free' : 'Paid'}</span>
                      <span className="text-[9px] text-faint ml-auto">{plugin.category}</span>
                    </div>
                  </button>
                );
              })}
        ...
        {/* Plugin Detail */}
        <div className="w-[40%] border-l border-line flex flex-col overflow-hidden">
          {!selectedPlugin ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 p-6 text-center">
              <Puzzle className="w-12 h-12 text-faint" />
              <p className="text-sm text-faint">Select a plugin to see details</p>
            </div>
          ) : (
            <>
              <div className="p-4 border-b border-line">
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-ink">{selectedPlugin.name}</h3>
                      <p className="text-[10px] text-faint">by {selectedPlugin.author} • v{selectedPlugin.version}</p>
                    </div>
                  </div>
                  <button onClick={() => setSelectedPlugin(null)} className="text-faint hover:text-muted"><X className="w-4 h-4" /></button>
                </div>
                <p className="text-xs text-muted mb-3">{selectedPlugin.description}</p>
                <div className="flex gap-1.5 flex-wrap mb-3">
                  <span className={`text-[9px] px-2 py-0.5 rounded-full border ${selectedPlugin.free ? 'text-success bg-emerald-500/10 border-emerald-500/20' : 'text-warn bg-amber-500/10 border-amber-500/20'}`}>{selectedPlugin.free ? 'Free' : 'Paid'}</span>
                  <span className="text-[9px] text-faint bg-raised px-2 py-0.5 rounded-full">{selectedPlugin.category}</span>
                  {selectedPlugin.tags.map(t => <span key={t} className="text-[9px] text-faint bg-raised px-1.5 py-0.5 rounded-md">{t}</span>)}
                </div>
                <button
                  onClick={() => addPlugin(selectedPlugin)}
                  className="w-full py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-all bg-violet-600 hover:bg-violet-500 text-on-accent"
                >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[10px] text-faint uppercase tracking-wider">Setup Code</p>
                    <button onClick={() => copyCode(selectedPlugin.setupCode)} className={`text-[9px] flex items-center gap-1 px-2 py-0.5 rounded-lg transition-all ${copied === selectedPlugin.setupCode.slice(0, 30) ? 'text-success bg-emerald-500/10' : 'text-faint bg-raised hover:text-muted'}`}>
                      {copied === selectedPlugin.setupCode.slice(0, 30) ? <><Check className="w-2.5 h-2.5" /> Copied</> : <><Check className="w-2.5 h-2.5" /> Copy</>}
                    </button>
                  </div>
                  <pre className="bg-surface border border-line rounded-xl p-3 text-[9px] font-mono text-success overflow-x-auto whitespace-pre-wrap">{selectedPlugin.setupCode}</pre>
                </div>

                {/* Config Keys */}
                {selectedPlugin.configKeys.length > 0 && (
                  <div>
                    <p className="text-[10px] text-faint uppercase tracking-wider mb-2">Required Config</p>
                    <div className="space-y-1.5">
                      {selectedPlugin.configKeys.map(k => (
                        <div key={k} className="flex items-center gap-2 bg-surface border border-line rounded-lg px-3 py-2">
                          <Shield className="w-3.5 h-3.5 text-warn shrink-0" />
                          <code className="text-xs text-warn font-mono flex-1">{k}=your_value</code>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {installed.includes(selectedPlugin.id) && (
                  <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-3">
                    <p className="text-xs text-success flex items-center gap-1.5 mb-1"><Check className="w-3.5 h-3.5" /> Added to your app</p>
                    <p className="text-[10px] text-faint">Its setup code was inserted into your project — now set this integration&apos;s keys in .env.</p>
                  </div>
                )}
              </div>
            </>
