...
export const VaultLockGate: React.FC<{
  ...
  const showPasswordField = askPassword
    && !!auth.currentUser
        ...
        <div className="mt-5 space-y-2.5">
          ...
          {hasDeviceLock === true && canUseDevice !== false && (
            <button
              ...
              disabled={!!busy}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold text-white hover:bg-indigo-500 disabled:opacity-50"
            >
              <Fingerprint size={16} />
          ...
          <button
            onClick={() => (askPassword ? void unlockViaAccount() : setAskPassword(true))}
            disabled={!!busy}
          ...
          >
            <KeyRound size={16} />
            {busy === 'account' ? 'Checking…' : askPassword ? 'Confirm and unlock' : 'Use my account password'}
          </button>
          ...
          {canUseDevice === true && hasDeviceLock !== true && (
            <>
              {!showPasswordField && askPassword === false && null}
              <button
                onClick={() => { setAskPassword(true); void setUpDeviceLock(); }}
                disabled={!!busy}
                className="w-full rounded-xl border border-indigo-500/30 px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-indigo-300 hover:bg-indigo-500/10 disabled:opacity-50"
              >
                {busy === 'register' ? 'Setting up…' : `Set up ${deviceLabel().toLowerCase()} lock`}
              </button>
              <p className="text-[10px] leading-snug text-gray-500">
                Uses this device's own face, fingerprint or PIN. Confirm your password once to set it up.
              </p>
            </>
          )}
