    ...
    <div className="flex flex-col h-full bg-surface text-body">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-line shrink-0">
        <span className="text-[10px] font-black uppercase tracking-widest text-muted flex items-center gap-1.5">
          ...
          Problems
          {problems.length > 0 && (
            <span className="ml-1 px-1.5 py-0.5 rounded bg-amber-500/15 text-warn text-[9px] font-black">{problems.length}</span>
          )}
        </span>
        {onClose && (
          <button onClick={onClose} className="p-1 text-muted hover:text-ink hover:bg-raised rounded transition-colors" aria-label="Close panel">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        {problems.length === 0 ? (
          <div className="h-full flex items-center justify-center gap-2 text-[11px] text-muted p-4">
            ...
            {problems.map((p, i) => (
              <li key={`${p.file}:${p.line}:${p.column}:${i}`}>
                <button
                  ...
                  className="w-full text-left flex items-start gap-2 px-3 py-1.5 hover:bg-raised transition-colors group"
                  title={p.file ? `Open ${p.file}:${p.line}` : undefined}
                >
                  <AlertCircle className="w-3.5 h-3.5 text-danger shrink-0 mt-0.5" />
                  <span className="flex-1 min-w-0">
                    <span className="block text-[11px] text-body whitespace-pre-wrap break-words">{p.message}</span>
                    {p.file && (
                      <span className="block text-[9.5px] text-muted font-mono mt-0.5 group-hover:text-accent-text transition-colors">
