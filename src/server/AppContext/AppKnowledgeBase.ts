...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    howToUse: 'Build your app in NavBharatAI Pro v5.0, then open Publish (the rocket) → find the green "Put it on App Mart" card next to "Make an Android app" → type the store name → tap "Publish to the store". The share link is copied automatically — send it to anyone; it opens the app full-screen in their browser with nothing to install. Manage it later under Home → App Mart → My apps (copy link, make private with a password, unpublish). A share link looks like navbharatai.com/store/app/<id> and needs no login to run a public app.',
