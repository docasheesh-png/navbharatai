...
- **Next un-done item (P1):** native tool-use loop — `ClaudeClient` (Anthropic SDK streaming + `tools`) + `AgentRunner` + `ToolCatalog`/`ToolDispatcher` on a real E2B sandbox. BLOCKED on admin answers to design §11 (sandbox/budget/Opus/persistence/allowlist).
