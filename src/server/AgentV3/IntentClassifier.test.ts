...
describe('classifyIntent — comparison/explanation asks are chat, not a build (real report: "v5.0 aur claude code ko campair karo")', () => {
  it('routes the exact reported message to chat, not a ~9-minute build', () => {
    // Real report: v5.0 spent a full build cycle ("Estimated build time: ~9 min", "Setting up your
    ...
    expect(classifyIntent('v5.0 aur claude code ko campair karo')).toBe('chat');
  });
  it('routes English "compare X and Y" / "X vs Y" phrasings to chat', () => {
    expect(classifyIntent('compare v5.0 and Claude Code')).toBe('chat');
  ...
  it('marks it HIGH-confidence so the LLM upgrade cannot be talked into a build', async () => {
    expect(classifyIntentWithConfidence('v5.0 aur claude code ko campair karo')).toEqual({ intent: 'chat', confidence: 'high', signal: 'informational' });
    const llm = async () => 'build';
    expect(await classifyIntentSmart('v5.0 aur claude code ko campair karo', llm)).toBe('chat');
  });
