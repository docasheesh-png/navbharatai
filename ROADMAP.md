...
| A5 | ✅ **ALREADY BUILT — verified against live code 2026-08-21. This row was a FALSE-OPEN.** | minor 1 | `AgentV3Panel.tsx` `handleEdit` (~923) → `MessageActions` `onEdit` (~3640) | It ships as **Edit** on the last user message: it takes the message back with the SAME full server purge Unsend uses (`unsend()` — transcript *and* workspace memory), drops the text into the composer to re-write, and refuses while a turn is in flight (`unsending` guard). It only prefills on a GENUINE purge, so a failed take-back can never leave the message in both the thread and the composer. ⚠️ **Do not "add" this by copying ProfessionalChat's rewind.** That surface rewinds a transcript; v5 must also purge SERVER state, or the AI still remembers the old wording while the screen says otherwise — a rewind that only looks like one. Offered on the LAST user message alone for exactly that reason: it is the only one whose reply has not yet been flushed, so removing it cannot strand an orphaned answer. |
...
| A7 | ✅ **ALREADY DELIVERED by the ADVISE mode — verified 2026-08-21. This row was a FALSE-OPEN.** | minor 10 | mode selector at the message box: `🔨 Build · 🧠 Plan · 🔍 Advise` | The row asked for a *toggle*; the capability shipped as a *MODE*, which is why a name-based check missed it. **Advise is structurally incapable of building** — the route handles `chatRole` BEFORE the build lock, with no tools at all, on the free router (`routes/agentv3.ts` ~6153, `RoleChats.ts`). So a question asked in Advise cannot spend a build, which is precisely what this row wanted. It is also already documented in `AppKnowledgeBase.ts` (id `agentv3_roles`), so every AI in the app can point a user at it. ⛔ **Do NOT add a second "answer only" toggle** — two controls for one behaviour is worse than one, and the second would have to duplicate the no-tools guarantee instead of inheriting it. The genuine residual is DISCOVERABILITY (a user must know to switch modes), and the honest fix for that is wording/onboarding, not another switch. |
...
> ⚠️ **A5 and A7 were BOTH false-opens, found 2026-08-21 by reading the code before writing any.**
> That is the sixth and seventh time this file has told a session to build something that already
> ships. Both misses have the same shape, and it is not the one the "How to use this file" section
> warns about: neither was hiding in a file with an unexpected NAME. **They were hiding behind
> unexpected UI SHAPES.** A5 exists as a button called *Edit* (the row expected a "rewind"); A7 exists
> as a chat MODE called *Advise* (the row expected a "toggle"). Grepping the domain noun would have
> found neither, because the row's noun was never the product's word.
> **So the check that works for a UX row is: describe the BEHAVIOUR the user gets, then find what
> already gives it to them** — "can a user ask a question without starting a build?" finds Advise
> instantly; "is there an answer-don't-build toggle?" finds nothing. Only A6 is genuinely open.
...
| B3 | ✅ **RESTART DONE 2026-08-21** — start/stop of individual non-dev services still open | minor 25 | The reboot always existed; it was only reachable from the **"No live preview yet" EMPTY state**, so a user whose preview URL still resolved while the server behind it had died (blank page, connection refused, stuck mid-boot) could not get to it — exactly the "needs a full rebuild" this row named. It is now in the preview **toolbar** while the preview is showing, running the SAME model-free `preview-diagnose` (hydrate → install → pre-kill → boot → verify the port), so there is no new server surface, rate limit or ownership path. Deliberately a SEPARATE button from the ↻ beside it: reload only re-points the iframe, restart is a real 30–90s sandbox reboot, and one button doing both is how a user learns to distrust both. `previewRestart.ts` holds the offer rule + the wording, with the honesty rule test-locked — a failed restart NEVER reads as a fixed one, and a failure with no reason still reports failure. **Still open:** starting/stopping an INDIVIDUAL non-dev service (a backend on its own port) from the App Logs strip. |
| B4 | ✅ **DONE 2026-08-21** — re-run only the failing test | minor 28 | `run_tests` now takes an optional `filter`. `testRunner.ts` `withTestFilter` maps it to each runner's OWN flag (`-t` vitest/jest, `-g` playwright, `-k` pytest, `-run` go, `-Dtest=` maven, `--tests` gradle). 🔒 The filter is MODEL-written text pasted into a sandbox shell command, so it goes through the single server-wide `lib/shellQuote.ts` — `'; rm -rf /` becomes an ordinary argument. 🔒 An npm `"test"` script is left ALONE with an honest note: the script could be any runner, and a silently-dropped filter would let a green FULL-suite run be read as proof that one fix worked. A filtered run is labelled **PARTIAL** in the result AND in project memory, so a one-test pass can never be recorded as a suite pass. |
...
| B6 | ✅ **DONE 2026-08-27** — History → "Compare two versions": pick two checkpoints, per-file added/removed with renames, via `git diff --numstat -M` in the ALREADY-WARM sandbox (never boots one — same cost rule as the per-version preview). Cold sandbox / missing history / invalid sha each get their own honest sentence (`checkpointDiff.ts`, 9 tests + pure selection helpers `checkpointCompare.ts`). NUMSTAT not the patch, deliberately — the question is "which files, how much"; a per-file patch view can build on it later. | minor 14 | |
...
| D3 | Open a PR, read CI, reply to review comments | minor 33, 34, 35 | 🟡 **TWO THIRDS WERE ALREADY BUILT** (found 2026-08-21 by the pre-build existence check, not by reading this row — which said only "needs the GitHub App token path"). `GitHubPrFlow.mergeViaPullRequest` has opened the PR, read the combined CI verdict (commit statuses AND check-runs) and merged **only on green** since the git-native storage phase. The missing third was minor 35 — nothing ever read what a HUMAN reviewer wrote. **Built + tested 2026-08-21:** `GitHubAppClient.listReviewComments` (BOTH comment APIs — inline review comments and the conversation below, because most non-developers review in the latter) and `replyToReviewComment` (threaded, falling back to the conversation), plus the pure `prReviewFeedback.ts` triage that decides what is actually a change request. The triage is conservative on purpose: our own comments, bots, resolved threads, OUTDATED comments (their line no longer exists), non-collaborators, and pure praise are all SKIPPED and reported to the user, because acting on a comment that was not a request means rewriting working code. The reviewer's words are FENCED and attributed in the prompt, never presented as a system directive. ⚠️ **STILL OPEN — the TRIGGER.** A reviewer comments hours after the build ends, when nothing is running, so this needs either a user action ("check my pull request") or a webhook. Until that exists these are capable, tested building blocks and **D3 is NOT done** — recorded here rather than quietly marked complete, which is the exact false-open this file's header warns about. |
...
|---|---|---|---|
| E1 | Share a build with a read-only link | minor 42 | ⚠️ **SUPERSEDED BY AN ADMIN DECISION — do not build from this row.** It WAS built (`ShareStore.ts`, `routes/share.ts`) and then the admin explicitly shut the share-link surface on 2026-08-25 ("band karo! share link hi hata do!!") after it turned out a forwarded link resumed a billed sandbox for someone else's audience — an unbounded cost hole. The published-app link (App Mart instant apps) is the sanctioned way to show someone your app. Re-opening shared LIVE previews needs the admin, and a cost design, first. (Found 2026-08-27 during the competitor comparison — this row still said "Build it".) |
...
| # | Item | Notes |
|---|---|---|
| 9.1a | ✅ **DONE 2026-08-27** (with 9.1b/9.1c, as `CODE_LITERACY_RULE` in `systemPrompt.ts`) | Carried by every prompt that explains code: architect, plan, Advise/role chats, the plain-chat fallback, and Free chat's own rule. Test-locked in `tests/codeLiteracyRule.test.ts`. |
| 9.1b | ✅ **DONE 2026-08-27** — inside `CODE_LITERACY_RULE`: "ERRORS ARE EXPLAINED, NEVER JUST SHOWN" — every error is followed by what it means + what to change, in the user's language. | |
| 9.1c | ✅ **DONE 2026-08-27** — the register is IN the rule and test-locked: technical nouns stay English ("is function me loop galat chal raha hai"), never Devanagari translations of variable/function. The product risk this row named is exactly what the test pins. |
...
|---|---|---|---|---|
| 2.1 ◑ | **Slice 2's last mile — the wallet debit** — BUILT 2026-09-12: `hosting-daily-bill` (04:00 UTC, exclusive) measures the last COMPLETE UTC day for every hosted app, prices it at cost + 20%, and debits the ONE wallet as `NavBharatAI hosting`. Idempotent by a Firestore `create` on `<app>_<day>` written BEFORE the money moves. `NAVBHARAT_BILL_HOSTING` is unset, so today it measures and reports and charges ₹0. D3 decided: no separate hosting allowance — the wallet IS the allowance. Still open: the six per-unit rates are unset (every line bills ₹0 and is named), and nothing pauses an app whose owner runs out of credit — deliberately the admin's call. | S | `hostingCost` + `hostingUsage` exist and feed an admin report. Add the daily job: measure → price → `computeRolledUpDebit` (one row per app per day, label `NavBharatAI hosting` — never a vendor name) → debit. Unmeasured lines bill zero AND are named, exactly as the modules already do. D3 decides the free allowance. | A hosted test app shows a daily hosting row in the wallet that matches `wouldBill` to the paisa. |
...
| 2.4 ✅ | **Slice 5 — ONE Publish button** — SERVER HALF DONE 2026-09-12: an app that needs a server is no longer refused; when NavBharat Cloud can run it, Publish hosts it and returns the live URL, and the app is now RECORDED so it appears in "Your published apps" and "Take offline" has a row to act on (it never did before). `choosePublishRoute` is the pure decision; static still wins whenever static works. Still admin-only (`NAVBHARAT_CLOUD_PUBLIC` unset until 2.1 meters it), and the bring-your-own paths (deploy to the user's own Vercel/Netlify/Cloudflare/GitHub Pages, and "I host it myself" through a PR into their repo) now sit behind an **Advanced disclosure** — 2026-09-13, `advancedPublish.ts` — collapsed for a new user and **opened automatically for anyone already on one of those paths**, since hiding a control somebody relies on reads as deleting it. The Render backend offer was already conditional on a publish refusal, so it needed no move. Remaining: nothing in the UI; the flag `NAVBHARAT_CLOUD_PUBLIC` is what still gates this to admins. | M | Publish stops refusing a fullstack app; `planDeployment` picks static / Cloud Run; GitHub + BYO Render move under "Advanced". Deletes the Blueprint walk the admin did on 2026-09-10. | A fullstack app reaches a live URL with zero steps outside NavBharatAI. |
...
| # | Item | Size | Notes | Done when |
|---|---|---|---|---|
| 3.1 ✅ | **AI gateway for user apps** — BUILT 2026-09-12, behind `APP_AI_GATEWAY` (unset ⇒ today's behaviour). The published app calls `POST /api/app-ai/ask` with a public, app-scoped token stamped in at publish; the answer rides the ordinary Professional chain and the cost lands on the owner's existing wallet. `generate_ai` now DEFAULTS to the no-key path (`provider` omitted ⇒ `navbharat`); BYO OpenAI/Anthropic stays, by name. Per-app ₹20/day and per-visitor ₹2/day caps are the real defence — the token is an identifier, never an authorisation. Owner-facing cap override and an in-product usage screen are deliberately NOT built yet. | M | 80% exists: the ONE-WALLET LAW, `aiSpendZone`, provider routing, `redactProviders`. New: a per-app token minted at publish (never the user's session), a per-app daily cap, and the model choice made by tier — the user's app never learns a provider name (white-label law applies to the app's users too). Replaces "bring your own OpenAI key" as the default; BYO stays under Advanced. | A generated chatbot works on a fresh account with no key, and its cost appears as one wallet row. |
...
| 3.6 ✅ | **Duplicate app** — DONE, verified end to end 2026-09-12. `POST /api/agentv3/conversations/:id/duplicate` → `useAgentV3Build.duplicateConversation` → the history menu in `AgentV3Panel`. The copy carries the FILES and the CHAT under a new name (`copyName`: "X (copy)", "X (copy 2)" …) and deliberately carries NOTHING that points at a place in the world — no GitHub repo (two apps pushing to one repo would overwrite each other), no deployment, no domain, no site settings, no secrets. It starts unpublished and unconnected, the only honest state for something that has never been published. Decisions are pure in `duplicateApp.ts`. | S | — |
