...
• QUALITY DEFAULTS (U-2 — Production basics by default): v3.0 can give the app the launch basics automatically — SEO + OpenGraph meta tags (title, description, share preview), a mobile viewport, an html lang attribute, a web app manifest (installable), and robots.txt — adding only what is missing and never overwriting an existing manifest. So the apps it builds are search-friendly, shareable and installable out of the box, not missing the essentials.
• DEAD-CODE / UNWIRED FILE CHECK: v3.0 can find modules it built but never wired in — a component, hook or util that nothing imports (and isn't an entry, test, config or route). This catches the common "created it, forgot to use it" bug where the code compiles but the feature never shows up; v3.0 then imports it where it belongs or removes it, so nothing you asked for is silently orphaned.
• TOOLCHAIN CHECK (D11 — Version pinning for imported repos): when v3.0 imports or clones an existing project, it can report the toolchain the repo declares it needs — Node (.nvmrc / engines), Python (.python-version / pyproject), Java (pom.xml), Go (go.mod) — and flag any internal contradiction where two files pin different versions. That silent mismatch is a classic "works for the author, breaks here" build failure, and v3.0 surfaces it honestly instead of leaving you guessing.
• PACKAGE HEALTH CHECK (GA-3 — Scripts that actually run): v3.0 checks package.json for two common ship-blockers other tools miss — an npm script that calls a build tool the project never installed (like a lint script using ESLint with no ESLint dependency, which fails with "command not found"), and a package declared in both dependencies and devDependencies. So the scripts it ships actually run instead of breaking on the first command.
• LINT ENGINE (GA-12 — ESLint + Prettier): v3.0 can run the project's own ESLint and Prettier as part of verification — catching a class of real bugs a typecheck does not (unused variables, React hooks exhaustive-deps, no-undef, promise misuse) plus formatting drift. ESLint errors block "done"; Prettier lists files to reformat. Together with running the real tests and compiling every language, this makes "verified" mean genuinely clean, not just type-correct.
• API CONTRACT CHECK (GA-5 — Frontend↔backend wiring): for a full-stack app, v3.0 can cross-check the backend routes it defined against the fetch/axios calls the frontend makes, and flag any call that has NO matching route — the silent bug where the UI calls an endpoint that does not exist (it compiles, the preview loads, but the feature is broken at runtime). It detects Express/Fastify, FastAPI/Flask and Spring routes, lists unused routes too, and fixes mismatches before calling the app done.
• ARCHITECTURE ONBOARDING (A2 — Understand before editing): when v3.0 opens an app it did not just build — an imported repo or a large existing project — it can produce a quick orientation map from the real import graph: the entry points, the core (most-imported) modules, the structural areas by folder, the key dependencies, and a suggested reading order. So it understands how the app is put together before making changes, instead of editing blind.
• AST CODEMODS (C7 — Surgical multi-file edits): for repo-wide structural changes v3.0 uses exact AST codemods instead of risky whole-file rewrites — rename a symbol across every file, add a React prop and update every usage site, or move/rename a file and rewrite every import that points to it (relative or @/alias) in one atomic step. So a rename or file-move updates all call sites correctly instead of missing some and breaking the build.
• CODE-GRAPH QUERIES (A1 — Safe editing at scale): before changing a shared file, v3.0 can QUERY the project's real import graph instead of guessing — "who imports this file?", "what does it depend on?", the full set of files a change would ripple to (its change-impact / blast radius), and where a symbol is defined. This lets it update every call site and read the affected files first, so edits to a big/unfamiliar app are deliberate, not blind.
• CROSS-LANGUAGE TYPECHECK (B6 — Every language compiles): for a polyglot app, v3.0 doesn't just check the frontend TypeScript — it compiles EVERY language in the project and reports OK/FAIL per language: tsc for TS/TSX, Python compile for .py files, Maven compile for Java, and go build for Go. So "verified" means the Java, Python, and Go parts actually compile too, not just the UI — a type/compile error in any language is caught before the build is called done.
• RUNS THE PROJECT'S OWN TESTS (B4 — Earned Verification): beyond a typecheck, v3.0 can auto-detect and actually RUN your app's real test suite — Vitest/Jest/Playwright for JS/TS, pytest for Python, Maven/JUnit for Java, and go test for Go — then read honest passed/failed counts and the names of failing tests. It picks up the project's own "test" script or test config automatically (great for an imported repo that already ships tests), reports real PASS/FAIL rather than assuming, and fixes failures before claiming the build is verified — without ever faking a green result.
...
• INFRASTRUCTURE-AS-CODE (GA-15 — IaC engine): for deploying to a cluster or cloud, v3.0 can also generate real Kubernetes manifests (a non-root Deployment with health probes + resource limits, a Service, an HPA, and an Ingress), a values-parameterized Helm chart, and Terraform for Google Cloud Run — ready to "kubectl apply" / "helm install" / "terraform apply".
• VULNERABILITY SCAN (GA-13 — supply-chain security): v3.0 can scan the app's dependencies for known CVEs / GitHub advisories against the OSV.dev database and report each vulnerable package with its advisory IDs, so a real app ships without known-vulnerable packages. It is honest — if the advisory database is unreachable it says the scan could not run, never a fake "all clear".
• LICENSE CHECK (dependency copyleft compliance): v3.0 can check your app's dependency licenses and flag copyleft-compliance risk — strong-copyleft (GPL/AGPL) packages that could require you to release your own source, plus weak-copyleft (LGPL/MPL) ones for awareness — reading package-lock.json and classifying each SPDX license. It confirms "no copyleft risk" when the app is all-permissive. Advisory only (never blocks a build). Ask to "check my licenses / any GPL dependencies?" before shipping.
• DATABASE MIGRATIONS (GA-10 — migration runner): for a fullstack app with a database, v3.0 detects the migration tool the project uses (Prisma, Knex, Drizzle, TypeORM, Sequelize, Flyway, Alembic) and applies the schema by running the real migration command in the sandbox — reporting the true result, never a fake "migrated" — so the app does not boot against an empty database and crash.
• ENGINEERING MEMORY (GA-6 — ADR + migration history): v3.0 remembers a project's architecture across builds. On a successful build it captures the real detected stack (framework + database + styling + language) as a dated, numbered decision record written to docs/decisions/ADR-NNN.md, and reads prior decisions back into the next build so follow-up work stays consistent with the established stack (a no-change rebuild adds no duplicate record). It also keeps a migration-run history per project, so before re-running migrations the AI sees what schema was already applied, when, and whether it succeeded — instead of blindly re-running.
• BRING-YOUR-OWN DATABASE (connection wiring): v3.0 can wire the app to connect to your OWN database — Supabase, Neon, Firebase, or plain Postgres — generating a real client module + the .env.example keys + the dependency. You paste your credentials into .env (NavBharatAI never stores them). It wires the connection; creating the database itself is done in the provider's own console.
• PAYMENTS (Razorpay / Stripe): v3.0 can add a real payment checkout to your app — a server route that creates the order/session AND verifies the payment signature (the payment is never trusted from the client alone) plus a client checkout helper. Razorpay (India-first) and Stripe are supported. You paste your provider keys into .env; NavBharatAI never stores them.
• TRANSACTIONAL EMAIL (Resend / SendGrid): v3.0 can add a real server-side sendEmail() helper for signup confirmation, password reset, receipts and notifications. You paste your provider API key + a verified sender into .env; NavBharatAI never stores them.
• FILE UPLOADS / STORAGE (S3 / R2 / Supabase / Cloudinary): v3.0 can add real file uploads — a server route + a client uploadFile() helper that uploads directly to storage (presigned URL for S3-compatible providers, a signed upload for Cloudinary), so the file never proxies through your server and your secret never leaves it. You paste your keys into .env; NavBharatAI never stores them.
• REALTIME (Pusher / Ably): v3.0 can add live pub/sub for chat, notifications, presence and collaborative updates — a server publish() helper + a client subscribe() that returns an unsubscribe cleanup. You paste your keys into .env (the secret stays server-side); NavBharatAI never stores them.
• FULL-TEXT SEARCH (Algolia / Meilisearch): v3.0 can add real search to a content-heavy app (catalog, docs, marketplace) — a server indexer (indexRecords, uses the admin/write key) + a client search() (uses a search-only key that is safe in the browser). The admin key stays server-side; NavBharatAI never stores your keys — you paste them into .env.
• MOBILE APP EXPORT (Android / iOS via Capacitor): v3.0 can turn your generated web app into a native mobile project — it emits a capacitor.config.ts wrapper + a MOBILE_EXPORT.md runbook and lists the Capacitor dependencies/scripts to add, so your app can ship to the Play Store / App Store. HONEST LIMIT: it generates the wrapper config, not the final signed .apk/.aab/.ipa binary — building that needs the Android SDK / Xcode plus your own signing keystore on a build machine (the runbook gives the exact steps). Ask for a "mobile / Android / app-store version" of your app to use it.
• DESKTOP APP EXPORT (Windows .exe / macOS .dmg / Linux .AppImage via Electron): v3.0 can turn your generated web app into a desktop app — it emits electron/main.cjs + electron-builder.yml + a DESKTOP_EXPORT.md runbook and lists the Electron devDependencies/scripts + the package.json "main" entry to set. HONEST LIMIT: it generates the wrapper config, not the final signed installer — electron-builder must run on the matching OS (Windows for .exe, macOS for a signed .dmg); the runbook gives the steps (and the required relative-base-path fix so assets load under file://). Ask for a "desktop / .exe / installable version" of your app to use it.
• BROWSER EXTENSION EXPORT (Chrome / Edge / Firefox, Manifest V3): v3.0 can turn your generated web app into a browser extension — it emits a manifest.json (Manifest V3) that serves your built app as the extension popup + an EXTENSION_EXPORT.md runbook (build → load unpacked → package). HONEST LIMIT: it generates the manifest + instructions, not a published extension — publishing needs a Chrome Web Store / Firefox AMO developer account + review; the runbook gives the steps (and the relative-base-path fix so the popup isn't blank). Ask for a "browser / Chrome extension version" of your app to use it.
• DATABASE TYPES (schema → TypeScript): v3.0 can generate TypeScript interface types from your database schema (Prisma models / SQL CREATE TABLE) and write them to src/types/db.ts, so your frontend and backend share ONE typed shape of the database instead of hand-written types that drift out of sync. Prisma enums become string-union types. Ask to "generate types from my schema" after defining or changing it.
• SCHEMA GRAPH / BLAST RADIUS: v3.0 can show your database schema's relationship graph (Prisma models / SQL tables and how they reference each other) and the change-propagation blast radius — ask "what depends on the User model?" and it lists every model/table that references it, so you (and the AI) review those before renaming, dropping, or changing a key, instead of silently breaking dependents. Works for both Prisma schema and SQL migrations.
• INFRA OPTIMIZER (Docker / Kubernetes / Terraform): v3.0 can scan your infrastructure files for real security + reliability anti-patterns — a base image on :latest, a container running as root, a secret baked into an image layer, a K8s pod with no resource limits or running privileged, a public (allUsers) Cloud Run binding, an unpinned Terraform provider — and reports each with a concrete fix. Ask to "harden/optimize my Docker / Kubernetes / Terraform" or it can run before deploy.
• CI WORKFLOW REPAIR (GitHub Actions · GitLab CI · Jenkins): v3.0 can detect and fix a broken CI pipeline (.github/workflows/*.yml, .gitlab-ci.yml, or Jenkinsfile) that would FAIL when it runs — an "npm ci" step with no committed lockfile (auto-fixed to "npm install"), a setup-node cache keyed to the wrong package manager (repointed to the one your project actually uses), or an "npm run <script>" step for a script your package.json doesn't define (flagged for a manual fix). It only touches workflows that are actually broken. This also surfaces automatically in the build readiness check, so a broken CI file is caught before you push.
...
• POLYGLOT BACKENDS (AB-1 — Fullstack sandbox): beyond JavaScript/TypeScript and Python, v3.0 can now build and RUN real Java (Spring Boot, JDK 17 + Maven) and Go (1.23) backends. Ask for a "Spring Boot REST API" or a "Go web service" and v3.0 scaffolds a real, runnable app (a live web server bound to 0.0.0.0:$PORT so the preview works) and routes that build onto a dedicated fullstack cloud sandbox that has the JDK, Maven, Go, MongoDB and Redis pre-installed — so a Java/Go backend actually compiles and starts, not just gets written. JS/Python/frontend builds are unchanged (they keep using the fast default sandbox).
...
• MOBILE FOOTER (phone/tablet): while NavBharatAI Pro v3.0 is open on a phone or tablet, the app's bottom bar shows v3.0's OWN six buttons — History (the session-history list as a bottom sheet), Pro Chat (back to the chat), Preview (the live preview), Files (the files v3.0 built — tap a file for Open in Code Studio / Copy file / Copy path / Delete, and Delete is REAL: the file is removed from the durable workspace too, it never comes back on reload), Report (downloads the real build report JSON), and More (Framework picker, Diff, Terminal, Checkpoints, Report history, GitHub, Deploy, Live site, New chat). The v3.0 header on mobile is slim — just the title, framework icon, build stamp and the Stop/Resume button. On desktop everything stays in the header exactly as before. Also: once you open the Preview it now STAYS alive when you switch tabs or go back to chat — switching away no longer destroys the rendered preview.
...
• COST BREAKDOWN ("Why this cost?"): after a billed build, a "Why this cost?" toggle appears next to the ₹ amount in the result footer — expand it to see exactly how the charge was computed: your input/output token split, the tier that ran, the base (Sonnet-equivalent or real-Opus) cost, the markup multiplier applied, and the final USD → ₹. Free builds show no charge to explain.`,
  ...
  },
  {
    ...
    path: 'NavBharatAI Pro v3.0 → mode selector at the message box (bottom-left, next to the settings button) — a small "🔨 Build" dropdown that opens upward with Build · Plan · Advise (+ the Queue chip)',
