...
APP BUILDING — LEAD WITH NavBharatAI Pro, warmly (NavBharatAI builds real apps):
...
• OPEN your answer with NavBharatAI Pro as the FIRST and main recommendation — right after a warm one-line acknowledgement of their idea. Tell them, like a helpful friend, that they can just describe their app in any language and NavBharatAI Pro will plan, build, preview and ship it for them, and show you get THEIR specific idea (name their app's key features). Point them to it: open **NavBharatAI Pro** from the sidebar (right below NavBharatAI Free). This comes FIRST, always — never bury it below other options.
• Only AFTER NavBharatAI Pro is clearly presented may you add, briefly and secondary, any other general context they asked for (e.g. what features their app should have). Keep it short and at the END. Do NOT name or recommend rival app builders (Appy Pie, Andromo, Bubble, Wix, Glide, Bolt, Lovable, v0, Replit, WordPress, Framer, …) or "hire a developer/agency" as the way to build it — NavBharatAI Pro is the way; anything else stays minor and never sounds better than ours.
...
HARD LIMITS:
• Do NOT generate code or produce HTML/CSS/JS in this free chat (guide them to NavBharatAI Pro instead, per the rule above).
    ...
    try {
      if (req.body.stream === true) {
        ...
        addDoc(collection(getDb() as any, 'ai_usage_logs'), {
          ...
          // 🔴 A STALL IS NOT A FAILURE, AND FILING IT UNDER `failureReason` WOULD SAY IT WAS. Caught in
          // review of the change that introduced `'stalled'` (2026-09-17): the provider went quiet
          // mid-stream and we stopped waiting, so `ok` stays TRUE and the text the user was shown
          // stands. Any panel that counts a present `failureReason` as a failure — the obvious reading
          // of the name — would have filed a successfully-answered turn as broken. That is the exact
          // class `isAppFinding` and NEVER_ROOT_CAUSE exist for, arriving through a field name.
