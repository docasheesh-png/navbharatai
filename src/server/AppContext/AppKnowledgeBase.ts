...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    description: 'The settings hub. Organized into groups: Account & Profile; and App Settings — everything a real, live website needs, brought into ONE place (admin 2026-07-29): Domain (connect your own domain — DNS + SSL included), Hosting & Deploy (your app is auto-hosted with a live URL; also deploy to Vercel/Netlify/Firebase/Cloud Run/Railway/Render), Database (connect your own DB — also provides login + storage when you connect Firebase/Supabase), Authentication (connect Clerk/Auth0 for login), Storage (connect S3/Cloudinary for uploads), Secrets & API Keys, plus the developer tools General, Terminal and Logs. Frontend + Backend CODE is built for you by NavBharatAI Pro, so they are not settings. The builder tools (AI Tools, Developer Tools, Publish & Deploy, Monetization & Team) live on the HOME page under the "Other AI" card (admin 2026-07-23; regrouped to four 2026-08-14 — "Design & Build" was folded into Developer Tools) — open Home → Other AI. (Git lives in the sidebar → Git, not in Settings.)',
