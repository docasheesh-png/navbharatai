...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    description: 'A real-time room where a signed-in team edits the same code together over Firestore, with a mobile-friendly 3-tab layout (Code / AI / Team). Create a room to get a Room ID and share it. JOINING NEEDS OWNER APPROVAL: a joiner\'s request is "pending" until the room owner approves it — until then they cannot see the code, chat, or AI. The OWNER can approve/reject requests and KICK any member (removed instantly, rules-enforced). Features: a clean shared code editor (a new room starts EMPTY — it never dumps your app\'s preview), room CHAT, presence + LIVE CURSORS (each member\'s current line), LINE-ANCHORED COMMENTS, and a SHARED AI THREAD (any member asks NavBharatAI; the question + answer appear for everyone, billed to whoever asked). Code sync is last-write-wins with a short debounce (no CRDT — dependency-free by design).',
