...
async function importAnthropicRoutes() {
  const { registerAnthropicRoutes } = await import('../src/server/routes/anthropic');
  return registerAnthropicRoutes;
}

describe('Anthropic routes — /api/anthropic validation', () => {
  it('registers the anthropic proxy endpoint', async () => {
    const register = await importAnthropicRoutes();
    const routes = captureRoutes(register);
    expect(routes.has('POST /api/anthropic')).toBe(true);
  });

  it('returns 400 when userId or messages are missing', async () => {
    const register = await importAnthropicRoutes();
    const routes = captureRoutes(register);
    const handler = routes.get('POST /api/anthropic')!;
  ...
  it('returns 400 when only userId is provided (messages missing)', async () => {
    const register = await importAnthropicRoutes();
    const routes = captureRoutes(register);
    const handler = routes.get('POST /api/anthropic')!;
  ...
  it('returns 400 when only messages are provided (userId missing)', async () => {
    const register = await importAnthropicRoutes();
    const routes = captureRoutes(register);
    const handler = routes.get('POST /api/anthropic')!;
