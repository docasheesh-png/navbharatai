...
export function architectSystemPrompt(): string {
  return [
    ...
    '  EDIT/ADD files. Do NOT run `npm create vite` / `create-vite` / `npx create-*`: those',
    '  scaffolders need a newer Node than the sandbox and will FAIL. If you need a different',
    '  stack, write its config files directly with write_file (never a create-* generator).',
