...
• UNSEND (take back your last message): hover the LAST message you sent in a build chat and tap the small ✕ (Unsend) under it. This stops any build still running for that message AND permanently removes the message from the conversation — the visible thread, the durable transcript the AI replays, and the AI's project memory — so it never resurfaces or influences a future turn. It does NOT undo files already written (Unsend forgets the message, it doesn't roll back the app — the preview and file list keep reflecting reality); to undo file changes, use the Git/History checkpoints. Only the newest message is unsendable; older messages keep Copy. Use it when you sent the wrong thing, a typo, or something you'd rather the AI never remembered.
...
• DEPLOY CONFIG — Railway/Render/Fly (generate_deploy_config): v5.0 can add the platform config for a git-push PaaS — render.yaml (Render Blueprint), railway.json (Nixpacks + health check + restart policy), or fly.toml (http_service + health check). Each references /health + PORT. HONEST: it generates the config; the user deploys from their OWN Railway/Render/Fly account (BYO), it does not auto-deploy. One-click deploy already exists for Firebase/Vercel/Netlify/Cloudflare; AWS/Azure need full IaC (generate_iac). Pairs with generate_deploy_artifacts (Dockerfile). No keys.
...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    id: 'ai_debugger',
    name: 'AI Debugger',
    path: 'Settings → AI Tools → AI Debugger',
    description: 'Paste any error message (and optionally the related code) and get a REAL AI analysis from NavBharatAI\'s engine: the exact root cause, a working fix (with corrected code), step-by-step explanation, and prevention tips so the bug class never returns. Understands JS/TS, CSS, network/CORS, build, and React errors; replies mirror your language (Hinglish works). Honest by design: if the analysis service is briefly unavailable it says so plainly — it never shows a canned fake answer.',
    howToUse: 'Settings → AI Tools → AI Debugger. Pick the error category tab, paste the error text (plus code context if you have it), and tap Analyze — the root cause, fix, explanation, and prevention appear as cards. Recent analyses are kept in History on your device.',
    relatedFeatures: ['ide', 'settings_logs', 'pro_chat'],
    keywords: ['ai debugger', 'debugger', 'debug', 'error fix', 'fix error', 'error samjhao', 'error kya hai', 'bug fix', 'stack trace', 'exception', 'error analysis', 'galti dhundo'],
  },
  {
    id: 'ai_image_gen',
    name: 'AI Image Gen',
    path: 'Settings → AI Tools → AI Image Gen',
    description: 'Generate REAL images with NavBharatAI\'s own image engine — logos, banners, app icons, illustrations, avatars, backgrounds, thumbnails. Write a prompt (specific = better), pick a style (Minimal / Vibrant / Dark / Gradient / Flat / 3D), a size/aspect (Square, Wide/OG, Portrait, App Icon), and optional color hints, then Generate. The image is created on NavBharatAI\'s server and returned to you — copy it or download it as PNG; this session keeps your recent generations for quick reuse. Honest by design: if generation is briefly unavailable it tells you plainly — it never shows a placeholder pretending to be your image.',
    howToUse: 'Settings → AI Tools → AI Image Gen. Describe the image (e.g. "Modern fintech app logo with blue gradient and rupee symbol"), choose style + size, tap Generate. Use the copy/download buttons on the result; tap a Recent thumbnail to bring one back.',
  ...
  },
  {
    id: 'bot_builder',
    name: 'Bot Builder',
    path: 'Settings → AI Tools → Bot Builder',
    description: 'Design a chatbot conversation visually and then BUILD it for real. Drag nodes onto the canvas — Start, Message, Button Menu, Condition, API Call, End — and connect them into a flow; pick the WhatsApp / Telegram chat styling; test the exact flow in the built-in Simulator; export the flow as JSON if you want it elsewhere. Then tap "Build Bot App" — the designed flow is converted into a full build brief and handed to NavBharatAI Pro v5.0 (chat opens prefilled; press Send) which genuinely builds a working chatbot web app implementing your exact flow: quick-reply buttons, branching conditions, real API calls, typing indicator, restart control.',
    howToUse: 'Settings → AI Tools → Bot Builder. Add and connect nodes to design the conversation, edit each node\'s text/options in the side panel, and try it with Simulate. When it feels right, tap "Build Bot App" — Pro v5.0 opens with the full build brief; press Send to build the real chatbot app.',
  ...
  // ─── SETTINGS ─────────────────────────────────────────────────────────────
  {
    ...
    description: 'The settings hub. Organized into groups: App Settings (General, Secrets & Keys, Database, Terminal, Logs), AI Tools (Modules / Brain Engine), Developer Tools, Design & Build, Publish & Deploy, and Monetization & Team. (Git lives in the sidebar → Git, not in Settings.)',
