...
• PUBLISH / HOSTING CHOOSER (Hosting Phase 1): the "Publish" button (top of the v5.0 chat, or the mobile More sheet → Publish) opens a chooser with two ways to put your app online, both publishing the SAME built app: (1) "Host on NavBharatAI" — our own one-click hosting, Free, no account, permanent link (frontend/static now; full-stack hosting with a running backend + database is coming soon), and (2) "Host somewhere else" — publish to your own Vercel / Netlify / Cloudflare / GitHub Pages (your account, your bill, free from us). Only providers that are actually configured are offered, so a publish can never target a host that isn't set up. After publishing, a "Live site" link appears. When custom-domain hosting is enabled, "Host on NavBharatAI" also shows a "Connect your own domain" option: enter your domain (e.g. myshop.com), and NavBharatAI shows the exact DNS records to add at your registrar (Hostinger/GoDaddy/etc.) and issues free HTTPS automatically once they resolve — the domain then serves this app. Publish once after connecting so the domain serves your latest build.
