...
• PROJECT HYGIENE check (Section I #22 — Developer Experience): v3.0 checks your project has the basics — a .gitignore (so node_modules/.env/secrets don't get committed), a tsconfig.json for TypeScript, and a lockfile for reproducible installs — and flags what's missing.
...
• SECURITY CONFIG check (Section I #4 — Security): v3.0 scans for insecure configuration — disabled TLS certificate verification (man-in-the-middle risk) and wildcard "*" CORS (any site can call your API) — and flags them so your app isn't shipped with an open security hole.
