...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    howToUse: 'Home → Other AI → AI Debugger. Pick the error category tab, paste the error text (plus code context if you have it), and tap Analyze — the root cause, fix, explanation, and prevention appear as cards. Recent analyses are kept in History on your device.',
  ...
  },
  {
    ...
    howToUse: 'Home → Other AI → AI Image Gen. Describe the image (e.g. "Modern fintech app logo with blue gradient and rupee symbol"), choose style + size, tap Generate. Use the copy/download buttons on the result; tap a Recent thumbnail to bring one back.',
  ...
  },
  {
    ...
    howToUse: 'Home → Other AI → Bot Builder. Fully touch/mobile-friendly: tap a node type in the bottom palette bar to add it, drag nodes with your finger to arrange them, tap a node then use its floating toolbar (Edit / Connect / Move / Duplicate / Delete), and tap a connection line to delete it. Try the whole flow with Simulate. Then you have TWO ways to ship: (1) "Go Live" — publish the flow as a REAL Telegram bot in seconds (paste the token from @BotFather; it hosts on NavBharatAI and runs your exact flow with quick-reply buttons), or connect a WhatsApp Cloud API number; or (2) "Build Bot App" — hand the flow to Pro v5.0 to build a full chatbot WEB app you can deploy anywhere.',
  ...
  // ─── SETTINGS ─────────────────────────────────────────────────────────────
  {
    ...
    aiSurface: 'nbi_chat',
    description: 'The fourth card on the Home page (next to NavBharatAI Free, Pro and Professionals). Opening it reveals every AI-builder utility, grouped: AI Tools (Bot Builder, AI Image Gen, AI Debugger, Code Review), Developer Tools (Test Runner, API Tester, Diff Viewer, Versioning, Performance, Minifier), Design & Build (Multi-Page, Components, Design System, Dark Mode Gen, Figma Import), Publish & Deploy (APK Builder, CI/CD Pipeline, Multi-Cloud, Custom Domain, SEO Optimizer, App Store), and Monetization & Team (Monetize, Team, Live Collab, Whitelabel, Analytics, Insights & Webhooks, Database). These moved here from Settings (admin 2026-07-23) so all builder tools sit alongside the main AIs; each tile opens the same tool it always did.',
