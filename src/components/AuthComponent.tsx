...
/**
 * Turn a GitHub sign-in failure into an ACTIONABLE message. The failures that block GitHub login are
 * config, not code, so name the exact fix (which provider to enable, which domain to authorize) so it
 * can be resolved without a desktop console. Everything else falls back to the generic describer.
 */
function describeGithubError(err: any): string {
  const code = err?.code || '';
  const host = typeof window !== 'undefined' ? window.location.hostname : 'this domain';
  if (code === 'auth/operation-not-allowed') {
    return `GitHub sign-in is not enabled yet. An admin must enable the GitHub provider in Firebase Console → Authentication → Sign-in method (project ${firebaseConfig.projectId}) and paste the GitHub OAuth App's Client ID + Secret.`;
  }
  if (code === 'auth/unauthorized-domain') {
    return `GitHub sign-in is blocked for "${host}". An admin must add this exact domain in Firebase Console → Authentication → Settings → Authorized domains (project ${firebaseConfig.projectId}).`;
  }
  if (code === 'auth/account-exists-with-different-credential') {
    return 'This email is already registered with another sign-in method (e.g. Email/Password). Please sign in with that method first, then link GitHub from settings.';
  }
...
export const AuthComponent = ({ auth, setUser, onClose }: { auth: Auth, setUser: any, onClose: () => void }) => {
  ...
  // GitHub sign-in (real Firebase auth — creates/sings-in a normal NavBharatAI user, exactly like
  // email/phone). Popup first (instant, clear errors on desktop); if the popup is blocked or closed
  // (common on mobile), fall back to a full-page redirect — getRedirectResult() at the App root
  // (App.tsx) finalizes that return. Honest errors when the GitHub provider isn't enabled yet
  // (auth/operation-not-allowed) — never a fake success.
  const handleGithubSignIn = async () => {
    ...
    try {
      ...
      const code = err?.code || '';
      // Popup unavailable (blocked, or the user closed it / a second popup raced it) — on mobile
      // especially, redirect is the reliable path. Fall through to it instead of erroring.
      if (code === 'auth/popup-blocked' || code === 'auth/cancelled-popup-request' || code === 'auth/popup-closed-by-user' || code === 'auth/operation-not-supported-in-this-environment') {
        try {
          ...
          return; // the whole page navigates to GitHub and back; handled at the App root.
        } catch (rErr: any) {
          setError(describeGithubError(rErr));
        }
      } else {
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4 flex flex-col items-center gap-4">
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-4 bg-[#24292f] hover:bg-[#30363d] disabled:opacity-40 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-3 transition-all shadow-lg active:scale-95 border border-white/10"
            >
