...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    description: 'On/off toggle (default ON) that adds a small "made by NavBharatAI" badge to the bottom-right corner of every app you build. The badge links to navbharatai.com, so anyone you share your app with can discover NavBharatAI too. TURNING IT OFF IS A PAID FEATURE (2026-08-21) — for everyone else the switch is locked and the card says why, rather than moving and quietly doing nothing. TWO THINGS PEOPLE GET WRONG: (1) paying does NOT remove the badge on its own — someone entitled who never touches the switch still gets the badge on every app; the entitlement buys the RIGHT to turn it off, the switch still has to be turned off. (2) The switch only records a preference on your device — NavBharatAI\'s server decides on every single build whether to honour it, so nothing edited in the browser can remove the badge. HONEST STATE RIGHT NOW: the subscription that unlocks it is not on sale, so the badge stays on every app for the moment and there is nothing a user can buy to change that; the card says exactly that instead of offering an upgrade button that would fail.',
    howToUse: 'Open Settings → General and find "Made by NavBharatAI" Signature. If your account is entitled, toggle it off and your NEXT build is made without the badge. If it is locked, the note under it tells you why — removing the badge is a paid option, and it is not currently purchasable, so no action will remove the badge today. Never look for a workaround: the decision is made on the server on every build, so changing anything in the browser has no effect.',
    relatedFeatures: ['settings_general', 'settings_root'],
    keywords: ['signature', 'made by navbharatai', 'watermark', 'badge', 'branding', 'remove badge', 'built by', 'credit', 'hataye', 'signature hatao', 'badge hatao', 'watermark hatao', 'badge kaise hataye', 'signature kaise hataye', 'made by navbharatai kaise nikale', 'toggle off nahi ho raha', 'signature off nahi ho raha', 'paid feature', 'subscription', 'locked'],
  },
  {
    id: 'settings_modules',
