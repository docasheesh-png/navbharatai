...
• RESIZE THE SPLIT — the border between the chat and the workspace MOVES (admin 2026-08-17). On DESKTOP: drag the border left or right to give either side more room, or double-click it to snap back to an even 50/50. While dragging, a small number shows the preview's live width in pixels — drag it narrow and you are looking at your app at a phone width, which is the quickest way to check that it works on a small screen. On TABLET: tap the ◀ ▶ buttons ON the border to step it left or right (a tap is exact where dragging a thin line is not). Keeping ▶ going past the last step closes the workspace so the chat fills the screen. Arrow keys work too when the border is focused. Your chosen width is REMEMBERED for next time. ONE-TAP DEVICE WIDTHS: on the Preview tab's header there are Phone · Tablet · Full buttons and the preview's REAL width in pixels. Tap Phone and your app is genuinely laid out at 390px (a real iPhone width); Tap Tablet for 768px; Full gives the preview all the room the window has. Nothing is faked or emulated — it is your actual app in a real browser at that width, which is how you check that it works on a small screen without leaving the builder. If the window is too narrow to show a true tablet, that button is greyed out and says so rather than showing you something narrower and calling it a tablet, and the px number always shows the real width. On a PHONE there is no split at all — the chat and the workspace each take the full screen and you switch between them — so there is nothing to resize there.
