...
function renderReport(text: string): React.ReactNode {
  ...
  return lines.map((line, i) => {
    ...
    return (
      <div key={i} className={heading ? 'font-bold text-ink mt-3 mb-1' : 'leading-relaxed'}>
        {renderInline(clean)}
      </div>
    );
  });
...
function renderInline(s: string): React.ReactNode {
  ...
  return parts.map((p, i) => {
    ...
    if (/^`[^`]+`$/.test(p)) return <code key={i} className="px-1 py-0.5 rounded bg-raised text-accent-text text-[12px]">{p.slice(1, -1)}</code>;
...
export function RepoAnalystTool({ userId }: { userId?: string }) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-surface text-body">
      {/* Header */}
      <div className="px-5 py-4 border-b border-line shrink-0">
        <div className="flex items-center gap-2 mb-1">
          <div className="w-8 h-8 rounded-lg bg-indigo-500/10 text-accent-text flex items-center justify-center"><GitBranch className="w-4 h-4" /></div>
          <h1 className="font-black text-ink tracking-tight">GitHub Repo Analyst &amp; Improver</h1>
        </div>
        <p className="text-[12px] text-muted">Paste a public GitHub repo — it fetches the real repo (read-only) and analyses it: strengths, gaps, security/quality, license &amp; a prioritised improvement plan. It can’t modify repos you don’t own; “copy good things” means learning patterns license-respectfully.</p>
      </div>

      {/* URL input */}
      <div className="px-5 py-3 border-b border-line shrink-0 flex items-center gap-2">
        <div className="flex-1 flex items-center gap-2 bg-card border border-line rounded-xl px-3 py-2 focus-within:border-indigo-500/40">
        ...
        <button onClick={analyse} disabled={!repoUrl.trim() || loading} className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-on-accent text-sm font-semibold flex items-center gap-2 shrink-0">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-5 py-4">
        {!analysedRepo && !loading && (
          <div className="h-full flex flex-col items-center justify-center text-center text-faint gap-3">
            <GitBranch className="w-10 h-10 opacity-40" />
            <p className="text-sm max-w-md">Paste a public GitHub repository above and press <span className="text-muted font-semibold">Analyse</span>. Try a popular open-source project, or your own repo.</p>
          </div>
        )}

        {analysedRepo && (
          <div className="mb-3 text-[12px] text-muted">Analysing: <span className="text-accent-text font-mono">{analysedRepo}</span></div>
        )}

        <div className="space-y-4 max-w-3xl">
          {turns.filter((t) => t.role === 'assistant' || (hasReport && t.role === 'user' && turns.indexOf(t) > 1)).map((t, i) => (
            t.role === 'assistant' ? (
              <div key={i} className="bg-card border border-line rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap">
                {renderReport(t.content)}
              </div>
            ) : (
              <div key={i} className="flex justify-end"><div className="max-w-[85%] rounded-2xl px-4 py-2 text-sm bg-indigo-600 text-on-accent">{t.content}</div></div>
            )
          ))}
          {loading && (
            <div className="bg-card border border-line rounded-2xl px-4 py-3 flex items-center gap-2 text-sm text-muted">
              <TirangaLoader className="w-4 h-4 text-accent-text" /> Fetching &amp; analysing the repository…
            </div>
          )}
          ...
          {hasReport && (
            <div className="pt-1">
              <button
                ...
                className="text-[13px] px-3 py-2 rounded-xl bg-emerald-600/90 hover:bg-emerald-600 disabled:opacity-40 text-on-accent font-semibold flex items-center gap-2"
              >
                {genLoading ? <TirangaLoader className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
                Generate improvement files
              </button>
              <p className="text-[11px] text-faint mt-1">Original, ready-to-use files for YOUR fork (tests, README, CI, fixes…). Type a specific ask in the box below first if you want (e.g. “a CI workflow + a test”). License-respecting — apply them in your own copy.</p>
            </div>
          )}

          {genSummary && (
            <div className="bg-card border border-emerald-500/20 rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap text-body">{genSummary}</div>
          )}

          {genFiles.map((f, i) => (
            <div key={i} className="border border-line rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between gap-2 px-3 py-2 bg-card border-b border-line">
                <span className="font-mono text-[12px] text-success truncate">{f.path}</span>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => copyFile(f)} title="Copy" className="p-1.5 rounded-lg hover:bg-raised text-muted"><Copy className="w-3.5 h-3.5" /></button>
                  <button onClick={() => downloadFile(f)} title="Download" className="p-1.5 rounded-lg hover:bg-raised text-muted"><Download className="w-3.5 h-3.5" /></button>
                </div>
              </div>
              <pre className="px-3 py-2 text-[12px] text-body overflow-x-auto custom-scrollbar max-h-80 whitespace-pre"><code>{f.content}</code></pre>
            </div>
          ))}
      ...
      {hasReport && (
        <div className="p-3 border-t border-line flex items-end gap-2 shrink-0">
          ...
          />
          <button onClick={() => ask()} disabled={!followUp.trim() || loading} className="w-9 h-9 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-on-accent flex items-center justify-center shrink-0">
