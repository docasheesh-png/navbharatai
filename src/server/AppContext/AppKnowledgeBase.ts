...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    howToUse: 'Build your app in NavBharatAI Pro v5.0, then open Publish (the rocket) → find the green "Put it on App Mart" card next to "Make an Android app" → optionally add an APP ICON (Add app icon — shows as your app\'s logo on the store card instead of a globe) and up to 3 SCREENSHOTS (shown when someone opens your app\'s details on App Mart) → type the store name → tap "Publish to the store". Icon and screenshots are optional but make the listing look real. The share link is copied automatically — send it to anyone; it opens the app full-screen in their browser with nothing to install. On App Mart, tapping an app\'s icon/name opens its details (description + your screenshots) with an Open button; "Open" still runs it directly. Manage it later under Home → App Mart → My apps (copy link, make private with a password, unpublish). A share link looks like navbharatai.com/store/app/<id> and needs no login to run a public app.',
