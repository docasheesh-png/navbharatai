...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    howToUse: 'Home → Other AI → Bot Builder. Fully touch/mobile-friendly: tap a node type in the bottom palette bar to add it, drag nodes with your finger to arrange them, tap a node\'s round link handle then tap a target node to CONNECT them (tap a connection line to delete it), and edit each node\'s text/options in the panel (a bottom sheet on mobile, side panel on desktop). Try the whole flow with Simulate. When it feels right, tap "Build Bot App" — Pro v5.0 opens with the full build brief; press Send to build the real chatbot app.',
