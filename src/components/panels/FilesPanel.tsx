...
const EXT_COLOR: Record<string, string> = {
  ...
  ts: 'text-info', tsx: 'text-info', json: 'text-success',
  md: 'text-accent-text', py: 'text-success',
};
...
export function FilesPanel({
  ...
  return (
    <div className="relative flex-1 h-full overflow-hidden bg-surface flex flex-col">
      {/* Upload conflict popup */}
      {fileUploadConflict && (
        <div className="fixed inset-0 z-[9999] bg-scrim flex items-center justify-center p-4" onClick={() => onResolveConflict('merge')}>
          <div className="bg-card border border-line rounded-2xl p-6 max-w-sm w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            <p className="text-[13px] font-black text-ink mb-1">
            ...
            <div className="flex gap-3">
              <button onClick={() => onResolveConflict('replace')} className="flex-1 py-2.5 rounded-xl bg-red-600/20 border border-red-500/30 text-danger text-[11px] font-black hover:bg-red-600/30 transition-all active:scale-95">Replace</button>
              <button onClick={() => onResolveConflict('merge')} className="flex-1 py-2.5 rounded-xl bg-indigo-600/20 border border-indigo-500/30 text-accent-text text-[11px] font-black hover:bg-indigo-600/30 transition-all active:scale-95">Merge</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-line bg-card">
        <div className="flex items-center gap-1 bg-raised rounded-lg p-0.5">
          <button
            onClick={() => setTab('files')}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest transition-all ${tab === 'files' ? 'bg-indigo-600 text-on-accent' : 'text-muted hover:text-ink'}`}
          >
            <FolderOpen className="w-3 h-3" /> Files
          </button>
          <button
            onClick={() => setTab('history')}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest transition-all ${tab === 'history' ? 'bg-indigo-600 text-on-accent' : 'text-muted hover:text-ink'}`}
          >
        ...
        {tab === 'files' && (
          <div className="ml-auto flex items-center gap-2">
            {hasGeneratedCode && (
              <span className="text-[8px] text-success font-black uppercase tracking-widest mr-1">
            ...
            {onAddFile && (
              <button
                ...
                title="New file"
                className="flex items-center gap-1 px-2.5 py-1 bg-raised hover:bg-raised border border-line rounded-lg text-[9px] font-black uppercase tracking-wider text-muted hover:text-ink transition-all active:scale-95"
              >
            ...
            {hasGeneratedCode && (
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1 bg-indigo-600/20 hover:bg-indigo-600/35 border border-indigo-500/30 rounded-lg text-[9px] font-black uppercase tracking-wider text-accent-text hover:text-accent-text transition-all active:scale-95"
              ...
              {hasGeneratedCode && Object.keys(files).length > 5 && (
                <div className="px-3 py-2 border-b border-line">
                  <input
                    ...
                    className="w-full bg-raised border border-line rounded-lg px-2 py-1 text-[10px] text-ink placeholder-faint outline-none focus:border-indigo-500/40 transition-all"
                  />
                </div>
              )}
              {/* C3 — new file input */}
              {showNewFile && (
                <div className="px-3 py-2 border-b border-line flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-raised border border-indigo-500/30 rounded-lg px-2 py-1 text-[10px] text-ink placeholder-faint outline-none"
                  />
                  <button onClick={commitNewFile} className="p-1 text-success hover:text-success"><Check className="w-3 h-3" /></button>
                  <button onClick={() => { setShowNewFile(false); setNewFileName(''); }} className="p-1 text-faint hover:text-ink"><X className="w-3 h-3" /></button>
                </div>
              )}
              {!hasGeneratedCode ? (
                <div className="flex-1 flex items-center justify-center flex-col gap-3 text-center p-8">
                  <FolderOpen className="w-12 h-12 text-faint" />
                  <p className="text-[11px] text-faint font-medium">No app generated yet.</p>
                  <p className="text-[9px] text-faint">Build an app in NavBharatAI Pro — files will appear here.</p>
                </div>
              ) : (
              <div className="p-4 space-y-1">
                {Object.entries(files)
                  ...
                  .map(([path, content]) => {
                    ...
                    const color = EXT_COLOR[ext] || 'text-muted';
                    ...
                    return (
                      <div key={path} className="rounded-xl hover:bg-raised transition-colors">
                        ...
                        {isRenaming ? (
                          <div className="flex-1 flex items-center gap-1 px-2 py-1.5">
                            <input
                              ...
                              className="flex-1 bg-raised border border-indigo-500/30 rounded px-1.5 py-0.5 text-[10px] text-ink outline-none min-w-0"
                            />
                            <button onClick={commitRename} className="p-0.5 text-success hover:text-success shrink-0"><Check className="w-2.5 h-2.5" /></button>
                            <button onClick={() => setRenamingPath(null)} className="p-0.5 text-faint hover:text-ink shrink-0"><X className="w-2.5 h-2.5" /></button>
                          </div>
                        ) : (
                          <>
                            <button
                            ...
                            >
                              ...
                              <span className="text-[11px] font-medium text-body flex-1 truncate">{path}</span>
                              <span className="text-[8px] text-faint font-mono shrink-0">{lines}L · {sizeLabel}</span>
                            </button>
                            ...
                            <div className="flex items-center gap-0.5 pr-2 opacity-60 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0">
                              ...
                              <button
                                ...
                                className="p-1 text-faint hover:text-success rounded transition-colors"
                              >
                              ...
                              {onDeleteFile && (
                                <button
                                  ...
                                  title="Delete"
                                  className="p-1 text-faint hover:text-danger rounded transition-colors"
                                >
                      ...
                      {tapActions && actionPath === path && !isRenaming && (
                        <div className="flex flex-wrap items-center gap-1.5 px-3 pb-2.5">
                          <button
                            ...
                            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-600/20 border border-emerald-500/40 text-[10px] font-semibold text-success touch-manipulation"
                          >
                            <Eye className="w-3 h-3" /> See
                          </button>
                          <button
                            onClick={() => { setActionPath(null); onOpenFile(path); }}
                            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-indigo-600/20 border border-indigo-500/40 text-[10px] font-semibold text-accent-text touch-manipulation"
                          >
                            <FileCode className="w-3 h-3" /> Open
                          </button>
                          <button
                            onClick={() => copyToClipboard(contentStr, 'File')}
                            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-raised border border-line text-[10px] font-semibold text-body touch-manipulation"
                          >
                          ...
                          {onDeleteFile && (
                            <button
                              ...
                              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-red-500/10 border border-red-500/40 text-[10px] font-semibold text-danger touch-manipulation"
                            >
                              <Trash2 className="w-3 h-3" /> Delete
                            </button>
                          )}
                          {copyNote && <span className="text-[10px] text-success">{copyNote}</span>}
                        </div>
                      )}
                      </div>
                    );
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
          {loadingHistory ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
              <History className="w-12 h-12 text-faint" />
              <p className="text-[11px] text-faint font-medium">No build history yet.</p>
              <p className="text-[9px] text-faint">Every successful build creates a checkpoint here.</p>
            </div>
          ) : (
            <div className="space-y-2">
              <p className="text-[9px] text-faint font-black uppercase tracking-widest mb-3">{versions.length} checkpoint{versions.length !== 1 ? 's' : ''}</p>
              {restoreError && (
                <p className="text-[10px] text-danger bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2 mb-2">{restoreError}</p>
              )}
              {versions.map((v, i) => (
                <div key={v.id} className="bg-card border border-line rounded-xl p-3 flex items-start gap-3 group">
                  <div className="mt-0.5 flex-shrink-0">
                    <GitCommit className={`w-3.5 h-3.5 ${v.isEdit ? 'text-warn' : 'text-success'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-bold text-body truncate" title={v.commitMessage}>{v.commitMessage}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[8px] text-faint">{formatRelativeTime(v.createdAt)}</span>
                      <span className="text-[8px] text-faint">·</span>
                      <span className="text-[8px] text-faint">{v.fileCount} files</span>
                      {v.tier && <span className="text-[8px] text-faint">· {v.tier}</span>}
                    </div>
                    <div className="flex items-center gap-1 mt-0.5">
                      <span className="text-[7px] text-faint font-mono bg-raised px-1.5 py-0.5 rounded">
                        v{versions.length - i}
                      </span>
                    </div>
                  </div>
                  {onRestoreVersion && (
                    <button
                      ...
                      className="flex-shrink-0 flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-600/20 border border-indigo-500/30 text-accent-text text-[8px] font-black hover:bg-indigo-600/35 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {viewPath !== null && (
        ...
        >
          <div className="flex items-center gap-2 px-3 py-2.5 border-b border-line bg-card">
            <Eye className="w-3.5 h-3.5 text-success flex-shrink-0" />
            <span className="text-[11px] font-semibold text-body truncate flex-1" title={viewPath}>
              {viewPath}
            </span>
            <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[9px] font-bold text-success bg-emerald-500/15 border border-emerald-500/30">
              READ ONLY
            </span>
            ...
            {isSecretFile(viewPath) && (
              <button
                ...
                className="flex-shrink-0 px-2 py-1 rounded-lg text-[10px] font-bold text-warn bg-amber-500/10 border border-amber-500/30 hover:bg-amber-500/20 transition-colors touch-manipulation"
              >
            ...
            <button
              ...
              className="flex-shrink-0 p-1.5 rounded-lg text-muted hover:text-ink hover:bg-raised transition-colors touch-manipulation"
            >
          ...
          <div className="flex-1 overflow-auto custom-scrollbar">
            <pre className="p-3 text-[11px] leading-relaxed text-body font-mono whitespace-pre-wrap break-words select-text">
              {viewerText.length > 0 ? viewerText : '(this file is empty)'}
            </pre>
            {isSecretFile(viewPath) && !revealSecret && (
              <p className="px-3 pb-3 text-[10px] text-warn leading-relaxed">{maskNotice(viewPath)}</p>
            )}
          </div>
          <div className="flex items-center justify-between gap-2 px-3 py-2 border-t border-line bg-card">
            {copyNote
              ? <span className="text-[10px] text-success">{copyNote}</span>
              : <span className="text-[10px] text-faint">Viewing only — nothing here can change the file.</span>}
            <button
