...
| A4.5 | **THE APP IS BUILT HERE; GITHUB ONLY PACKAGES IT** (`mobileShipPrebuilt.ts`, 2026-09-22, admin: *"toote hi na" wala banao*) — the app's own PRODUCTION build (`npm run build`, with the same `npx vite build` rescue for a type-only failure that Step B5 has) runs in the app's sandbox, the output is read out with the one reader the platform already has (`downloadDistFiles`, bridge-stripped), and it ships as `www/` with the honest no-op build script — so Step B5 is a no-op and the runner compiles nothing. `www/.nbai-prebuilt` is the stamp; `www/.nbai-shipped` records every `www/` path the push wrote, and only a recorded path is ever removed by a later push (a `www/` the user already owned is never touched). Before the build a stale marker is placed in every existing output directory — nothing is deleted from the machine — and an output that still carries it was not rewritten by this build and is refused as stale. The pushed package.json keeps only Capacitor, TypeScript (Capacitor's CLI reads `capacitor.config.ts` with it — `TYPESCRIPT_MISSING` below) and the plugins the machine named (read from `node_modules`, never guessed), so Step B4 installs seconds of packages, not the app's whole tree. Refuses ONLY on a positive app fault (`APP_CODE_BUILD_FAILED`, the npm resolution classes, `BUILD_SCRIPT_MISSING`); `UNKNOWN`, an out-of-memory or a killed build fall through to the source ship. Stands down when another build of the app is in flight, and never starts a build with under 45 s left on its clock. May WAKE a paused sandbox and seed an empty one (unlike A4.6); bounded by `MOBILE_SHIP_PREBUILT_MS` (default 240 s); `MOBILE_SHIP_PREBUILT=off` reverts | `APP_CODE_BUILD_FAILED` on the runner, on an app that already built here | A built app is shipped built; every stand-down (static app, no machine, timed out, output unreadable or too large) falls back to the source ship below, exactly as before; the ONE refusal is a build that failed here in a way the runner would fail too — the same 422 as A4.6 |
...
`hashFiles('package.json')`, the one manifest that IS pushed, with a prefix fallback key). Saved after a run that FAILED too
(`!cancelled()`, and only when the restore step actually ran), so a run that died after installing still
leaves its downloads for the retry after the repair — which is exactly the run that should not pay for
them again (2026-09-22). Every cache step is `continue-on-error`: a cache-service problem can never turn a
build that would have passed into a red run. A cache step is the runner's housekeeping and never
appears in the user's step list (`friendlyBuildStep`).
...
0. **`TYPESCRIPT_MISSING`** (added 2026-09-22) — Capacitor's CLI reads `capacitor.config.ts` with the
   PROJECT's own TypeScript and fatals ("Could not find installation of TypeScript") when package.json
   does not declare it — true of every hand-written static ship before this date. Every pushed
   package.json now declares it (`buildPackageJson`), and an old repository that dies this way is
   repaired by the rules tier (declare `typescript` in devDependencies).
