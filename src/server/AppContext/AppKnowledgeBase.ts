...
• IMPORT PROJECT (.zip) — ANY SIZE (admin 2026-07-28): tap the 📎 attach button in NavBharatAI Pro v5.0 and choose "Import project (.zip)". This is a DEDICATED option, separate from "Choose file", because a project is an IMPORT, not a chat attachment. Your zip is uploaded in small pieces (so there is no size limit — a 161 MB or larger app works), unpacked on the server, and your files open straight in the Files tab / Code Studio, ready to edit. The live PREVIEW then starts on its own (admin 2026-07-31): a static or simple-React app previews instantly in-browser (free, no tokens); a framework app (Vite/Next/React with a package.json) is AUTO-BOOTED — NavBharatAI installs its dependencies and starts the dev server so the live preview runs without you typing "run this app". (Booting a framework app runs the real dev server, so it spends a little build credit — that is the cost of an actual running preview; the free in-browser cases still cost nothing.) If the zip only contains node_modules or build output it says so honestly instead of importing nothing. (A zip attached through the ordinary "Choose file" option was previously packed into the build request and refused above 18 MB — that path is gone; use this option.)
