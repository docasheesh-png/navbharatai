      ...
      {!p ? (
        ...
        <>
          ...
          {p.today.length > 0 ? (
              ...
              {p.today.map((u, i) => (
                <li key={`${u.email}-${i}`} className="flex items-center justify-between gap-2 px-3 py-2">
                  <span className="min-w-0">
                    <span className="block truncate text-xs font-bold text-gray-100">{u.name}</span>
                    <span className="block truncate text-[10px] text-gray-500">{u.email}</span>
                  </span>
                  <span className="shrink-0 font-mono text-[10px] text-gray-500">{when(u.atMs)}</span>
                </li>
              ))}
              {p.todayTruncated && (
            ...
            <p className="py-3 text-center text-[11px] text-gray-500">Nobody has signed in yet today.</p>
          )}
        </>
      )}
    </div>
...
export function AudienceCard({ data, onOpen, loading }: AudienceCardProps) {
  ...
  return (
    <div className="rounded-[2rem] border border-white/5 bg-[#0d1117]">
      <button
        type="button"
        onClick={toggle}
        aria-expanded={open}
      ...
      >
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span className="min-w-0">
            <span className="block text-sm font-black uppercase tracking-widest text-white">Who came to NavBharatAI</span>
      ...
      {open && (
        <div className="space-y-4 border-t border-white/5 p-4">
          {!data && loading && <p className="py-4 text-center text-xs text-gray-500">Reading the counters…</p>}
          {!data && !loading && (
            <p className="py-4 text-center text-xs text-amber-300">
