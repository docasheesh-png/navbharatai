...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    id: 'api-tester',
    name: 'API Tester',
    path: 'Home → Other AI → Developer Tools → API Tester',
    ...
    howToUse: 'Home → Other AI → Developer Tools → API Tester. Choose the method, paste the endpoint URL, add any headers/body/token, keep "Route via NavBharatAI (bypass CORS)" ON for a cross-origin API, then tap Send to see the live status, headers and body. If a call in your app is failing, test that exact endpoint here to see the real response and status.',
  ...
  },
  {
    ...
    path: 'Home → Other AI → Developer Tools → Versioning',
  ...
  },
  {
    ...
    path: 'Home → Other AI → Developer Tools → Minifier',
    ...
    howToUse: 'Home → Other AI → Developer Tools → Minifier. On the left, tap your app → tap the file you want to make smaller (its code loads by itself). Tap the big "Optimise this code" button at the bottom. Check the saving shown (e.g. "42% smaller"), then tap the green "Apply to <file>" button on the right to save it into your app for real. Changed your mind? Home → Other AI → Developer Tools → Versioning → Restore. To optimise code that is not from your app, just paste it into the middle box, pick the language, and tap Optimise.',
  ...
  },
  {
    ...
    path: 'Home → Other AI → Design & Build → Multi-Page',
    ...
    howToUse: 'Home → Other AI → Design & Build → Multi-Page. Use the Pages panel to add each page you want and rename it. Open the Editor to write its content and set its title/description. In "Save & Nav", pick the navigation style, choose your app, and tap "Save N pages into my app" — the pages appear as real files in that app. Made a mess? Home → Other AI → Developer Tools → Versioning → Restore.',
  ...
  },
  {
    ...
    keywords: [
      ...
      'apk kaise', 'apk kaise banaye', 'apk download', 'apk chahiye kaise', 'app download kaise kare',
      'phone me install', 'mobile app file', 'android file', 'apk file kaha se',
      'play store', 'google play', 'publish app', 'app banao', 'android banao', 'apk banao',
      'app store par daalo', 'play store par daalo', 'signed apk', 'keystore', 'signing key',
      'app icon', 'icon upload', 'icon banao', 'mobile app', 'phone app', 'install app',
      'apk chahiye', 'app file', 'real app', 'native app',
    ],
    nav: { view: 'apk' },
  },
  {
  ...
  },
  {
    ...
    path: 'Home → Other AI → Design & Build → Components',
    ...
    howToUse: 'Home → Other AI → Design & Build → Components. At the top pick your app and the page to add to. Browse a category or search, tap a component to preview it, then tap "Add to my app" — it is written into that page for real. Use the CDN Libraries tab to add a library (jQuery, Chart.js, etc.) the same way. To undo: Home → Other AI → Developer Tools → Versioning → Restore.',
  ...
  },
  {
    ...
    path: 'Home → Other AI → Design & Build → Design System',
    ...
    howToUse: 'Home → Other AI → Design & Build → Design System. On the Colors tab tap any swatch to change it; check Typography and Spacing too. Open the Export tab, pick your app and the file to add the tokens to (usually your main .css or index.html), and tap "Save tokens into my app". Undo any time from Home → Other AI → Developer Tools → Versioning.',
  ...
  },
  {
    ...
    path: 'Home → Other AI → Design & Build → Dark Mode Gen',
    ...
    howToUse: 'Home → Other AI → Design & Build → Dark Mode Gen. Tap Auto-Generate, pick the dark palette you like, and change any colour you want. Choose your app and the page at the bottom, then tap "Save dark mode into my app". To reverse it: Home → Other AI → Developer Tools → Versioning → Restore.',
  ...
  },
  {
    ...
    path: 'Home → Other AI → Design & Build → Figma Import',
  ...
  // ─── SETTINGS ─────────────────────────────────────────────────────────────
  {
    ...
    aiSurface: 'nbi_chat',
    description: 'The fourth card on the Home page (next to NavBharatAI Free, Pro and Professionals). Opening it reveals every AI-builder utility, grouped: AI Tools (Bot Builder, AI Image Gen, AI Debugger, Code Review), Developer Tools (Test Runner, API Tester, Versioning, Performance, Minifier), Design & Build (Multi-Page, Components, Design System, Dark Mode Gen, Figma Import), Publish & Deploy (APK Builder, CI/CD Pipeline, Custom Domain, SEO Optimizer, App Store — Multi-Cloud Deploy moved to Settings → App Settings), and Monetization & Team (Monetize, Team, Live Collab, Whitelabel, Analytics, Insights & Webhooks, Database). These moved here from Settings (admin 2026-07-23) so all builder tools sit alongside the main AIs; each tile opens the same tool it always did.',
  ...
  },
  {
    ...
    description: 'The settings hub. Organized into groups: Account & Profile; and App Settings — everything a real, live website needs, brought into ONE place (admin 2026-07-29): Domain (connect your own domain — DNS + SSL included), Hosting & Deploy (your app is auto-hosted with a live URL; also deploy to Vercel/Netlify/Firebase/Cloud Run/Railway/Render), Database (connect your own DB — also provides login + storage when you connect Firebase/Supabase), Authentication (connect Clerk/Auth0 for login), Storage (connect S3/Cloudinary for uploads), Secrets & API Keys, plus the developer tools General, Terminal and Logs. Frontend + Backend CODE is built for you by NavBharatAI Pro, so they are not settings. The builder tools (AI Tools, Developer Tools, Design & Build, Publish & Deploy, Monetization & Team) live on the HOME page under the "Other AI" card (admin 2026-07-23) — open Home → Other AI. (Git lives in the sidebar → Git, not in Settings.)',
  ...
  },
  {
    ...
    description: 'Instead of leaving NavBharatAI to create a Supabase project, find two keys and paste them back, you can connect your Supabase account once and let NavBharatAI create the database for you. IMPORTANT: the project is created INSIDE YOUR OWN Supabase account — your data and its billing stay yours, and NavBharatAI never hosts your app data. After it is created, the keys are saved encrypted into Secrets & API Keys automatically, so there is nothing to copy and your next build uses that database by itself. NavBharatAI asks for only five permissions (create projects, read your organization, read the new project keys, configure auth, run migrations) and never asks for analytics, domains, storage or edge functions. Honest limits: Supabase\'s FREE plan allows only 2 projects per organization — if yours is full, NavBharatAI says so and tells you to delete an unused project or upgrade in Supabase, rather than failing silently. A new database takes a minute or two to start; NavBharatAI waits until it is genuinely usable before saying it is ready. Disconnecting makes NavBharatAI forget your connection, but does NOT revoke access on Supabase\'s side — remove the NavBharatAI app in your Supabase account settings to do that. If this deployment has no Supabase app configured, the one-tap card does not appear at all and you simply use the manual form.',
  ...
  },
  {
    ...
    path: 'Settings → App Settings → Terminal  (also Code Studio → Terminal tab)',
    description: 'A REAL, PERSISTENT shell into the sandbox where NavBharatAI Pro v5.0 builds your app — literally the same terminal Code Studio has, reachable straight from Settings so you can run a command without opening the full IDE. It is a genuine terminal session, not a one-command box: your directory stays where you left it (cd into a folder and the next command runs there), environment variables persist, output streams LIVE as it is produced, and there is no time limit, so npm install or a full build can run as long as it needs. Ctrl+C really interrupts. Colours, progress bars and full-screen programs display properly, and interactive prompts can be answered. Use "+ New" to open several independent terminals at once. It shares the exact workspace with Pro v5.0, Code Studio, Files, and Preview — a file the build wrote is immediately visible to ls here. Needs a warm sandbox: start or continue a build in NavBharatAI Pro v5.0 first; until then it honestly says the sandbox is not active (it never fakes output).',
    howToUse: 'Settings → App Settings → Terminal. Type a command and press Enter. Press Ctrl+C to stop a running command, and use "+ New" inside the terminal to open or switch between terminals. If it says the sandbox is dormant, send a message in NavBharatAI Pro v5.0 chat to bring it back online, then run your command.',
    relatedFeatures: ['ide_terminal', 'settings_logs', 'pro_chat'],
    keywords: [
      'terminal settings', 'shell', 'command line', 'console', 'bash', 'terminal', 'run command',
      'command chalao', 'npm install', 'terminal kahan', 'cd', 'ctrl+c', 'stop command', 'persistent shell',
      'real shell', 'live output', 'multiple terminals', 'naya terminal',
    ],
  },
  {
    id: 'settings_logs',
