  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line pb-4">
        <div>
          <span className="rounded-xl border border-sky-500/20 bg-sky-500/10 px-3 py-1.5 font-mono text-[10px] font-black uppercase tracking-wider text-info">
            Statement
          </span>
          <h3 className="mt-3 text-xl font-black uppercase tracking-tight text-ink">Every credit and every charge</h3>
        </div>
        <button
          onClick={() => void load()}
          disabled={loading}
          className="inline-flex items-center gap-2 rounded-xl bg-raised px-4 py-2 text-[10px] font-black uppercase tracking-widest text-muted transition-colors hover:text-ink disabled:opacity-40"
        >
      ...
      {!data ? (
        ...
        <p className="text-xs font-semibold text-warn">Could not load your statement. Please try again.</p>
      ) : data.exists === false ? (
        <p className="text-xs font-semibold text-muted">This account has no wallet activity yet.</p>
      ) : (
        <>
          ...
          <div className={`rounded-2xl border p-5 ${
            ...
            <div className="flex items-start gap-3">
              {verdict === 'balanced' ? <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-success" />
                : verdict === 'off' ? <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-danger" />
                : <HelpCircle className="mt-0.5 h-5 w-5 shrink-0 text-warn" />}
              <div className="min-w-0">
                <p className={`text-sm font-black ${
                  verdict === 'balanced' ? 'text-success' : verdict === 'off' ? 'text-danger' : 'text-warn'}`}>
                ...
                {(data.notes ?? []).map((n) => (
                  <p key={n} className="mt-1.5 text-[10px] font-semibold leading-relaxed text-muted">{n}</p>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-line bg-well">
            <table className="w-full text-left font-mono text-[11px]">
              <thead>
                <tr className="border-b border-line bg-well text-[9px] font-black uppercase tracking-widest text-muted">
              ...
              <tbody className="divide-y divide-line font-semibold">
                {rows.length === 0 ? (
                  <tr><td colSpan={4} className="px-4 py-6 text-center text-muted">No entries yet.</td></tr>
                ) : (
                  ...
                  [...rows].reverse().map((r, i) => (
                    <tr key={`${r.timestamp}-${i}`} className="transition-all hover:bg-raised">
                      <td className="whitespace-nowrap px-4 py-3 text-muted">{when(r.timestamp)}</td>
                      <td className="px-4 py-3 text-ink">{r.description}</td>
                      <td className={`whitespace-nowrap px-4 py-3 text-right font-bold ${
                        r.kind === 'credit' ? 'text-success' : r.kind === 'debit' ? 'text-danger' : 'text-muted'}`}>
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-right text-ink">{inr(r.runningRupees)}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          <p className="text-[10px] font-semibold leading-relaxed text-muted">
