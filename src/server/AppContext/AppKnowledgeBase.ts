...
• POWER SELECTOR (build-options menu — the ⚙️ gear/settings popover opened from the toolbar just below the message box): five tiers of build engine, cheapest → strongest. "Weak" = the FREE tier (runs on NavBharatAI's fast economy engine — the free tier that never uses the premium engines). "Normal" = balanced (the standard engine, adaptive). "Strong 💪" = a stronger engine, pinned for the whole build. "Powerful" = NavBharatAI's most capable engine at higher reasoning effort. "Full Team" = the most capable engine at maximum effort (ultracode). The tier you select is exactly the engine that runs — the backend pins it. TALK TO THE TEAM WHILE THEY BUILD — ON EVERY TIER (2026-08-18): you do NOT have to stop a build to correct it. While an app is being built, the message box stays LIVE: type what you want changed and send it, and the team acts on it at the very next step (a "queued" note appears instantly, then "picked up" when they fold it in). Your Send button becomes a message-the-team button and Stop moves next to it, still one tap away. This used to be a Full Team–only feature and is now open to EVERY tier, including the free Weak tier — because the alternative was stopping the build and rebuilding from scratch, which costs far more of your credits than one message does. Up to 5 messages can wait at once; if you send more, NavBharatAI honestly tells you the queue is full instead of quietly dropping the message. USE IT WHEN: you see the build heading the wrong way, you forgot to mention something, or you want a colour/name/feature changed — mid-build is cheaper and faster than waiting and asking for a rebuild. IF A USER ASKS "build ke beech me message kaise bhejein / can I talk while it builds": just type in the normal message box while the build runs and press Send — nothing to switch on. FULL TEAM PREMIUM EXPERIENCE (Fix 60): on the Full Team tier a running build ALSO shows the live ⚡ FULL TEAM HQ card above the message box — the real agent roster (Architect + every specialist, with what each is doing right now), real plan-progress squares, and a live clock. That card stays exclusive to Full Team; the mid-build messaging above does not. WHO SEES WHAT: a FREE user (never purchased) can use ONLY "Weak"; once you add credits (become a paid user) all five unlock, defaulting to "Normal", and you can pick any of them per build. Billing follows the tier and the real work done: the lower tiers cost the least, and the two top ("Powerful"/"Full Team") tiers cost more because they run the most capable engine at higher effort — the bill scales with the tokens actually spent, and a live cost estimate is always shown. This is enforced server-side — a free account can never spend the paid engines. IF A FREE USER ASKS why the other tiers are locked / "select nahi ho raha" / greyed out with a 🔒: explain warmly, IN THE USER'S OWN LANGUAGE and in your own words (never a canned copy-paste — vary the phrasing each time, keep the meaning), that on a free account only the "Weak" tier is available, and that recharging (adding credits to the wallet) instantly unlocks every tier from Normal to Full Team — the tier selector is behind the ⚙️ options button (the Settings gear, title "Build options") in the toolbar just below the message box. IF THE USER SAYS THEY ALREADY RECHARGED/PAID but the tiers are still locked: NEVER repeat "recharge karo" — instead walk them through (in their language, own words): (1) close and reopen the 🎛️ options popover (it re-checks your account) or refresh the page once; (2) open Wallet & Billing from the sidebar and confirm the payment shows as SUCCESS and the tokens were credited; (3) if the payment shows SUCCESS there and the tiers are STILL locked after a refresh, apologize and tell them to contact support with their payment reference — do not blame them or loop the same suggestion.
...
• RESIZE THE SPLIT — the border between the chat and the workspace MOVES (admin 2026-08-17). On DESKTOP: drag the border left or right to give either side more room, or double-click it to snap back to an even 50/50. While dragging, a small number shows the preview's live width in pixels — drag it narrow and you are looking at your app at a phone width, which is the quickest way to check that it works on a small screen. On TABLET: tap the ◀ ▶ buttons ON the border to step it left or right (a tap is exact where dragging a thin line is not). Keeping ▶ going past the last step closes the workspace so the chat fills the screen. Arrow keys work too when the border is focused. Your chosen width is REMEMBERED for next time. ONE-TAP DEVICE WIDTHS: the Preview's own toolbar (in the row with "In-browser / Live server") has Auto · Mobile · Tablet · Desktop buttons. Auto fills the panel; Mobile, Tablet and Desktop render your actual app at a real phone/tablet/desktop width so you can check a small screen without leaving the builder — nothing is faked or emulated, it is the real app in a real browser at that width. (A duplicate Phone/Tablet/Full set that used to sit in the Preview header was removed on 2026-08-19 — it did the same job as this toolbar, so there was one control too many.) On a PHONE there is no split at all — the chat and the workspace each take the full screen and you switch between them — so there is nothing to resize there.
• TWO PREVIEWS — FREE vs PAID (admin 2026-08-16): the Preview tab has two modes. IN-BROWSER (the default) renders your app right in your own browser — instant, always available, and FREE (no server, no credits). LIVE SERVER runs the full app on a real cloud machine (real npm/runtime, full fidelity) — this is a PAID service: it uses your credits while it is on, so it is for when you need the true running app, not for casually leaving open. The Live-server toggle is marked "Paid", and switching to it shows a one-time note explaining this. If someone asks whether the live preview costs anything: in-browser is free, live server is paid.
...
• NEXT-BUILD SUGGESTIONS 💡 (admin 2026-08-13): after a build finishes, a small lightbulb icon lights up BELOW the message box (bottom-right of the v5.0 composer), with a little count badge. Tap it to see a short list of tailored "what could I build next?" ideas for YOUR specific app. THE ORDER IS DELIBERATE — the ideas come from your app's OWN MEMORY first, never a random list (upgraded 2026-08-20): (1) ANYTHING YOU ASKED FOR THAT IS NOT THERE — if you said "add a search box" in an earlier message and it did not get built, that comes FIRST, in your own words ("Finish the search — you asked for this earlier"); when NavBharatAI could not fully prove it is missing, it says so and invites you to ignore it if it is already there. (2) WHAT THE LAST BUILD ACTUALLY FOUND about this app and left unfixed — errors it showed when it ran, data that did not survive a reload, a page that failed to load, plainer inner pages, unsafe libraries, failing tests, accessibility gaps. (3) ONLY THEN the ideas guessed from your app's files — domain gaps (e.g. for a game: sound, save/high-score, a tutorial; for a shop: cart, payments, order management) and finally universal polish (dark mode, mobile-friendly layout, search, share, export, animations). It NEVER runs anything on its own: tapping a suggestion just drops a ready-to-send instruction into the message box for you to review, edit, or ignore, then send when you want. The ideas are computed for FREE from your own app — your past requests, the last build's own findings, and your app's files (no AI cost, instant) — only show what your app does not already have, and refresh after each build. If there is nothing worth suggesting, the bulb stays hidden. GUIDED ROADMAP (for BIG apps): when you ask for a very large app in one line (for example a well-known app or game — "PUBG jaisa", "WhatsApp jaisa", "an app like Instagram"), NavBharatAI is honest that it is big, builds a real WORKING first version (the core) quickly so you get a live preview in a few minutes, and then lays out the rest as a simple step-by-step ROADMAP inside this same 💡. Each checkpoint shows what it will add and whether it needs extra setup; tap "Build next step" and it drops that step's instruction into the box for you to review and send — you grow the app one working milestone at a time, and the roadmap remembers where you are even if you come back later. This guided roadmap appears only for genuinely large apps; ordinary apps just build directly with no extra steps.
...
• NAME YOUR SAVED VERSIONS (2026-08-18): every build saves a restore point, and in the History tab you can now give each one YOUR OWN name — tap the pencil next to it and type something you will recognise, like "login theek hone se pehle" or "before I changed the colours". Press Enter to save, Escape to cancel. Before this, the list was a row of near-identical automatic messages, so going back to the right version meant guessing — the restore feature existed but was not really usable. The name is saved to your account, so it is still there on your next visit and on your other devices. Names can be in Hindi or any language, up to 80 characters. Clear the box and press Enter to remove a name. If a name cannot be saved, NavBharatAI says so plainly instead of showing a name that would disappear later. IF A USER ASKS "purana version wapas kaise laayein / kaunsa version sahi tha": tell them to open History, name the good versions as they go, then use Preview (safe, opens in a new tab) or Restore on the one they want.
• A REAL TERMINAL INSIDE YOUR APP — 30 FREE MINUTES A DAY, AND YOU CAN OPEN SEVERAL (updated 2026-08-21): the Terminal tab opens on a PINNED first tab called \"NavBharatAI · build log\" — the record of what NavBharatAI ran while building your app. It is read-only and cannot be closed, because it is our log, not a shell you type in. To get YOUR OWN terminal, press \"+ New\" in the terminal header: each one is a REAL terminal inside your app's own machine (run npm commands, look at files, check versions — anything a developer would), each has its own close (✕) button, and you can keep several open at once (for example a dev server running in one while you work in another). Switching between them never kills what is running. IMPORTANT AND OFTEN ASKED: simply LOOKING at the build log costs NOTHING — no terminal is started until you press + New. FREE, WITH A DAILY LIMIT of 30 minutes per day, which resets every day, and the header always shows how much is really left today (for example \"22 free minutes left today\"). The 30 minutes are counted ONCE by the clock even if several terminals are open — they all run on the same machine, so opening a second one does NOT make the time run out faster. The limit exists for an honest reason: an open terminal keeps a real cloud machine running and NavBharatAI pays for that time. When you are close to the limit NavBharatAI warns you in the terminal itself, and when it runs out it says so clearly instead of going silent — YOUR APP AND FILES ARE NEVER AFFECTED, only the terminal pauses until tomorrow. IF A USER ASKS \"terminal band kyun ho gaya / 30 minute khatam\": explain warmly that it resets tomorrow, their app is untouched, and building/editing through chat has no such limit.
• TELL NAVBHARATAI WHICH FILES TO NEVER TOUCH (2026-08-18): create a file called .navbharataiignore in your project and list what must be left alone — one per line, like "payments/" for a whole folder, "*.env" for a type of file, or "/config.ts" for one exact file at the top level. NavBharatAI will then REFUSE to create, change, move or delete anything on that list — this is a real block, not a polite request, so it holds even during a big rebuild. You can make an exception with an exclamation mark: write "payments/" to protect the folder and then "!payments/README.md" on the next line to still allow that one file. WHY IT MATTERS: the one thing worse than a bug is an AI quietly changing the part of your app that was already working and earning — your payment flow, your live config. This makes that impossible through the AI. IF IT NEEDS TO CHANGE SOMETHING PROTECTED, IT ASKS YOU instead of doing it. HONEST LIMIT: this stops the AI's own file edits; a command you yourself run in the Terminal can still change those files, because that is you, not the AI. Ask in chat to create it — ".navbharataiignore banao aur usme payments folder daal do" — and it will be made.
• POINT AT A FILE WITH @ (2026-08-18): type @ in the message box and a list of your project's files appears — pick one (Tab to insert, Esc to close, or just tap it) and NavBharatAI works on that file instead of hunting for the right one. For example: "@src/Header.tsx isko upar chipka do". WHY IT IS WORTH USING: without it, every edit request starts with NavBharatAI SEARCHING your project to work out which file you meant — that search costs your tokens, and on a big app it can pick the wrong file, which costs you again to fix. Naming the file removes both. You can name more than one file in a message (up to 10). You do not have to use the list — typing the path yourself works too, and a plain filename like @Header.tsx is enough when only one file has that name. IF THE FILE IS NOT FOUND, YOU ARE TOLD: NavBharatAI says "I could not find @… in this project" instead of quietly ignoring it, so you are never left wondering why nothing changed. Naming a file is a STARTING POINT, not a fence — if the change also needs another file, NavBharatAI still edits that one, so nothing ends up half-done.
• YOUR APP CAN HAVE ITS OWN PERMANENT RULES (2026-08-18): create a file called NAVBHARATAI.md in your project and write your rules in it in plain language — "sab labels Hindi me", "prices hamesha ₹ me dikhao", "payments folder ko mat chhedo, pehle mujhse poochho", "design simple aur mobile-first rakho". NavBharatAI reads that file BEFORE every single change, so you never have to repeat those instructions again in every message. The easiest way to make it: just ask in chat — "NAVBHARATAI.md banao aur usme likho ki sab labels Hindi me hone chahiye" — and it will be created. You can edit it later like any other file. WHEN IT IS USED, YOU ARE TOLD: a small line appears saying "Using your project rules from NAVBHARATAI.md", so a rule is never applied invisibly. IMPORTED PROJECTS WORK TOO: if your project already has AGENTS.md, CLAUDE.md or .cursorrules from another tool, NavBharatAI reads that instead — nothing to convert. The file must sit at the TOP level of your project (not inside a folder), and only about the first 8,000 characters are used — if it is longer, NavBharatAI says so rather than quietly ignoring the rest. Your rules never override the two things that always come first: your app must keep working, and whatever you ask in your CURRENT message wins over a standing rule. IF A USER ASKS "har baar same baat kyun likhni padti hai / AI meri pasand yaad kyun nahi rakhta": this file is the answer.
• NAVBHARATAI TELLS YOU WHEN A CHAT IS GETTING TOO LONG (2026-08-18): a small bar appears just above the message box when the conversation has grown long, saying that older messages are being summarised to make room — and when it is nearly full, it says so plainly and offers a "New chat" button. WHY THIS MATTERS: in a very long chat the AI genuinely starts to lose the earliest details, and until now that just looked like the app getting worse for no reason. Now it has a visible cause and an obvious fix. Your app, your files and your saved versions are NEVER affected by starting a new chat — only the conversation history restarts. The bar stays hidden while everything is fine, so it never nags. IF A USER ASKS "AI purani baat bhool raha hai / pehle jaisa accha kaam nahi kar raha": check whether this bar is showing; if the chat is long, starting a new chat for the next change is the real fix, and their work is safe.
• SEE WHICH PARTS OF YOUR APP ARE ACTUALLY RUNNING (2026-08-18): at the top of the App Logs tab there is now a small live status strip — a green dot for each part of your app that is really running, a red dot for a part that should be running and is NOT, and the port each one uses. This is usually the whole answer when an app "looks broken": if your app has a separate backend and only the front page is running, the screens load but nothing saves — the red dot says exactly that, in one line, instead of leaving you guessing. Background jobs that never use a port are shown in grey and are NOT reported as broken, because they are working normally. Anything else using a port is listed too, and named where NavBharatAI knows it — for example your app's own database — so you never see a mystery number. Every dot is CHECKED live at that moment, never assumed from settings: NavBharatAI will not show green for something it has not actually found running. IF A USER ASKS "app khul to raha hai par kaam nahi kar raha / data save nahi ho raha": send them to App Logs and read this strip first.
• SEE WHAT YOUR OWN APP IS PRINTING — the "App Logs" tab (2026-08-18): open it from the tab row (on a phone: the … More button → App Logs) to watch your app's REAL server output live — every error, every crash, every line your backend prints. This is the thing to open when the app "just does not work" and the screen shows nothing useful: the reason is almost always sitting in here. It is DIFFERENT from the Terminal tab — Terminal shows what NavBharatAI did while building, App Logs shows what YOUR app is doing right now. It updates by itself every couple of seconds, follows the newest line, and stops following the moment you scroll up to read something, so an error you are reading is never yanked away. If the app restarts, or it printed faster than the screen could read, a small honest line says so instead of letting the jump look like a bug. An empty pane always tells you WHICH kind of empty it is — never built yet, built but not running (send a message to wake it), or running but silent — because those three look the same and mean completely different things. IF A USER ASKS "error kaise dekhein / app kyun nahi chal raha / server error kahan hai": send them here first. It only checks while the tab is open, so it costs nothing when closed.
• READS A LINK YOU PASTE (2026-08-18): paste any public web address into the chat — API docs, a reference page, a spec, a pricing page — and NavBharatAI opens it and READS it before building, instead of guessing from what it already knows. So "is page ke API ke hisaab se banao" works: it sees the actual field names, the actual endpoints, the actual version. It could already SEARCH the web and take a PICTURE of a page; reading one is new. HONEST LIMITS, so nobody is misled: it only opens public http/https pages (never a private/office/localhost address, for safety), it does not follow a page that redirects somewhere else — paste the final address — and a very long page is read up to about 30,000 characters and it SAYS when it stopped. A page that draws itself entirely with JavaScript may come back empty; NavBharatAI then says so and looks at it as an image instead. It never signs in for you, so anything behind a login cannot be read — paste the text yourself.
...
• COST BREAKDOWN ("Why this cost?"): after a billed build, a "Why this cost?" toggle appears next to the ₹ amount in the result footer — expand it to see exactly how the charge was computed: your input/output token split, the tier that ran, the base cost for that tier, the markup applied, and the final USD → ₹. Free builds show no charge to explain.`,
  ...
  },
  {
    ...
    description: 'Publish your built app to a PERMANENT public URL. The "Publish" button opens a Hosting chooser — host free on NavBharatAI, or bring your own provider (no lock-in) — then runs the production build and publishes, returning a real https URL that anyone can open and that STAYS LIVE even after the cloud sandbox stops. Providers: Firebase Hosting (always available) plus Cloudflare Pages, Vercel and Netlify (and more) when their API token is configured by the admin. Once published, a "Live site" link appears in the same row — click it to open your live app, or share the URL with anyone. The live link is saved, so it comes back even after you refresh or return in a new session.  REMOVING A PUBLISHED APP (2026-08-21): open Publish and use \"Remove this app from NavBharatAI hosting\" inside the green NavBharatAI card. It appears ONLY for an app that is actually live right now. It asks once to confirm, because anyone holding the link stops being able to open it the moment it runs. The code and the chat are NOT touched and the app can be published again at any time — this is not a deletion and it is not a ban. Removing an app also frees one of the five free slots. NOTE: deleting the CHAT does NOT take a published app offline — a link someone shared keeps working — so to take an app down, use this button while the chat still exists. HOW MANY ARE FREE (2026-08-21): you can keep FIVE apps published on NavBharatAI hosting for free. UPDATING an app you have already published is ALWAYS free and never counts against that five — publish a fix as often as you like. The five is about how many DIFFERENT apps are live at once, because each live app occupies real hosting we pay for. There is also a fair-use size limit: 50 MB per publish, and 200 MB total across your published apps (a normal app is well under 1 MB, so this is room for hundreds of apps). IF SOMEONE HITS THE LIMIT: they can remove an app they no longer need, or publish that app to their OWN free host (Vercel / Netlify / Cloudflare Pages) from the same Publish sheet — that runs on their account and is free from us, with no limit from NavBharatAI at all. SEEING EVERYTHING YOU HAVE PUBLISHED (2026-08-21): open Publish and click \"Your published apps\" inside the green NavBharatAI card. It lists EVERY app you currently have live on NavBharatAI hosting — the link, the size and when it was last updated — and shows how many of your five free slots are used, so you can see the limit before it stops you. Each app has a \"Take offline\" button, so you can remove one from here without opening its chat. This is the ONLY place to reach an app whose chat you already deleted: that app stays live (a shared link keeps working) but has no chat to open, and the list says so plainly and still lets you take it down. To put it on your own domain, use Settings → App Settings → Domain.',
    ...
    keywords: ['deploy', 'publish', 'go live', 'live url', 'public url', 'host', 'hosting', 'share app', 'live site', 'make it live', 'put online', 'deploy karo', 'live karo', 'publish karo', 'app live', 'website live', 'permanent url', 'share link', 'firebase hosting', 'cloudflare', 'cloudflare pages', 'vercel', 'netlify', 'hosting provider', 'kahan deploy', 'no lock-in', 'launch', 'my published apps', 'my live apps', 'meri published apps', 'kitni app live hai', 'take offline', 'unpublish', 'remove app', 'app hatao', 'live app list', 'how many apps published', 'free slots'],
    aiSurface: 'nbi_chat',
  },
  {
    id: 'agentv3_preview',
    ...
    keywords: ['preview', 'live preview', 'in-browser preview', 'browser preview', 'blocked request', 'preview not working', 'preview nahi chal raha', 'app dekho', 'see app', 'run app', 'sandbox preview', 'static preview', 'dual preview', 'preview kaise', 'diagnose', 'diagnose button', 'diagnose preview', 'preview diagnosis', 'internal problem', 'preview kya problem hai', 'restart server', 'restart the server', 'server restart karo', 'preview atak gaya', 'preview stuck', 'blank preview', 'preview blank hai', 'app nahi chal raha', 'server band ho gaya', 'preview khali hai', 'sleep mode', 'preview sleep', 'wake up', 'wakeup', 'wake up preview', 'preview so gaya', 'preview sleep mode', 'preview wapas kaise', 'preview band ho gaya', 'preview dobara chalao'],
    aiSurface: 'nbi_chat',
  },
  {
...
• SELF-REVIEWS edits with a focused pass to catch missing imports and logic bugs.`,
  ...
  // ─── DOCTOR AI ───────────────────────────────────────────────────────────
  {
    ...
    description: 'Senior-doctor assistant AND mentor for qualified/junior/rural doctors. TWO modes: (1) PATIENT CASE — efficient high-yield history, red-flag screening, ranked differentials, investigation guidance, coded, unit-tested clinical calculators (CURB-65, CRB-65 [no-lab/PHC], qSOFA, GCS, Wells DVT/PE, CHA2DS2-VASc, eGFR/Cockcroft-Gault for renal drug dosing, anion gap, Killip class, Centor/McIsaac, paediatric maintenance fluids & weight-based dosing), an independent second-AI safety cross-check, and answers grounded in standard safety references with a clear "manage here vs refer NOW" decision. (2) GENERAL HELP — solves other junior-doctor queries: procedures/how-to, guidelines & protocols, drug information, documentation & medico-legal (discharge summary, referral letter, informed consent, certificates), communication (breaking bad news/SPIKES), exam/career guidance, and wellbeing/burnout support. (3) REPORT & IMAGE ANALYSIS — send ANY medical report as an image or PDF (ECG, X-ray, USG, CT/MRI film, lab report, prescription) and it reads it systematically and helps build a PROVISIONAL DIAGNOSIS: report type + patient details as printed, systematic findings (ECG: rate/rhythm/axis/intervals then lead-by-lead morphology; X-ray: systematic anatomical review; labs: each value graded vs range), ranked differentials with confidence, red flags first, and next steps with a manage-here vs refer decision. The report STAYS in the session: follow-up questions about the same report ("lead V2 dekho", "is xray me fracture hai?") re-examine the actual image — you do not need to send it again. COMPARE WITH AN OLDER REPORT: send the new one and ask "purane ECG se compare karo" — both films go together and it states exactly what has CHANGED and which finding is NEW, which is what decides management. A second AI also re-reads the report itself to cross-check the reading before you act on it. It states honestly which region is unreadable instead of guessing. IMPORTANT: decision-support only — assists, never replaces, the treating physician; medico-legal/career answers are general guidance to verify locally.',
    howToUse: 'Open Doctor AI and either describe a patient case (it does a fast focused workup), attach a report photo/PDF (ECG, X-ray, USG, lab report) and ask for an analysis/provisional diagnosis — then keep asking follow-ups about the same report without re-sending it — or just ask any doctor question — e.g. "how do I write a discharge summary", "steps for lumbar puncture", "breaking bad news", "PG exam prep". It answers in the right mode automatically.',
...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    keywords: [
      ...
      'stop build', 'build stop', 'build roko', 'cancel build', 'build cancel', 'build rokna',
      'build report', 'apk build report', 'build fail kyu', 'build kyu fail hui', 'why build failed',
      'build failed reason', 'report download', 'json report',
    ],
    nav: { view: 'apk' },
  },
  {
  ...
  {
    id: 'nav_app_store',
    name: 'App Mart — play apps others made, install Android apps, and publish your own',
    path: 'Home → App Mart (the 5th card on the Home page — it is NO LONGER inside Other AI; or the direct link navbharatai.com/store — same as navbharatai.com/?view=appstore)',
    description: 'NavBharatAI\'s own app store. BROWSE shows every published app — tap one to read what it does, see exactly what it will be allowed to do on your phone, and download the .apk. PUBLISH is ONLY for apps you built with NavBharatAI — there is NO app-file upload, so nobody can put someone else\'s app (or malware) on the store. You cannot upload a .apk, a .zip, or any file from your device; you publish an app straight from its finished build (see MY APPS / How to use). A NavBharatAI app you saved to your own GitHub counts too — it is still a NavBharatAI build. Publishing is FREE (₹0). MY APPS shows where each of your submissions stands. Every submission goes through the same checks, in this order: NavBharatAI first confirms the file really is a properly signed Android app (an unsigned one is refused, because no phone can install it), records its exact fingerprint, and reads the permissions it asks for; then it is scanned for malware against around 70 security engines. An app the engines call malicious is refused and never stored. An app that cannot be scanned is NOT published — never published unscanned. An app that passes still does not go live: it waits for a person to check it, because malware built for one particular attack is often unknown to every engine on the day it appears. Only a NavBharatAI reviewer can publish an app. Apps that ask for sensitive permissions — reading your text messages and bank OTPs, controlling the screen, installing other apps, drawing over your bank app — are clearly marked, both for the reviewer and for anyone about to download. An app that turns out to be harmful is removed and its file deleted, not just hidden.',
    howToUse: 'Home → App Mart (the 5th Home card — it moved OUT of Other AI on 2026-08-16). Browse has two halves: "Play instantly" (apps that run in the browser, nothing to install) and "Install on Android" (real .apk apps). To install something: stay on Browse, tap an app, read what it can do on your phone, then tap "Download .apk" — Android will ask you to allow installs from your browser. To publish your own app you do NOT upload a file — you publish it straight from its build (only apps NavBharatAI built can go on the store): (1) open the app you built with NavBharatAI — a new one, or one saved to your GitHub; (2) on the build screen (Other AI → Publish & Deploy → App Builder), build its Android app (APK); (3) once it is built, tap the "Publish to App Mart" button right next to Download, fill in your name, contact email and app details, tick the confirmation, and tap "Send for review" — NavBharatAI takes the app straight from the build, so there is no file to choose or upload. It is scanned for malware straight away and then waits for a reviewer, usually a day; watch its progress under My apps. The Publish tab in App Mart itself explains this and points you to the build — it deliberately has no file chooser, because a device file could be anyone\'s app. SHAREABLE LINK: the store also opens directly from navbharatai.com/store (or navbharatai.com/?view=appstore) — this lands anyone straight on the public Browse tab without logging in, so it is the link to share when you want someone to see whether an app is live.',
  ...
  },
  {
    ...
    name: 'Instant apps on App Mart — publish in one click, others run it in their browser',
    path: 'NavBharatAI Pro v5.0 → Publish → "Put it on App Mart" (the green Instant card, next to "Make an Android app"). To browse: Home → App Mart → Play instantly.',
    description: 'Publish the app you built in v5.0 straight to App Mart as an INSTANT app: one click, no APK, no hosting, no deploy — other people open it from the store (or your share link) and it runs immediately, full-screen, in their own browser. What ships is a snapshot of your app taken at publish time, so your later edits never break it for viewers until you publish again (same link, new version). Your source files, your .env and your saved keys NEVER ship — and publishing is REFUSED with the exact file and line if a real API key is found hardcoded in the app, because published code is visible to every viewer. Apps that need a live server cannot be instant apps; the publish button says so honestly and points to hosting instead. Your share link works the moment you publish; appearing in the store\'s public Browse list happens after a quick human review (same discipline as the APK store). You can make an app private with a password (checked on our server — the files are not sent without it), copy its link, see how many times it ran, or unpublish it (which really deletes the published files) from Home → App Mart → My apps → My instant apps. Viewers can report a bad app from inside the player; a person reviews every report. REMIX — every instant app has a \'Make it yours\' button: one tap copies that app into YOUR OWN NavBharatAI Pro v5.0, ready to edit (works even signed out). SELLING APPS — COMING SOON: right now EVERY app on the store is FREE to remix, for everyone. Nobody is charged, no wallet is touched, and no price can be set — "Selling — coming soon" appears where the price control will be. The version being built pays the creator DIRECTLY INTO THEIR OWN BANK (their own payment account, money never held by NavBharatAI), which is why it is worth waiting for rather than shipping a version where earnings could only be spent on NavBharatAI credit. Do not tell a user they can sell an app or set a price today — they cannot. API KEYS ARE NEVER SOLD WITH AN APP: the original creator\'s API keys physically cannot ship (publishing is refused if a key is found in the code, and .env files never publish) — a buyer/remixer always brings their OWN keys, and the buy screen says so before any money moves; after remixing, NavBharatAI asks for your keys when you build. SHARED DATA — apps built with window.NavData (chat, guestbook, leaderboard, bookings) genuinely share their rows between all viewers once published on the store; in the creator\'s preview the same app works with per-device data until it is published. BUY ONCE, COPY FOREVER: when selling opens, a purchase is a PERMANENT entitlement, not a one-time download — the app is recorded against your account and you can take its code as many times as you like, free, for that ONE app only (never the rest of the store). Home → App Mart → My apps shows an \'Apps you own\' section with an Open & copy button for each; if the creator later removes the app from the store, your copy still stays yours. AFTER \'Make it yours\', v5.0 opens with a green confirmation naming the app (\'<app> is yours now\' / \'copied again\') and every file already loaded in the Files tab — if you ever see an empty chat instead, the files are still safe on the server and reopening the app from App Mart copies them again. GAME MODE — while playing any instant app there is a game-controller button in the top bar. It is ON by default, which stops long-press text selection, the copy menu and double-tap zoom, so a game feels like an app instead of a web page. Tap it once to turn game mode OFF when you want to select and copy text (a recipe, a story, a reference app); the button changes to a text cursor. Your choice is remembered separately for each app, and switching never restarts the app — a game in progress keeps running.',
    howToUse: 'Build your app in NavBharatAI Pro v5.0, then open Publish (the rocket) → find the green "Put it on App Mart" card next to "Make an Android app" → optionally add an APP ICON (shows as your app\'s logo on the store card instead of a globe) and up to 3 SCREENSHOTS (shown when someone opens your app\'s details on App Mart) → type the store name → tap "Publish to the store". THREE WAYS TO GET AN ICON (2026-08-19): "Add app icon" picks a picture from your device; "Paste" takes the picture that is on your clipboard; "Make icon" opens AI Image Gen in its own tab so you can create one — make it, press its Copy button, come back to the publish screen (still exactly as you left it) and press Paste. Whatever you give it is fitted automatically — square, resized and compressed — so a big AI-made picture is never refused for being too large. Icon and screenshots are optional but make the listing look real. The share link is copied automatically — send it to anyone; it opens the app full-screen in their browser with nothing to install. On App Mart, tapping an app\'s icon/name opens its details (description + your screenshots) with an Open button; "Open" still runs it directly. Manage it later under Home → App Mart → My apps (copy link, make private with a password, unpublish). A share link looks like navbharatai.com/store/app/<id> and needs no login to run a public app.',
    ...
    keywords: [
      ...
      'app mart', 'appmart', 'app market', 'nav app store', 'store kahan hai',
      'dusro ki app', 'games khelo', 'app khelo', 'free games',
      ...
      'apps you own', 'purchase ki app', 'dobara copy', 'code nahi mila', 'code kahan gaya',
      'buy once copy forever', 'kharidi hui app',
  ...
  // ─── PROFESSIONALS HUB ───────────────────────────────────────────────────
  {
    ...
    description: 'The hub for specialized professional AI assistants. Currently hosts Doctor AI (clinical decision support), NavBharatAI Pro v5.0 (the autonomous app builder), and a large roster of expert professionals. Future AI assistants will appear here. Every config-driven professional (Teacher, Lawyer, CA, Mentor, etc.) supports FILE ATTACHMENTS: click the paperclip (or paste a file) to send images, PDFs, Word/Excel/PowerPoint documents, ZIPs, or text/code files — the AI reads their real content and answers about them. CLOSING A CHAT (2026-08-19): pressing the ✕ on a professional\'s tab genuinely ENDS that conversation — reopening the same professional starts a fresh chat instead of resuming the old one. Nothing is lost: the closed conversation moves into Professional History, marked "Closed" with its date, and its Open button brings it back exactly where it was. Switching tabs, reloading or closing the app do NOT end a chat — only the ✕ does.',
    howToUse: 'Open Professionals from the sidebar, then choose the AI specialist you need. To share a file, click the paperclip button next to the message box (up to 4 files, 10 MB each) or paste an image directly. To finish a conversation, press ✕ on its tab — it is saved in Professional History (Professionals → History), where you can reopen or delete it. Up to 5 closed conversations are kept per professional, newest first.',
    relatedFeatures: ['doctor_ai', 'engineer_ai', 'professionals_cost'],
    keywords: ['professionals', 'specialists', 'experts', 'professional ai', 'specialist ai', 'doctor ai engineer ai', 'attach file', 'upload file', 'send photo', 'send pdf', 'file bhejo', 'photo bhejo', 'document upload', 'close chat', 'chat band karo', 'chat close nahi hota', 'end conversation', 'professional history', 'purani chat', 'reopen chat', 'new chat'],
  },
  {
  ...
  },
  {
    ...
    description: 'The settings hub. Organized into groups: Account & Profile; and App Settings — everything a real, live website needs, brought into ONE place (admin 2026-07-29): Domain (connect your own domain — DNS + SSL included), — note there is NO Hosting/Deploy tile here any more (removed 2026-08-20): every app is auto-hosted the moment it builds, and publishing (including to your own Vercel/Netlify/Cloudflare account) happens from the Publish button inside NavBharatAI Pro v5.0 — Database (connect your own DB — also provides login + storage when you connect Firebase/Supabase), Authentication (connect Clerk/Auth0 for login), Storage (connect S3/Cloudinary for uploads), Secrets & API Keys, plus Logs. GENERAL SETTINGS is now its OWN group (admin 2026-08-14), listed ABOVE App Settings — it holds View Mode, Theme, Accessibility, Chat Language and the app-signature toggle, THE "MADE BY NAVBHARATAI" SIGNATURE (Settings → General → "Made by NavBharatAI" Signature, updated 2026-08-21): every app you build carries a small "made by NavBharatAI" badge in the bottom-right corner that links to navbharatai.com. TURNING IT OFF IS A PAID FEATURE — the switch is locked for everyone else, and the card says so instead of moving and doing nothing. TWO THINGS THAT ARE OFTEN CONFUSED: (1) paying does NOT remove the badge by itself — an entitled user who has never touched the switch still gets the badge on every app; the entitlement only buys the RIGHT to turn it off, and the switch is still what turns it off. (2) The switch is only a preference saved on your device; NavBharatAI\'s server decides on every single build whether to honour it, so editing anything in the browser cannot remove the badge. HOW TO UNLOCK IT: it is part of the Custom Domain plan (₹99/month, paid from your normal wallet balance) — the SAME plan that removes the published-app badge, so ONE purchase removes BOTH. Buy it from Billing → Plans, then come back to Settings → General and turn the switch off. IF A USER ASKS "signature kaise hataye / badge remove karo / made by navbharatai kaise nikale": Billing → Plans se Custom Domain plan lein (₹99/month, wallet se), phir Settings → General me switch off karein — plan lene bhar se badge nahi hatta, switch off karna zaroori hai. i.e. how NavBharatAI itself looks, not how your built app is configured. Frontend + Backend CODE is built for you by NavBharatAI Pro, so they are not settings. The builder tools (AI Tools, Developer Tools, Publish & Deploy, Monetization & Team) live on the HOME page under the "Other AI" card (admin 2026-07-23; regrouped to four 2026-08-14 — "Design & Build" was folded into Developer Tools) — open Home → Other AI. (Git lives in the sidebar → Git, not in Settings.)',
  ...
  {
    id: 'settings_touch_feedback',
    name: 'Touch Feedback (Click Sound & Vibration)',
    path: 'Settings \u2192 General Settings \u2192 General \u2192 Accessibility \u2192 "Touch feedback"',
    description: 'Two independent on/off switches for what the app does when you tap something: CLICK SOUND (a soft "tak" tick on every tap \u2014 ON by default; it respects your phone\'s silent mode, so muting the phone silences it too) and VIBRATION (a short buzz on every tap \u2014 OFF by default, because the vibration motor feels heavy on many Android phones). They are separate switches on purpose, so any combination is reachable: sound only, buzz only, both, or complete silence. The choice is saved on your device (not your account) and applies from the very next tap \u2014 no restart needed. Touch feedback exists only in the installed Android/iOS app; in a web browser there is nothing to switch off.',
    howToUse: 'Open Settings \u2192 General Settings \u2192 General, scroll to Accessibility, and find "Touch feedback". Tap "Click sound" to turn the tap tick on or off, and "Vibration" to turn the buzz on or off. Each switch works on its own, and the change takes effect immediately on your next tap.',
    relatedFeatures: ['settings_general', 'settings_root', 'settings_reduce_motion', 'settings_font_scale'],
    keywords: ['sound', 'click sound', 'tap sound', 'touch sound', 'tick', 'beep', 'vibration', 'vibrate', 'haptic', 'haptics', 'buzz', 'silent', 'mute', 'turn off sound', 'sound off', 'sound band karo', 'awaz band karo', 'awaaz', 'click ki awaz', 'vibration band karo', 'vibration off', 'vibrate off', 'kampan', 'touch feedback', 'tap feedback', 'accessibility', 'annoying sound', 'noise'],
  },
  {
  ...
  },
  {
    ...
    path: 'Home → Developer Tools → Database Studio',
  ...
  {
    id: 'api_marketplace',
    name: 'API Marketplace — add a ready-made integration',
    path: 'Home → Developer Tools → API Marketplace',
    description: 'A catalogue of common third-party integrations (SMS/OTP, location, payments, maps and more) with a REAL, working code snippet for each. Pick one, read what it does and the steps to get its key from that provider, and tap Insert — the actual integration code is added to your app for you to wire up. The snippets are genuine working code (real fetch calls to the real provider), not placeholders. You still bring your OWN provider key — NavBharatAI never ships someone else\'s credentials — and the panel shows where to get it before you start.',
    howToUse: 'Open Home → Developer Tools → API Marketplace. Browse or search the integrations, tap one to see its description, the steps to get its API key, and the code. Tap Insert to drop the working snippet into your app, then add your own key where the code shows a placeholder.',
    relatedFeatures: ['settings_secrets', 'pro_chat'],
    keywords: [
      'api marketplace', 'api integration', 'add api', 'integration', 'sms', 'otp', 'msg91', 'payment api',
      'location api', 'maps api', 'api snippet', 'api code', 'ready made api', 'third party api',
      'api jodo', 'api lagao', 'integration add karo', 'api marketplace kahan hai',
    ],
    aiSurface: 'nbi_chat',
  },
  {
    id: 'localization',
    name: 'Localization — translate your app into many languages',
    path: 'Home → Developer Tools → Localization',
    description: 'Manage your app\'s text in up to 18 languages (Hindi, Tamil, Bengali, Telugu, Marathi and more, plus Spanish/French/Arabic/Chinese and others). Add translation keys, fill each language\'s value, and EXPORT real per-language JSON files (or all of them at once) to drop into your app; you can also import an existing JSON to merge it in. "Fill common strings" instantly fills the translations NavBharatAI already knows for common UI words (Welcome, Login, Save, Search, Settings…) across every active language — it is a built-in dictionary, not an AI call, so anything it does not recognise is left clearly for you to translate yourself (nothing is faked or machine-guessed).',
    howToUse: 'Open Home → Developer Tools → Localization. Turn on the languages you want on the left, add your translation keys, and type each translation. Tap "Fill common strings" to auto-fill the well-known UI terms across all active languages. Use Export to download a language\'s JSON (or all), and Import to merge an existing translations file.',
    relatedFeatures: ['pro_chat'],
    keywords: [
      'localization', 'localisation', 'translate app', 'translation', 'i18n', 'multi language',
      'multiple languages', 'hindi translation', 'language add karo', 'app ko translate karo', 'bhasha',
      'anuvad', 'translations json', 'export translations', 'localization kahan hai',
    ],
    aiSurface: 'nbi_chat',
  },
  {
    id: 'test_generator',
    name: 'Test Generator — auto-write tests for your code',
    path: 'Home → Developer Tools → Test Generator',
    description: 'Generates real Jest / React Testing-Library test CODE from your app\'s own code — unit, integration, edge-case and security tests, grouped by category. It reads your actual code (it finds your functions, and whether you use fetch/async) and writes matching tests, which you can expand to read, copy, or Export as a .test.js file to drop into your project. It does NOT claim to RUN the tests: a browser cannot execute your project\'s test suite, so it never shows a fake pass/fail — you run the generated tests in your own project, or with the Test Runner next to it. Everything it shows is a real generated test, nothing is faked.',
    howToUse: 'Open Home → Developer Tools → Test Generator. Tap "Generate Tests" and it writes tests from your current code. Filter by category, tap a test to expand its code, use Copy to grab one or "Export .test.js" to download them all, then run them in your project (or open the Test Runner to run tests).',
    relatedFeatures: ['pro_chat'],
    keywords: [
      'test generator', 'generate tests', 'write tests', 'unit test', 'test cases', 'jest', 'testing library',
      'test likho', 'test banao', 'automated tests', 'test code', 'test.js', 'test generate karo',
    ],
    aiSurface: 'nbi_chat',
  },
  {
    id: 'plugins_catalogue',
    name: 'Plugins — add a ready-made integration to your app',
    path: 'Home → Developer Tools → Plugins',
    description: 'A catalogue of popular ready-made integrations for the app NavBharatAI builds you — analytics (Google Analytics 4, Hotjar, Mixpanel, Microsoft Clarity), authentication (Firebase Auth UI, Clerk), payments (Razorpay, Stripe), storage (Cloudinary, Supabase Storage), UI libraries (shadcn/ui, Framer Motion), data fetching (SWR, TanStack Query) and security (reCAPTCHA, Zod). Each shows what it does and a short "how to set it up". Tap "Add to app" and its REAL setup code is inserted into your app for you to wire up — you then put that integration\'s own keys in .env (NavBharatAI never ships anyone else\'s keys). You can filter to just the ones you\'ve added.',
    howToUse: 'Open Home → Developer Tools → Plugins. Search or filter by category, tap a plugin to see its details and setup code, then tap "Add to app" to insert its real setup code into your app. Add the integration\'s keys in Settings → Secrets & API Keys / .env as the code shows.',
    relatedFeatures: ['api_marketplace', 'settings_secrets', 'pro_chat'],
    keywords: [
      'plugins', 'plugin', 'integration', 'add integration', 'google analytics', 'stripe', 'razorpay',
      'clerk', 'firebase auth', 'shadcn', 'framer motion', 'recaptcha', 'analytics add karo',
      'payment add karo', 'plugin lagao', 'integration jodo', 'plugins kahan hai',
    ],
    aiSurface: 'nbi_chat',
  },
  {
  ...
  },
  {
    ...
    path: 'Settings → App Settings → Secrets & API Keys — or, inside NavBharatAI Pro v5.0, the More (…) menu → Keys & Secrets',
    description: 'A secure, encrypted per-user vault for the API keys/secrets your BUILT APPS need — the safe place to store keys instead of pasting them into chat. When NavBharatAI Pro v5 builds an app that needs a key (e.g. OPENAI_API_KEY, STRIPE_SECRET_KEY, DATABASE_URL, a Supabase/Firebase key), it guides you where to get it and asks you to add it HERE, using the exact variable name the code reads. At build time those saved keys are injected automatically into the app\'s environment (.env) — so the app runs with the real key, which is never shown to the AI or exposed in chat, and never committed to git. Also stores GITHUB_TOKEN (NavBharatAI Pro v5.0 reads it to clone/push private repos) and Cashfree payment credentials. Keys are AES-256 encrypted, scoped to your account, and never shared. ASKED FOR WHILE THE APP IS BEING BUILT (2026-08-08): if v5.0 is building something that needs a key — a payment key, an SMS sender, a maps token — it now STOPS and asks you right there, with a small popup listing the exact key names and what each one is for. Type the values, tap Save, and they go into this same encrypted vault AND straight into your app\'s .env, so the build continues and uses them immediately — you never have to leave the screen or find this tile yourself. The popup has a MINIMISE button: tap it if you need to go and fetch a key from another site, and it shrinks to a one-line bar with whatever you already typed still there. You can also tap "Skip for now" — the build carries on and finishes, and that one feature is left visibly switched off rather than faked. Values are hidden as you type, are never shown to the AI, never written into your code and never committed to git. You never have to guess which keys an app needs: when a build finishes, NavBharatAI Pro v5.0 reads the app it just wrote and — in YOUR own language — lists any service that still needs a key from your own account (payments, email/SMTP, SMS/OTP, maps, your app\'s own AI key, storage, a hosted database), with the EXACT variable names and the exact screen to paste them into. It only lists what the built code genuinely uses, it never lists something you have already saved, and it never blocks the build. AND IT NOW TELLS YOU WHERE TO GET THE KEY IN THE FIRST PLACE (2026-08-17): every item in that list carries a direct link to the exact page in that service\'s own console where the key lives — Razorpay\'s API Keys screen, the Google app-password page, your Mapbox tokens — so you are never left holding a variable name with no idea where to find its value. NavBharatAI works out WHICH service your app actually uses from the app\'s own code, so an app built with Mapbox is sent to Mapbox and asked only for the Mapbox token, never for a Google key it would never read. The link is in your own language line, and it is a curated, human-checked list rather than something the AI makes up on the spot — so it does not invent a page that does not exist. For a service NavBharatAI does not have a checked link for, it honestly gives you the key name alone rather than a guessed link. And your app is never taken down by a key you haven\'t pasted yet: a feature whose key is missing is FROZEN as a visible, disabled "Coming soon" button that names the key it needs, while every other screen keeps working normally — v5.0 builds apps that refuse to crash at startup over one unset variable, and it never fakes a result (no pretend "payment successful", no OTP that always passes). Anything NavBharatAI can supply itself (the preview\'s PostgreSQL database) is provisioned automatically and never asked of you. YOUR SAVED DATABASE DETAILS ARE TESTED, NOT JUST COPIED: when a build loads your keys it actually opens a connection to any database connection string you saved and runs a real check, then tells you which keys were tested and working. If the details are stale, mistyped or the database has been deleted, you are told at the START of the build, in one line, naming this screen — instead of finding out from a preview that will not load. Keys we cannot test without spending your money (an API key, for example) are reported honestly as loaded-but-untested; a key is never silently dropped, and NavBharatAI never claims a key works when it has not checked. NAVBHARATAI ALSO WARNS YOU ABOUT THE TWO MISTAKES THAT HAPPEN AFTER YOU HAVE THE RIGHT KEY (2026-08-17), both of which leave the app looking perfectly fine: (1) if you saved a SECRET key under a name that starts with VITE_, NEXT_PUBLIC_ or REACT_APP_, that value gets printed inside your app\'s public code where any visitor can read it — NavBharatAI spots this, tells you to save it without that prefix and to change the key at the provider, because the old one must now be treated as public; (2) if you saved a TEST key (like one starting rzp_test_ or sk_test_), NavBharatAI reminds you that this is exactly right while building but cannot take real money — real payments will look successful and nothing will reach your account — so swap it for the live key before you share the app with customers. Both are notices, never blocks: your build still finishes normally. NavBharatAI only says this when it is certain — a key that is MEANT to be public (a publishable key, a Supabase anon key, a Mapbox public token) never triggers the warning. AND YOUR KEY IS NOW ACTUALLY TESTED THE MOMENT YOU SAVE IT (2026-08-17): after you tap Save, NavBharatAI asks the provider itself whether the key works and tells you straight away — a green "your key works, NavBharatAI checked it just now", or an honest "the provider did not accept this key, re-copy it". This works for Stripe, Razorpay, SendGrid, Resend, Twilio, Cloudinary, Mapbox, Clerk, OpenAI, Google AI Studio and Supabase, and it happens again at the start of every build so a key that stops working later is caught too. The check NEVER costs you money — it only ever reads (it never sends an SMS, mails anyone or charges a card), which is exactly why a Google Maps key and an MSG91 key are reported as "saved but not checked" instead: the only way to test those would spend your credit. And if a provider is simply down or slow, NavBharatAI says it could not check right now — it will never tell you a working key is wrong just because the provider had a problem. A key the provider rejects is still saved, never silently thrown away. THE FULL STEP-BY-STEP IS SHOWN RIGHT NEXT TO THE BOX YOU PASTE INTO (2026-08-17): as soon as you type a key name NavBharatAI recognises — on this screen, or in the popup that appears mid-build — it shows the exact page in that provider\'s console (a link you can tap), the exact clicks to get there, what it costs (free tier, card needed, KYC needed), and where that specific value hides — including warning you when a value is shown only ONCE so you copy it before closing. If the key must stay server-side it tells you BEFORE you paste not to put a VITE_ or NEXT_PUBLIC_ prefix on it. And where you do not need the key at all, it says so: UPI takes real payments with no gateway account and no fees, OpenStreetMap draws a real map with no token, and NavBharatAI can create a database for you in one tap — so the fastest fix is often to skip the console entirely. That "you may not need this" tip now appears in YOUR OWN LANGUAGE in the list NavBharatAI shows after a build, not only in English. ONE BUTTON CHECKS EVERY KEY YOU HAVE SAVED (2026-08-17): this screen has a "Check my keys" button that asks each provider whether your saved key still works and puts the answer next to its name — so a key that was revoked, rotated or expired is caught here instead of surfacing as a failed build later. It is not run automatically when you open the screen, because it makes real requests to those providers. YOUR EMAIL SETTINGS ARE CHECKED TOO: NavBharatAI signs in to your mail server exactly as your app would and tells you if the username and password are accepted — WITHOUT sending any email to anyone. If you use Gmail and it is refused, the usual cause is that the password must be a 16-character App Password rather than your normal Google password, and NavBharatAI says so. A key we cannot check for free (a Google Maps key, an MSG91 key, an Auth0 secret) is honestly shown as unchecked rather than guessed at — checking those would spend your own credit or quota. YOU CAN REACH THIS SAME VAULT FROM INSIDE PRO v5.0 TOO (2026-08-17): tap More (the … button at the bottom) → Keys & Secrets. It is the SAME place, not a second one — a key you add there appears here and vice versa, because both screens read and write one single vault for your account, and that is the vault your built app gets its keys from. It opens right inside v5.0 as a panel, so you do not lose your build, your preview or your chat while you add a key; the menu item also shows how many keys you already have saved. THE BUILD\'S LAST WORD IS THE ASK, NOT JUST "DONE" (2026-08-22): when a finished app still needs keys from your own accounts, the build now ENDS with a bright, unmissable card — "Your app is ready — one last step" — listing the exact keys with fill-in boxes right there, saved straight into this same encrypted vault (wired into the app the next time it starts; use the preview\'s restart/wake button to pick them up immediately). This happens automatically on every build, not only when the AI thinks to ask. Every key row has a "Guide me" button: tap it and a ready-made question drops into the message box — send it and NavBharatAI walks you to that exact key step by step in your own language, asking what you see on screen and telling you the next click, like a person guiding you over the phone. You can also tap "I\'ll do this later" — the app stays built and published, that one feature stays visibly off, and the keys can always be added here. AND YOU ARE NEVER LOCKED INTO THE PROVIDER THE APP WAS BUILT WITH (2026-08-22): the card also carries a \'switch to a different provider\' link — tap it and a ready question lands in the message box; send it and NavBharatAI lists your real alternatives (a different payment gateway, or keyless options like UPI which needs no gateway account at all) and rewires the app to the one you choose, as a normal edit.',
  ...
  },
  {
    ...
    description: 'On/off toggle (default ON) that adds a small "made by NavBharatAI" badge to the bottom-right corner of every app you build. The badge links to navbharatai.com, so anyone you share your app with can discover NavBharatAI too. TURNING IT OFF IS A PAID FEATURE (2026-08-21) — for everyone else the switch is locked and the card says why, rather than moving and quietly doing nothing. TWO THINGS PEOPLE GET WRONG: (1) paying does NOT remove the badge on its own — someone entitled who never touches the switch still gets the badge on every app; the entitlement buys the RIGHT to turn it off, the switch still has to be turned off. (2) The switch only records a preference on your device — NavBharatAI\'s server decides on every single build whether to honour it, so nothing edited in the browser can remove the badge. HOW TO UNLOCK IT: it is part of the Custom Domain plan (₹99/month, paid from your normal wallet balance) — the SAME plan that removes the published-app badge, so ONE purchase removes BOTH badges. Buy it from Billing → Plans, then turn this switch off in Settings → General.',
    howToUse: 'Open Settings → General and find "Made by NavBharatAI" Signature. If your account is entitled, toggle it off and your NEXT build is made without the badge. If it is locked, the note under it tells you why: removing the badge is part of the Custom Domain plan (₹99/month from your wallet). Buy it from Billing → Plans, then come back and turn the switch off — paying alone does not remove the badge, the switch still has to be turned off. Never look for a workaround: the decision is made on the server on every build, so changing anything in the browser has no effect.',
    relatedFeatures: ['settings_general', 'settings_root'],
    keywords: ['signature', 'made by navbharatai', 'watermark', 'badge', 'branding', 'remove badge', 'built by', 'credit', 'hataye', 'signature hatao', 'badge hatao', 'watermark hatao', 'badge kaise hataye', 'signature kaise hataye', 'made by navbharatai kaise nikale', 'toggle off nahi ho raha', 'signature off nahi ho raha', 'paid feature', 'subscription', 'locked', 'custom domain plan', 'billing plans', '99 rupees', 'plan lena hai'],
  },
  {
    id: 'settings_modules',
...
• Structured server logs queryable via GET /api/admin/logs`,
  ...
  },
  {
    ...
    howToUse: 'Open Team Collaboration and check the bell icon (top-right) for mentions. Under the hood a collaboration surface POSTs { "text": "…@handle…" } to /api/team/:teamId/mentions/notify to deliver, and each user reads their inbox from GET /api/mentions and marks read via POST /api/mentions/read. (Those two paths were /api/notifications until 2026-08-21, where they collided with the admin broadcast inbox registered on the same path — Express served the first registration, so the mention inbox silently read the wrong store and always showed an empty list. Separate paths now, one shape each.)',
