...
• NATIVE TOOL-USE engine on Claude. The "Power" selector in the build-options menu (the gear/settings popover above the message box) picks the build engine's reasoning power: Off = normal (fastest & lowest cost — billed at the Sonnet-equivalent token cost × 1.2, or × 3 if Sonnet itself runs); the three Opus power levels — "Strong 💪" (low effort), "Powerful Force" (high effort), and "Full Team" (ultracode / max effort) — run Claude Opus 4.8 at increasing reasoning effort and are billed at the REAL Opus token cost × 2 at every level (a higher level makes Opus think more, so the bill scales with the real tokens spent, not a fixed multiplier). Leave it Off for everyday builds; raise it only for genuinely complex/stuck work.
