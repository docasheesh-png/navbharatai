...
• A REAL TERMINAL INSIDE YOUR APP — 30 FREE MINUTES A DAY (2026-08-18): the Terminal tab now has TWO parts. On top is the build log (what NavBharatAI ran while building your app). Below it is a REAL terminal you can type in — it runs inside your app's own machine, so you can run npm commands, look at files, check versions, anything a developer would. USE IT WHEN: you want to check something yourself, or a developer friend is helping you. FREE, WITH A DAILY LIMIT of 30 minutes per day, which resets every day. The limit exists for an honest reason: an open terminal keeps a real cloud machine running and NavBharatAI pays for that time, so a tab left open all night would cost real money. Time counts only while the terminal is actually open. When you are close to the limit NavBharatAI warns you in the terminal itself, and when it runs out it says so clearly instead of just going silent — YOUR APP AND FILES ARE NEVER AFFECTED, only the terminal pauses until tomorrow. IF A USER ASKS "terminal band kyun ho gaya / 30 minute khatam": explain warmly that it resets tomorrow, their app is untouched, and building/editing through chat has no such limit.
