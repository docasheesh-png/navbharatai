...
describe('bundleForPreview (server-side esbuild)', () => {
  ...
  it('maps subpath imports (firebase/app) with a trailing-slash importmap entry', async () => {
    // The real-world failure: "Failed to resolve module specifier firebase/app".
    // Importmaps need BOTH "firebase" and "firebase/" entries.
    const files = {
      'package.json': JSON.stringify({ dependencies: { react: '^18.3.1', 'react-dom': '^18.3.1', firebase: '^10.12.0' } }),
      'index.html': '<body><div id="root"></div><script type="module" src="/src/main.tsx"></script></body>',
      'src/main.tsx': `import { createRoot } from 'react-dom/client';\nimport { initializeApp } from 'firebase/app';\nimport { getAuth } from 'firebase/auth';\nconst app = initializeApp({});\ngetAuth(app);\ncreateRoot(document.getElementById('root')!).render(<div>FB_APP</div>);`,
    };
    const html = await bundleForPreview(files);
    const imapRaw = html.match(/<script type="importmap">(.+?)<\/script>/s)![1].replace(/<\\\//g, '</');
    const imap = JSON.parse(imapRaw);
    // Bare entry AND trailing-slash entry must both exist.
    expect(imap.imports['firebase']).toBeTruthy();
    expect(imap.imports['firebase/']).toBeTruthy();
    expect(imap.imports['firebase/']).toMatch(/esm\.sh\/firebase@[\d.]+\/$/);
    // The externalized subpath imports survive in the bundle (not bundled away).
    expect(html).toContain('firebase/app');
