...
function statusBadgeClass(status: number): string {
  if (status >= 200 && status < 300) return 'bg-green-500/20 text-success border-green-500/40';
  if (status >= 400 && status < 500) return 'bg-yellow-500/20 text-warn border-yellow-500/40';
  if (status >= 500) return 'bg-red-500/20 text-danger border-red-500/40';
...
const KVEditor: React.FC<{
  ...
  return (
    <div className="space-y-1">
      {rows.map((row, i) => (
        <div key={i} className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder="Key"
            className="flex-1 bg-surface border border-line rounded px-2 py-1 text-xs text-body placeholder-faint focus:outline-none focus:border-indigo-500"
          />
          <input
            value={row.value}
            ...
            className="flex-1 bg-surface border border-line rounded px-2 py-1 text-xs text-body placeholder-faint focus:outline-none focus:border-indigo-500"
          />
          <button onClick={() => remove(i)} className="text-faint hover:text-danger transition-colors">
            <Trash2 size={12} />
          </button>
        </div>
      ))}
      <button
        onClick={add}
        className="flex items-center gap-1 text-xs text-accent-text hover:text-accent-text mt-1"
      >
...
const APITester: React.FC<APITesterProps> = () => {
  ...
  return (
    <div className="relative flex h-full bg-surface text-body font-mono text-sm overflow-hidden">
      {/* ── History slide-out drawer (mobile-friendly) ─────────────── */}
      {historyOpen && (
        <div className="absolute inset-0 bg-well z-30" onClick={() => setHistoryOpen(false)} aria-hidden />
      )}
      <div
        className={cn(
          'absolute top-0 left-0 h-full w-[270px] max-w-[82%] bg-card border-r border-line z-40 flex flex-col transition-transform duration-200 ease-out',
          historyOpen ? 'translate-x-0' : '-translate-x-full'
        )}
      >
        <div className="px-3 py-2.5 border-b border-line flex items-center justify-between text-xs text-muted uppercase tracking-wide">
          <span className="flex items-center gap-2"><Clock size={12} /> History</span>
          <button onClick={() => setHistoryOpen(false)} className="p-1 -mr-1 text-faint hover:text-body transition-colors">
            <X size={16} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto">
          {history.length === 0 && (
            <p className="text-xs text-faint px-3 py-4 text-center">No saved requests</p>
          )}
          {history.map(cfg => (
            <button
              key={cfg.id}
              onClick={() => loadFromHistory(cfg)}
              className="w-full text-left px-3 py-2.5 hover:bg-raised border-b border-line transition-colors group"
            >
              <div className={cn('text-[10px] font-bold', METHOD_COLORS[cfg.method])}>{cfg.method}</div>
              <div className="text-xs text-muted truncate">{cfg.name}</div>
              <div className="flex items-center justify-between mt-0.5">
                <span className="text-[10px] text-faint truncate">{cfg.url}</span>
                <button
                  onClick={e => { e.stopPropagation(); setHistory(prev => prev.filter(h => h.id !== cfg.id)); }}
                  className="opacity-60 sm:opacity-0 sm:group-hover:opacity-100 text-faint hover:text-danger transition-all p-1 -mr-1"
                >
                  <Trash2 size={12} />
                </button>
              </div>
        ...
        <div className="border-t border-line px-2 py-2 space-y-1">
          <p className="text-[10px] text-faint uppercase tracking-wide mb-1">Quick Tests</p>
          {QUICK_TESTS.map(qt => (
            <button
              key={qt.label}
              onClick={() => handleQuickTest(qt)}
              className="w-full text-left flex items-center gap-1 text-[11px] text-accent-text hover:text-accent-text hover:bg-indigo-500/10 rounded px-1.5 py-1.5 transition-colors"
            >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="px-2 sm:px-4 py-2 bg-card border-b border-line flex flex-col gap-2">
          {/* Row 1: history toggle + method + URL */}
          <div className="flex items-center gap-2">
            <button
              onClick={() => setHistoryOpen(o => !o)}
              title="History & quick tests"
              className="relative flex items-center gap-1.5 px-2 py-1.5 rounded border border-line text-muted hover:text-body hover:border-indigo-500/50 transition-colors shrink-0"
            >
              <Clock size={14} />
              <span className="hidden sm:inline text-xs">History</span>
              {history.length > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-indigo-600 text-on-accent text-[9px] leading-none rounded-full min-w-[15px] h-[15px] px-1 flex items-center justify-center">
                  {history.length}
                </span>
              )}
            </button>
            ...
            <div className="relative shrink-0">
              ...
              {methodOpen && (
                <div className="absolute top-full left-0 mt-1 bg-card border border-line rounded shadow-xl z-50">
                  {HTTP_METHODS.map(m => (
                    <button
                      key={m}
                      onClick={() => { setMethod(m); setMethodOpen(false); }}
                      className={cn('block w-full text-left px-4 py-1.5 text-xs font-bold hover:bg-raised transition-colors', METHOD_COLORS[m])}
                    >
                      {m}
                    </button>
                  ))}
            ...
            <input
              ...
              className="flex-1 min-w-0 bg-surface border border-line rounded px-3 py-1.5 text-sm text-body placeholder-faint focus:outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-x-3 gap-y-2 flex-wrap">
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed rounded text-on-accent text-xs font-semibold transition-colors flex-1 sm:flex-none"
            >
            ...
            <button
              ...
              title="Save request"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-line text-faint hover:text-accent-text hover:border-indigo-500/50 transition-colors text-xs shrink-0"
            >
              <Save size={14} /> <span className="hidden sm:inline">Save</span>
            </button>

            {/* CORS bypass toggle — route cross-origin requests through NavBharatAI's SSRF-guarded proxy. */}
            <label className="flex items-center gap-2 text-[11px] text-muted cursor-pointer select-none" title="Cross-origin APIs are blocked by the browser (CORS). Routing through NavBharatAI's server fetches them for you.">
              <input
                type="checkbox"
                checked={useProxy}
        ...
        {insecureTarget && (
          <div className="flex items-start gap-2 px-2 sm:px-4 py-1.5 bg-amber-500/10 border-b border-amber-500/20 text-[11px] text-warn">
        ...
        {/* Save request input */}
        {showSaveInput && (
          <div className="px-4 py-2 bg-card border-b border-line flex items-center gap-2">
            ...
            <button
              ...
              className="text-xs bg-indigo-600 hover:bg-indigo-500 px-3 py-1 rounded text-on-accent transition-colors"
            >
              Save
            </button>
          </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col border-b border-line overflow-hidden">
          {/* Tabs */}
            ...
            {tabs.map(tab => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id
                    ? 'text-accent-text border-b-2 border-indigo-500 bg-surface'
          ...
          <div className="flex-1 overflow-y-auto p-3">
                ...
                {['POST', 'PUT', 'PATCH'].includes(method) ? (
                  <>
                    <div className="flex justify-end">
                      <button
                        onClick={formatJson}
                        className="text-[10px] text-accent-text hover:text-accent-text flex items-center gap-1"
                      >
                    ...
                    <textarea
                      ...
                      className="flex-1 w-full bg-surface border border-line rounded px-3 py-2 text-xs text-body placeholder-faint focus:outline-none focus:border-indigo-500 resize-none font-mono"
                      rows={8}
                    />
                  </>
                ) : (
            ...
            {activeTab === 'auth' && (
              <div className="space-y-3">
                <label className="block text-xs text-muted">Bearer Token</label>
                <input
                  value={authToken}
                  onChange={e => setAuthToken(e.target.value)}
                  placeholder="Enter token..."
                  type="password"
                  className="w-full bg-surface border border-line rounded px-3 py-1.5 text-xs text-body placeholder-faint focus:outline-none focus:border-indigo-500"
                />
                <p className="text-[10px] text-faint">
                  Adds <code className="text-accent-text">Authorization: Bearer &lt;token&gt;</code> header.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-3 py-1.5 bg-card border-b border-line shrink-0">
            ...
            {response && (
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={handleCopy}
                  className="flex items-center gap-1 text-xs text-faint hover:text-accent-text transition-colors"
                >
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-surface">
            {loading && (
              <div className="flex items-center gap-2 text-accent-text text-xs">
                <TirangaLoader size={14} /> Sending request...
              </div>
            )}
            {error && (
              <pre className="text-xs text-danger whitespace-pre-wrap leading-relaxed">{error}</pre>
            )}
            ...
            {response && !showRawHeaders && (
              <pre className="text-xs text-muted whitespace-pre-wrap leading-relaxed">{response.body}</pre>
            )}
            {response && showRawHeaders && (
              <div className="space-y-1">
