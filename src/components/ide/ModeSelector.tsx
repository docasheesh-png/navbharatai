    ...
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={cn(
          ...
          isAuto
            ? "bg-indigo-600/20 border-indigo-500/40 text-accent-text hover:bg-indigo-600/30"
      ...
      {isOpen && (
        <div className="absolute bottom-full left-0 mb-2 w-48 bg-surface border border-line rounded-xl shadow-2xl p-1 z-[200]">
          {modes.map((m) => (
            <button
              key={m.id}
              ...
              className={cn(
                ...
                mode === m.id
                  ? m.id === 'auto' ? "bg-indigo-600/20 text-accent-text" : "bg-indigo-600/20 text-accent-text"
