...
• RESILIENT HTTP CLIENT (generate_http_client): v5.0 can add a real HTTP client (server/lib/http.ts) — a dependency-free fetchJson(url, { timeoutMs, headers, method, body }) that adds a REAL timeout via AbortController (bare fetch has none, so a dead upstream hangs the request forever), throws HttpError on any non-2xx status (native fetch resolves on 4xx/5xx — a classic silent bug) and returns parsed JSON. Use for every third-party API call; wrap with the retry recipe for automatic backoff. No keys.
...
• INDIAN MONEY FORMATTING (generate_money_format): v5.0 can add real Indian money/number formatting (server/lib/money.ts) — dependency-free formatInr(paise) → a correctly lakh/crore-grouped ₹ string (12345678 → "₹1,23,456.78"), formatIndianNumber(n) → Indian-grouped counts, and rupeesInWords(rupees) → the amount in words (lakh/crore) for invoices/cheques. Built on native Intl en-IN so grouping is right (NOT Western thousands). Store money as integer paise. Distinct from currency conversion. No keys.
...
• COST BREAKDOWN ("Why this cost?"): after a billed build, a "Why this cost?" toggle appears next to the ₹ amount in the result footer — expand it to see exactly how the charge was computed: your input/output token split, the tier that ran, the base (Sonnet-equivalent or real-Opus) cost, the markup multiplier applied, and the final USD → ₹. Free builds show no charge to explain.`,
    ...
    keywords: [
      ...
      'http client', 'fetch timeout', 'fetchjson', 'api call', 'request timeout', 'fetch json', 'call external api', 'third party api', 'axios alternative', 'hanging request',
