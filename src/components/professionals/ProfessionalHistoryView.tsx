    ...
    <div className="flex-1 flex flex-col bg-surface h-full overflow-hidden p-6">
      <div className="flex items-center gap-3 mb-5">
        {onBack && (
          <button
            onClick={onBack}
            className="text-xs font-bold text-muted hover:text-ink transition-colors px-2.5 py-1.5 rounded-lg hover:bg-raised"
            aria-label="Back"
          >
            ← Back
          </button>
      ...
      {all.length > 0 && (
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full bg-card border border-line rounded-xl pl-8 pr-8 py-2 text-[11px] text-ink placeholder-faint outline-none focus:border-teal-500/50 transition-colors"
          />
          {searchQuery && (
            <button
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-6">
            <div className="w-20 h-20 bg-teal-600/10 border border-teal-600/20 rounded-[2rem] flex items-center justify-center">
              <Briefcase className="w-10 h-10 text-success" />
            </div>
          ...
          items.map((item) => (
            <motion.div
              ...
              className="border rounded-2xl p-6 bg-card border-line hover:border-teal-500/30 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
            >
              <div className="space-y-2 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="font-bold text-ink text-base leading-snug">{item.name}</h3>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border bg-teal-500/10 text-success border-teal-500/25">
                    <MessageSquare className="w-2.5 h-2.5" /> {item.turns} message{item.turns !== 1 ? 's' : ''}
                  </span>
                  {item.endedAt !== undefined && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border bg-raised text-muted border-line">
                      Closed {new Date(item.endedAt).toLocaleDateString()}
                    </span>
                  )}
                </div>
                {item.preview && <p className="text-[12px] text-muted leading-relaxed line-clamp-2">{item.preview}</p>}
              </div>

              <div className="flex items-center gap-3 shrink-0 self-start sm:self-auto">
                <button
                  onClick={() => openOne(item)}
                  className="flex items-center justify-center gap-2 px-5 py-2.5 bg-teal-600 hover:bg-teal-500 active:scale-95 text-on-accent font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg shrink-0"
                >
                ...
                <button
                  ...
                  className={cn(
                    ...
                    'bg-raised hover:bg-red-500/10 border-line text-muted hover:text-danger',
                  )}
