...
• TERMINAL — a REAL terminal, and you can open MANY at once: the "+ New" button inside the terminal opens a dropdown listing every terminal you have, with an X to close each one and "New Terminal" at the bottom. Switching between them keeps each one's output, history and any still-running command alive (nothing is thrown away), so you can watch a dev server in one and run commands in another. Closing the last terminal closes the panel. Each command runs in YOUR v5.0 sandbox and shows the actual stdout/stderr (e.g. ls, cat package.json, npm run build, node -v). Each command is bounded by a 30-second timeout with capped output (no runaway processes), and ↑/↓ recall your command history. It needs a warm sandbox — start or continue a build in NavBharatAI Pro v5.0 to activate it; until then it honestly says the sandbox isn't active (it never fakes output). Type "clear" to clear the screen.
...
• SETTINGS — workspace and IDE configuration.`,
  ...
  },
  {
    ...
    description: 'A REAL shell into the sandbox where NavBharatAI Pro v5.0 builds your app — the same terminal Code Studio has, reachable straight from Settings so you can run a quick command (ls, cat package.json, npm run build, node -v) without opening the full IDE. Every line shown is the actual stdout/stderr from YOUR live sandbox; commands are bounded by a 30-second timeout with capped output, and ↑/↓ recall history. It shares the exact workspace with Pro v5.0, Code Studio, Files, and Preview — a file the build wrote is immediately visible to ls here. Needs a warm sandbox: start or continue a build in NavBharatAI Pro v5.0 first; until then it honestly says the sandbox is not active (it never fakes output).',
    howToUse: 'Settings → App Settings → Terminal. Type a command and press Enter. If it says the sandbox is dormant, send a message in NavBharatAI Pro v5.0 chat to bring it back online, then run your command.',
    relatedFeatures: ['ide_terminal', 'settings_logs', 'pro_chat'],
    keywords: ['terminal settings', 'shell', 'command line', 'console', 'bash', 'terminal', 'run command', 'command chalao', 'npm install', 'terminal kahan'],
  },
  {
    id: 'settings_logs',
    name: 'Logs (Settings)',
