...
function FileDiffView({ path, patch, op }: { path: string; patch: string; op: 'create' | 'edit' }) {
  ...
  return (
    <div className="rounded-md border border-line overflow-hidden bg-raised">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-1.5 px-2 py-1 text-[11px] text-muted hover:text-body bg-raised"
      >
        ...
        <span className={`ml-auto shrink-0 text-[9px] uppercase tracking-wide ${op === 'create' ? 'text-success' : 'text-info'}`}>{op === 'create' ? 'new' : 'edit'}</span>
      </button>
      {open && (
        <pre className="text-[10.5px] leading-[1.5] font-mono overflow-x-auto px-2 py-1 max-h-64 overflow-y-auto">
          {shown.map((ln, i) => {
            ...
            const cls = add ? 'text-success bg-emerald-500/[0.06]'
              : del ? 'text-danger bg-red-500/[0.06]'
              : hunk ? 'text-info'
              : 'text-faint';
            return <div key={i} className={`whitespace-pre ${cls}`}>{ln || ' '}</div>;
          })}
          {lines.length > MAX_DIFF_LINES && (
            <div className="text-faint pt-0.5">… +{lines.length - MAX_DIFF_LINES} more lines — full diff in the Diff tab</div>
          )}
        </pre>
      )}
    </div>
...
function entryIcon(e: ActivityEntry) {
  const t = e.text || '';
  if (e.kind === 'file' || /^(writing|editing) /.test(t)) return <FileCode className="w-3.5 h-3.5 text-faint shrink-0" />;
  if (/^running: /.test(t)) return <TerminalIcon className="w-3.5 h-3.5 text-faint shrink-0" />;
  if (/^reading /.test(t)) return <BookOpen className="w-3.5 h-3.5 text-faint shrink-0" />;
  if (/^searching|^listing files/.test(t)) return <Search className="w-3.5 h-3.5 text-faint shrink-0" />;
  if (e.kind === 'agent') return <Bot className="w-3.5 h-3.5 text-faint shrink-0" />;
  if (e.kind === 'preview') return <Globe className="w-3.5 h-3.5 text-faint shrink-0" />;
  return <ClipboardList className="w-3.5 h-3.5 text-faint shrink-0" />;
}
      ...
      <div className="max-w-[90%] w-full">
        <button
          ...
          className={`group/act inline-flex items-center gap-2 max-w-full rounded-lg border px-2.5 py-1.5 text-xs touch-manipulation transition-colors ${
            block.failed
              ? 'border-red-500/30 bg-red-500/5 text-danger hover:bg-red-500/10'
              : 'border-line bg-raised text-muted hover:bg-card hover:text-body'
          }`}
        >
          {block.active
            ? <TirangaLoader className="w-3 h-3 text-accent-text shrink-0" />
            : block.failed
              ? <X className="w-3 h-3 text-danger shrink-0" />
              : <Check className="w-3 h-3 text-success shrink-0" />}
          <span className="truncate">{block.summary}</span>
          {block.progress && <span className="text-accent-text shrink-0">· {block.progress}</span>}
          {block.stats && (block.stats.plus > 0 || block.stats.minus > 0) && (
            <span className="shrink-0 font-mono text-[11px]">
              <span className="text-success">+{block.stats.plus}</span>{' '}
              <span className="text-danger">-{block.stats.minus}</span>
            </span>
          )}
          <ChevronRight className={`w-3.5 h-3.5 shrink-0 text-faint transition-transform ${open ? 'rotate-90' : ''}`} />
        </button>
        {open && (
          <div className="mt-1 ml-1.5 border-l border-line pl-3 py-1 space-y-1">
            {rows.slice(0, MAX_DETAIL_ROWS).map((e) => (
              <div key={e.id} className="flex items-center gap-2 text-[11px] text-faint min-w-0">
                {entryIcon(e)}
                ...
                {(() => {
                  ...
                  return label ? <span className="shrink-0 text-faint hidden sm:inline">· {label}</span> : null;
                })()}
                {e.active
                  ? <TirangaLoader className="w-3 h-3 text-accent-text shrink-0" />
                  : e.ok === false
            ...
            {rows.length > MAX_DETAIL_ROWS && (
              <div className="text-[11px] text-faint">+{rows.length - MAX_DETAIL_ROWS} more</div>
            )}
            {hasCommand && (
              <div className="text-[10px] text-faint pt-0.5">Full command output → Terminal tab</div>
            )}
          </div>
        )}
        ...
        {files.length > 0 && (
          <div className="mt-1">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-md border border-line bg-raised px-2 py-1 text-[11px] text-muted hover:text-body hover:bg-card touch-manipulation"
            >
              <FileDiff className="w-3 h-3 shrink-0 text-accent-text" />
              ...
              <ChevronRight className={`w-3.5 h-3.5 shrink-0 text-faint transition-transform ${diffOpen ? 'rotate-90' : ''}`} />
            </button>
            {diffOpen && (
              <div className="mt-1 space-y-1.5">
                {shownFiles.map((f) => <FileDiffView key={f.path} path={f.path} patch={f.patch} op={f.op} />)}
                {files.length > MAX_DIFF_FILES && (
                  <div className="text-[10px] text-faint">+{files.length - MAX_DIFF_FILES} more changed files — see the Diff tab</div>
                )}
              </div>
            )}
          </div>
