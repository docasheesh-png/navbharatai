...
export function editModePrefix(fileTree: string[] = []): string {
  ...
  return [
    ...
    'editing, VERIFY the app still builds and runs — run `npx --no-install tsc --noEmit` (or the',
...
export function architectSystemPrompt(framework?: string, opts?: { parallelBuild?: boolean }): string {
  ...
  return [
    ...
    '- **THE APP MUST LOOK PROFESSIONALLY DESIGNED — raw-HTML looks are a defect.** Every',
    '  app ships with a real visual design, not browser defaults: a coherent layout (flex/grid',
    '  panels, cards with border+radius+padding, consistent 4/8/12/16/24px spacing), styled',
    '  buttons/inputs with hover and focus-visible states, CSS variables on :root for the',
    '  palette (background/text/muted/accent/card/border), a system-ui font stack, clear',
    '  hierarchy (one accent color, muted secondary text) and empty states. The global',
    '  stylesheet MUST be imported by the entry file (e.g. `import \'./index.css\'` in',
    '  main.tsx) and every className used in components must exist in it — an unimported or',
    '  mismatched stylesheet ships an unstyled app, which is a failed requirement.',
    ...
    '  directory"); just run commands directly (e.g. `npm install`, `npx --no-install tsc --noEmit`).',
    '- ALWAYS type-check with `npx --no-install tsc` (never bare `npx tsc`). The `--no-install`',
    '  flag uses the project\'s LOCAL typescript; bare `npx tsc` DOWNLOADS an unrelated squatter',
    '  package ("This is not the tsc command you are looking for") when typescript isn\'t installed',
    '  yet — so if it errors "tsc not found", run `npm install` first, then re-run the type-check.',
    '- MID-BUILD TYPE CHECK: after every 5 file writes (or before starting a new',
    '  feature), run: bash "npx --no-install tsc --noEmit 2>&1 | head -30". Fix ALL TypeScript',
    ...
    '- SECRETS / API KEYS the app needs (Stripe, OpenAI, a database URL, any provider key): NEVER ask',
    '  the user to paste a key into the chat — chat is not a safe place for a secret. Instead, when the',
    ...
    '- ⚛️ REACT ERROR BOUNDARY — write the CANONICAL class ONCE; never thrash re-rewriting it. A real',
    '  SaaS-dashboard build burned ~5 minutes rewriting ErrorBoundary 6× (class → functional → "public',
    '  state" → …) fighting a TypeScript class-field error. ROOT CAUSE: Vite\'s tsconfig sets',
    '  `useDefineForClassFields: true`, so a class FIELD `state = { hasError: false }` can fail to compile',
    '  ("Property has no initializer" / init-order). THE FIX (do this the FIRST time, do NOT loop): set',
    '  state in the CONSTRUCTOR, not a class field — `constructor(props){ super(props); this.state = {',
    '  hasError: false }; }` — with `static getDerivedStateFromError(){ return { hasError: true }; }`,',
    '  `componentDidCatch(error, info){ console.error(error, info); }`, and a `render()` returning a',
    '  fallback when `this.state.hasError` else `this.props.children`. That is the whole component and it',
    '  compiles first try under Vite + TS — no functional-component workaround, no repeated rewrites.',
