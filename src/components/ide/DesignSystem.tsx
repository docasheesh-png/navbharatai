...
const DEFAULT_TOKENS: DesignTokens = {
  colors: [
    ...
    { name: 'text', value: 'var(--text-body)', description: 'Primary text' },
    { name: 'text-muted', value: 'var(--text-muted)', description: 'Muted text' },
    { name: 'text-subtle', value: 'var(--text-muted)', description: 'Subtle text' },
  ],
  spacing: [
  ...
  },
  {
    ...
    preview: (_: string) => (
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8, padding: '16px', width: '100%', maxWidth: 180 }}>
        ...
        <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>Card content here.</div>
      </div>
    ),
  },
  {
    ...
    <div style={containerStyle}>
      ...
      <div style={{ background: '#1e293b', borderBottom: '1px solid #334155', padding: '12px 16px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 600, fontSize: 15, color: 'var(--text-body)' }}>Design System</div>
            <div style={{ color: 'var(--text-muted)', fontSize: 11 }}>Tokens, components & style guide</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 6, overflowX: 'auto', maxWidth: '100%', paddingBottom: 2 }}>
          {(['colors', 'typography', 'spacing', 'components', 'export'] as const).map(t => (
            <button key={t} onClick={() => setActiveTab(t)} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 500, flexShrink: 0, background: activeTab === t ? '#a855f7' : '#334155', color: activeTab === t ? '#fff' : 'var(--text-muted)' }}>
        ...
        {activeTab === 'colors' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ ...cardStyle, display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-body)' }}>Describe your brand</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 8 }}>
              {tokens.colors.map((color, idx) => (
                <div key={idx} style={{ ...cardStyle, cursor: 'pointer' }} onClick={() => setEditingColor(editingColor === idx ? null : idx)}>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-body)' }}>--color-{color.name}</div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 2 }}>
                    <span style={{ ...monoStyle, color: 'var(--text-muted)' }}>{color.value}</span>
                    <button onClick={e => { e.stopPropagation(); copyText(color.value, color.name); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 0 }}>
                      {copied === color.name ? <Check size={10} color="#4ade80" /> : <Copy size={10} />}
                    </button>
                  </div>
                  <div style={{ fontSize: 10, color: 'var(--text-faint)', marginTop: 2 }}>{color.description}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {tokens.typography.map((t, idx) => (
              <div key={idx} style={{ ...cardStyle, display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: t.fontFamily, fontSize: Math.min(t.fontSize, 32), fontWeight: t.fontWeight, lineHeight: t.lineHeight, color: 'var(--text-body)' }}>
                    {t.name.charAt(0).toUpperCase() + t.name.slice(1)} — NavBharatAI
                  </div>
                </div>
                <div style={{ flexShrink: 0, textAlign: 'right', minWidth: 0 }}>
                  <div style={{ ...monoStyle, color: 'var(--text-muted)' }}>{t.fontFamily} · {t.fontSize}px · {t.fontWeight}</div>
                  <div style={{ fontSize: 10, color: 'var(--text-faint)', marginTop: 1 }}>line-height: {t.lineHeight}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            {tokens.spacing.map((s, idx) => (
              <div key={idx} style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ ...monoStyle, color: 'var(--text-muted)' }}>{s.value}{s.unit}</div>
                </div>
                <button onClick={() => copyText(`var(--spacing-${s.name})`, `spacing-${s.name}`)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                {tokens.borderRadius.map(r => (
                  <div key={r.name} style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ ...monoStyle, color: 'var(--text-muted)', fontSize: 10 }}>{r.value}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
              {(['preview', 'code'] as const).map(m => (
                <button key={m} onClick={() => setPreviewMode(m)} style={{ padding: '5px 12px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, background: previewMode === m ? '#334155' : 'transparent', color: previewMode === m ? '#e2e8f0' : 'var(--text-muted)' }}>
