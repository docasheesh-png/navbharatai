...
• IDE ↔ v3.0 FILE SYNC: a ZIP you upload in Code Studio is mirrored into the v3.0 workspace automatically, and files you delete in the IDE are removed from v3.0 too — so v3.0 always knows exactly which files exist and builds/edits on top of your real, current file set (the IDE and v3.0 share one workspace per session).
...
• HONEST billing: you are charged the Claude Opus-equivalent token cost × 2.5 (or × 5 in Only-Opus mode); a live cost estimate is shown.`,
  ...
  },
  {
    ...
    description: 'See exactly what NavBharatAI Pro v3.0 wrote. Open the Files tab in the v3.0 header to get the list of every file in your project; each row shows the file name, a colored status dot (green = newly created, yellow = modified) and its LINE COUNT (how many lines were actually written inside that file). Click any file to open it and read its full contents inside the builder (a back arrow returns you to the list). The contents are pulled live from your real project, not a guess. In addition, every file v3.0 builds is automatically synced into the app’s main Files view (sidebar → Files) once a build finishes, so you can browse the same project from there too (heavy generated folders like node_modules, dist and .git are left out so it stays the clean source). The live "Plan" checklist shown above the chat input can be minimized or expanded with a single tap so it never eats the chat area — collapsed, it still shows the current step.',
    howToUse: 'Build or open a project in NavBharatAI Pro v3.0. Tap the "Files" tab in the header to list the files, then tap a file name to read its contents (tap the back arrow to return). After a build finishes, open the main app sidebar → Files to see the same project files there. To shrink the Plan checklist above the message box, tap the "Plan" header (the chevron) — tap again to expand it.',
