...
export const STARTER_TEMPLATES: StarterTemplate[] = [
  ...
  {
    id: 'wedding-rsvp', label: 'Shaadi RSVP', icon: '💍', category: 'Personal', tier: 'simple',
    prompt: 'Build a wedding guest list and RSVP manager for an Indian wedding: add guests with a name, phone number, which side they are from (bride or groom), how many people are coming with them, and which events they are invited to (mehendi, haldi, sangeet, the wedding, the reception). Mark each guest as invited, coming, not coming or no reply yet, and show a live head count per event so the caterer can be given a real number. Filter by side, by event and by RSVP status, search by name or phone, and show totals for confirmed, pending and declined. Save everything in the browser so the list survives a reload, and allow the whole list to be exported as CSV. Hindi labels with English in brackets, mobile-first because the list is kept on a phone, light/dark mode.',
  },
  ...
  {
    id: 'courier', label: 'Courier', icon: '📦', category: 'Business', tier: 'pro',
    prompt: 'Build a courier and parcel tracking app for a small Indian delivery business: book a consignment with a sender, a receiver with a phone number and address, the pickup and destination city, the weight, the declared value and whether it is prepaid or to-pay, and generate a tracking number for it. Every consignment carries a status history — booked, picked up, in transit, out for delivery, delivered, or returned — where each change is stamped with the date and a place, so the history is a real audit trail rather than one overwritten field. Show a dashboard of how many parcels are in each status and how much cash is pending on the to-pay ones, a consignment list searchable by tracking number, phone or receiver name, and a public-style tracking view that shows one consignment journey as a timeline. Price a booking from the weight and whether the destination is local, within the state or outside it. Rupees throughout, save everything in the browser, mobile-friendly, light/dark mode.',
  },
  {
    id: 'ngo', label: 'NGO', icon: '🤝', category: 'Business', tier: 'pro',
    prompt: 'Build a donation and volunteer manager for an Indian NGO or trust: record donors with a name, phone, email and PAN, and record each donation with the amount, the date, the mode (UPI, cash, cheque or bank transfer) and which programme it was given for. Issue a numbered receipt for every donation, including the 80G declaration line, and be able to reprint any past receipt. Keep a register of volunteers with their skills and availability, and a list of programmes with a target amount each, showing how much has been raised against the target as a progress bar. A dashboard shows the total raised this financial year, the number of donors, the largest donation and the split by payment mode. Filter donations by programme, by mode and by date range, search donors by name or phone, and export the donation register as CSV for the accountant. Indian financial year from April to March, rupees throughout, save everything in the browser, mobile-friendly, light/dark mode.',
  },
  {
    id: 'school-erp', label: 'School ERP', icon: '🏫', category: 'Business', tier: 'pro',
    prompt: 'Build a school management system for an Indian school: classes from one to twelve with sections, students enrolled in a class with an admission number, roll number, parent name and parent phone, and teachers with the subjects they teach. Mark daily attendance for a whole class in one screen and show each student attendance percentage. Record marks per student per subject for named exams (unit test, half yearly, annual) and generate a report card that computes the total, the percentage and a grade. Track fees with a term wise fee structure per class, record payments with a receipt number, and show who has dues outstanding and how much. A dashboard shows the total students, today attendance percentage, fees collected this term and fees pending. Search students by name, admission number or parent phone, filter by class and section, and export a class list as CSV. Rupees throughout, save everything in the browser, mobile-friendly, light/dark mode.',
  },
];
