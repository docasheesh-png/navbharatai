...
describe('wiring — the policy actually governs the router, and the outcome reaches the log', () => {
  ...
  it('🔒 it says "we do not know" about tokens rather than writing a zero', () => {
    ...
    // is named for; `usageMeasured` lives in this object however long the object grows, so the object
    // is what to read. Same fix as `tests/helpers/sourceSlice.ts` exists for.
